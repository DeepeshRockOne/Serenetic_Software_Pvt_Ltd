<?php include (dirname(__DIR__)).'/../includes/connect.php';
...
if (isset($_POST['s_submit'])) {
  ...
  if ($image['name'] != "") {
    if (!in_array($image['type'], array("image/jpeg", "image/jpg", "image/pjpeg", "image/x-png", "image/png","application/msword","text/pdf","application/pdf"))) {
      $error = 'Only .jpg, .png, .pdf, .doc, .docx File';
    }
  }
  if ($image['name'] && $error=="") {
    ...
    //for local
   // $destination = $_SERVER['DOCUMENT_ROOT'] . '/cyberxgroup/uploads/ckeditor/' . $filename;
    //for live
    
    $destination = $CKEDITOR_DIR . $filename;
    ...
    $insParams=array(
        ...
        'location'=> makeSafe('Product Management'),
      ...
      <?php
        ...
        foreach ($fileRow as $file) {
          ...
          if ($file) {
            ...
            if (!preg_match('/\.(jpg|jpeg|png|gif)$/i', $file['file_name'])){
              $str = sprintf('<div class="img_base"><img src="%s" width="%d" height="%d" title="%s" alt=""><div class="delte_icon_div"><a href="javascript:void(0);" onclick="return del_image('.$file['id'].')" class="delete">Delete</a></div></div>', $dir . '/' .'default.png', 150, 84, $fileBaseName
            );
            }else{
