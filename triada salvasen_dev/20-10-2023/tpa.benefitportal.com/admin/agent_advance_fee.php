...
if(!empty($advFeeId)){
    $feeSql = "SELECT pm.id,pm.name,pm.product_code,pmt.price_calculated_on,pmt.price_calculated_type,pm.initial_purchase,pm.is_benefit_tier,pm.is_fee_on_renewal,pm.fee_renewal_type,pm.fee_renewal_count,pmt.pricing_effective_date,pmt.pricing_termination_date,pm.advance_month,pmt.price
            FROM prd_main pm
            JOIN prd_matrix pmt ON(pm.id=pmt.product_id AND pmt.is_deleted='N')
...
$template = "agent_advance_fee.inc.php";
