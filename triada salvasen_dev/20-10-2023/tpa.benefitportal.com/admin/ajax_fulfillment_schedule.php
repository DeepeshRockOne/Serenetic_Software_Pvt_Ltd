...
// step-2 variables
    ...
    $eligiblity_sql = "SELECT * FROM fulfillment_schedule WHERE is_deleted='N' AND file_id=:id";
    $where_id = array(':id' => $file_id);
    $fulfillment_res = $pdo->selectOne($eligiblity_sql, $where_id);
    ...
    if($cancel_processing == 'Y'){
        if($fulfillment_res){
            ...
            $pdo->update("fulfillment_schedule",$req_data,$req_where); 
        }
        $req_where = array(
            "clause"=>"id=:id",
            "params"=>array(
        ...
        $description['ac_message'] =array(
            ...
            'ac_message_1' =>' deleted scheduled delivery for fulfillment file ',
            'ac_red_2'=>array(
              'title'=> $file_name,
            ),
          ); 
          activity_feed(3, $_SESSION['admin']['id'], 'Admin', 0, 'fulfillment_files','Admin deleted scheduled delivery for fulfillment file', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($description));
...
if ($validate->isValid()) {
        ...
        if (!empty($fulfillment_res) && is_array($fulfillment_res)) {
            if(($fulfillment_res['schedule_type'] != $schedule_type) || ($fulfillment_res['schedule_end_type'] != $schedule_end_type)){
            ...
            $pdo->update("fulfillment_schedule", $ins_params, $up_where);

            $next_schedule_date = next_fulfillment_schedule($fulfillment_res['id'],date('Y-m-d'));
            ...
            $activityFeedDesc['ac_message'] =array(
              'ac_red_1'=>array(
                'href'=>$ADMIN_HOST.'/admin_profile.php?id='.md5($_SESSION['admin']['id']),
                'title'=>$_SESSION['admin']['display_id']),
              'ac_message_1' =>' Updated Fulfillment Schedule ',
              'ac_red_2'=>array(
                'href'=>'',
                'title'=>$file_name,
              ),
            ...
            if(!empty($activityFeedDesc) && !empty($activityFeedDesc['key_value']['desc_arr'])){ 
                activity_feed(3, $_SESSION['admin']['id'], 'Admin', $file_id, 'fulfillment_files','Admin Updated Fulfillment Schedule', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($activityFeedDesc));
            }

        }else{
            $scheduled_id = $pdo->insert("fulfillment_schedule",$ins_params);    
            generate_fulfillment_request($scheduled_id);
 
            //  next scheduled date code start
            $next_schedule_date = next_fulfillment_schedule($scheduled_id,date('Y-m-d'));
            if($next_schedule_date != ''){
                $req_where = array(
                "clause"=>"id=:id",
                "params"=>array(
            ...
            $description['ac_message'] =array(
              ...
              'ac_message_1' =>' Scheduled Fulfillment File ',
              'ac_red_2'=>array(
                //'href'=>  '',
                'title'=>$file_name,
              ),
            ); 
            activity_feed(3, $_SESSION['admin']['id'], 'Admin', $file_id, 'fulfillment_files','Admin Scheduled Fulfillment File', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($description));
        }
    $response['status'] = 'success';
} else {
