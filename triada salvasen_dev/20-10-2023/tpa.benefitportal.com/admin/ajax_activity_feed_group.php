...
if(isset($_REQUEST['export']) && $_REQUEST['export'] == 'export_activity') {
    ...
    $extra_params['activit_feed']['entity_type'] = 'Group';
    $job_id = add_export_request_api('EXCEL',$_SESSION['admin']['id'],'Admin',"Group History","group_history",$tmp_incr,$sch_params,$extra_params,'group_history');
    $reportDownloadURL = $AWS_REPORTING_URL['group_history']."&job_id=".$job_id;
...
try {
    ...
    $sel_sql = "SELECT af.* FROM activity_feed af LEFT JOIN customer c on ((c.id=af.entity_id or c.id=af.user_id) and c.type='Group') WHERE  af.entity_action!='New Order'  AND  ((user_id=:id or entity_id=:id) AND (af.user_type='Group' or af.entity_type='Group'))  AND af.entity_type!='leads' AND af.is_deleted ='N' $incr $key_incr AND af.is_deleted='N' ORDER BY $SortBy $currSortDirection $limit_incr";
