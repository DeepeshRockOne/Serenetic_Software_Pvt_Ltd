...
$options = array(
	...
	'url' => 'weekly_completed_commission.php?' . $query_string,
	'db_handle' => $pdo->dbh,
...
if ($is_ajaxed) {
	try {
		$sel_sql = "SELECT cs.customer_id,cs.id as commId,cs.type as commType,cs.status,
				    ...
				    WHERE cs.commission_duration='weekly' AND s.type !='Customer' AND cs.status NOT IN('Pending') AND cs.is_deleted='N' $incr 
	...
	include_once 'tmpl/weekly_completed_commission.inc.php';
	exit;
}

$template = 'weekly_completed_commission.inc.php';
