...
$is_ajaxed_leads = isset($_REQUEST['is_ajaxed_leads']) ? $_REQUEST['is_ajaxed_leads'] : '';
...
if($type=='Lead' && $is_all_activity_popup=='Y'){
...
try {     
	$limit_incr = ' LIMIT '.$from_limit.','.$limit;
		...
		$sel_sql = "SELECT af.* FROM activity_feed af 
                JOIN leads l ON ((l.id=af.entity_id AND af.entity_type IN('Lead','leads')) OR (l.id=af.user_id AND af.user_type IN('Lead','leads')))
...
if(!$is_ajaxed_leads)
{
	$sel_activity_type_user = $rpdo->select("SELECT DISTINCT entity_action FROM activity_feed WHERE user_type IN('leads','Lead')");
	$sel_activity_type_ent = $rpdo->select("SELECT DISTINCT entity_action FROM activity_feed WHERE entity_type IN('leads','Lead')");
	$activity_type_res = array_merge($sel_activity_type_user,$sel_activity_type_ent);
}

include_once 'tmpl/global_activity_lead_tabs.inc.php';
?>