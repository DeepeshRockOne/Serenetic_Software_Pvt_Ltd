...
if(!empty($ws_res['product_ids'])) {
    $product_res = $pdo->selectOne("SELECT is_gap_plus_product FROM prd_main WHERE id in(".$ws_res['product_ids'].") AND is_deleted = 'N'");
    if($product_res){
        $is_gap_product = 'Y';
    }
}
...
if ($is_update) {
    ...
    $primary_benefit_field_arr = $spouse_benefit_field_arr = $child_benefit_field_arr = array();
        ...
        foreach ($spouse_ids as $spouseKey => $spouse_id) {
            ...
            if(!empty($prd_spouse_field)){
                foreach($prd_spouse_field as $field_key => $row) {
                    ...
                    if($row['label'] == 'email' && !empty($control_value)){
    ...
    if ($validate->isValid()) {
        ...
        /*--- Primary Data ----*/
            ...
            if(!empty($primary_product_arr)) {
                ...
                foreach($primary_product_arr as $product_id => $val){
                    ...
                    if(isset($primary_in_patient_benefit[$product_id])){
                        $benefitAmountParams['in_patient_benefit'] = $primary_in_patient_benefit[$product_id];
                    }
                    if(isset($primary_out_patient_benefit[$product_id])){
                        $benefitAmountParams['out_patient_benefit'] = $primary_out_patient_benefit[$product_id];
                    }
                    if(isset($primary_monthly_income[$product_id])){
                        $benefitAmountParams['monthly_income'] = $primary_monthly_income[$product_id];
                    }
        ...
        activity_feed(3,$_SESSION['admin']['id'], 'Admin' , $cust_row['id'], 'customer', 'Admin Updated Member Health Details',$_SESSION['admin']['name'],"",json_encode($ac_desc));
        /*-----/Activity Feed ------*/
...
foreach ($ba_res as $key => $ba_row) {
    ...
    $primary_benefit_amount['monthly_benefit'][$ba_row['product_id']] = $ba_row['monthly_benefit'];
}
...
$benefitArr = array('benefit_amount','in_patient_benefit','out_patient_benefit','monthly_income'/*,'benefit_percentage'*/);
    ...
    if(isset($primary_benefit_amount['monthly_benefit'])){
        foreach ($primary_benefit_amount['monthly_benefit'] as $p_id => $amount) {
           $prd_benefit_amount['Primary'][$p_id]['monthly_benefit'][] = $amount;
        }
    }
