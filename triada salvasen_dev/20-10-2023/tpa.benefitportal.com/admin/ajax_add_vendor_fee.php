...
if (!empty($products) && !empty($vendor_id) && !$validate->getError('effective_date')) {
  $fincr = "";
  $fparams = array();
  if (!empty($products)) {
  ...
  if(!empty($vendor_id)){
    $fincr .= " AND md5(pf.id) = :vendorId"; 
    $fparams[":vendorId"] = $vendor_id;
  } 
  ...
  if(!empty($resTermPrd)){
    $validate->setError("products", "Product " . $resTermPrd["assignedProductCode"] . " is conflicted with another fee for this Vendor");
  }
}
if(!empty($products) && empty($vendor_id) && !empty($vendor_fee_id) && !$validate->getError('effective_date')){
  $fincr = "";
  $fparams = array();
  if (!empty($products)) {
    $fincr .= " AND pa.product_id IN(" . implode(',', $products) . ")";
  ...
  if (!empty($vendor_fee_id)) {
    $fincr .= " AND pa.fee_id IN(" . implode(',', $vendor_fee_id) . ")";
  }
  if (!empty($product_id) && $is_clone == 'N') {
    $fincr .= " AND md5(pa.fee_id) != :id";
    $fparams[":id"] = $product_id;
...
if($validate->isValid()) {
  ...
  if(!empty($vendor_id)){
    ...
    $vendor = $pdo->selectOne($getsql, $params);
    if(!empty($vendor)){
      $insert_params['prd_fee_id']=$vendor['id'];
    }
  }
  ...
  if(!empty($productRow)){
    ...
    //************* Activity Code Start *************
      ...
      $activityFeedDesc['ac_message'] =array(
        ...
        'ac_message_1' =>' Updated Vendor Fee ',
        'ac_red_2'=>array(
          //'href'=>'',
    ...
    array_push($vendor_fee_id, $product_id);
    ...
    //************* Activity Code Start *************
      $description['ac_message'] =array(
        ...
        'ac_message_1' =>' Created Vendor Fee ',
        'ac_red_2'=>array(
          //'href'=>  '',
          'title'=>$display_fee_id,
        ),
      ); 
      activity_feed(3, $_SESSION['admin']['id'], 'Admin', $product_id, 'vendor','Admin Vendor Fee', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($description));
    //************* Activity Code End *************
  ...
  if($is_benefit_tier == 'Y'){
    ...
    $feePlanSql = "SELECT id,product_id FROM prd_matrix WHERE is_deleted='N' AND product_id=:product_id and plan_type = :plan_type";
  ...
  if($productRow){
    //************* Activity Code Start *************       
      if(!empty($activityFeedDesc) && !empty($activityFeedDesc['key_value']['desc_arr'])){ 
        activity_feed(3, $_SESSION['admin']['id'], 'Admin', $productRow['id'], 'vendor','Admin Updated Vendor Fee', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($activityFeedDesc));
      }
    //************* Activity Code End   *************
  }
