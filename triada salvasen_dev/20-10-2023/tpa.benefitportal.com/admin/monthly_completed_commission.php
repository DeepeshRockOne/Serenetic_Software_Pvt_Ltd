...
$options = array(
	...
	'url' => 'monthly_completed_commission.php?' . $query_string,
	'db_handle' => $pdo->dbh,
...
if ($is_ajaxed) {
	try {
		$sel_sql = "SELECT cs.customer_id,cs.id as commId,cs.type as commType,
		 			   s.rep_id as agentRepId,cs.status,cs.pay_period,CONCAT(s.fname,' ',s.lname) as age_name,
				    ...
				    WHERE cs.commission_duration='monthly' AND s.type !='Customer' AND cs.status NOT IN('Pending') $incr 
				    GROUP BY cs.customer_id,cs.status ORDER BY $SortBy $currSortDirection";
	...
	include_once 'tmpl/monthly_completed_commission.inc.php';
	exit;
}

$template = 'monthly_completed_commission.inc.php';
