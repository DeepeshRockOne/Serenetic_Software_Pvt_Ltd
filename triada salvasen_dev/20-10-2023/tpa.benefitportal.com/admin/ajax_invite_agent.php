...
$commission_type = checkIsset($_POST['commission_type']);
...
$validate->string(array('required' => true, 'field' => 'commission_type', 'value' => $commission_type), array('required' => 'Please select option'));
...
if ($validate->isValid()) {
	...
	if (count($res_enroll) > 0) {
		...
		$enrollment_url = $ADMIN_HOST.'/invite_agent.php';
	...
	//Assign healthy steps

	$sel_product = "SELECT apr.* from agent_product_rule apr 
	JOIN prd_main p ON(apr.product_id=p.id and apr.is_deleted='N') 
	WHERE apr.agent_id=:agent_id and p.type='Fees' and p.product_type='Healthy Step' and p.is_deleted='N'";
    $variation_prd = $pdo->select($sel_product,array(":agent_id"=>$sponsor_id));

    if($variation_prd){
    	foreach ($variation_prd as $key => $value) {
	...
	setNotifySuccess("Agent contract created successfully");
	$response['link'] = $link;
