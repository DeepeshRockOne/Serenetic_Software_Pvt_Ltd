...
if($validate->isValid()){
    ...
    if(!empty($resNewAgent['id'])){
      ...
      $actFeed=$functionsList->generalActivityFeed($tmp,$tmp2,$link,$display_id,$resAgent['agentId'],$user_type,'Admin Updated Parent Agent',$msg);

      // Update Agent Members Level and Upline Sponsors
      // $web_id = $pdo->selectOne("SELECT GROUP_CONCAT(id) as ws_id from website_subscriptions where customer_id=:id",array(":id"=>$customerInfo['id']));

      // if(!empty($web_id['ws_id'])){
      //   $selMbrs = "SELECT GROUP_CONCAT(id) as ids FROM customer_enrollment WHERE website_id IN(".$web_id['ws_id'].")";
      //   $resEnrolledMbrs = $pdo->selectOne($selMbrs, array(":id" => $agentId));
      //   if(!empty($resEnrolledMbrs['ids'])){
      //     $enrollmentIds = explode(',',$resEnrolledMbrs['ids']);
      //     $updParams = array(
      //       "sponsor_id" => $newAgentId,
      //       "level" => $newAgentLevel+1,
      //       "upline_sponsors" => $newAgentUplineSponsor,
      //     );
