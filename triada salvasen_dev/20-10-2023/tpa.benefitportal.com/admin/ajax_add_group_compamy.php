...
if ($validate->isValid()) {
	...
	if(!empty($resCompany)){
        ...
        $group_update_activity = $pdo->update("group_company", $params, $update_where,true);
        ...
        $description['ac_message'] = array(
	        'ac_red_1'=>array(
	            'href' => 'agent_detail_v1.php?id='.md5($_SESSION['admin']['id']),
		...
		foreach($group_update_activity as $key2 => $val){
            if(array_key_exists($key2,$params)){
                ...
                if($key2 == "phone") {
                    ...
                    $params[$key2] = format_telephone($params[$key2]);
                }
                $key2_tmp = ucwords(str_replace('_',' ',$key2));
                $description['key_value']['desc_arr'][$key2_tmp] = ' Updated From '.$val." To ".$params[$key2].".<br>";
                $flg = "false";
            } else {
    	...
    	activity_feed(3,$_SESSION['admin']['id'],'Admin',$group_name['id'],'Group','Admin Updated Group Company Details',$_SESSION['admin']['fname'],$_SESSION['admin']['lname'],$desc);
        /*---/Activity Feed -----*/
	}else{
		$ins_company_id = $pdo->insert('group_company', $params);
        ...
        $desc=json_encode($description);
    	activity_feed(3,$_SESSION['admin']['id'],'Admin',$group_name['id'],'Group','Admin Added Company In Group',$_SESSION['admin']['fname'],$_SESSION['admin']['lname'],$desc);
        /*---/Activity Feed -----*/
	}
