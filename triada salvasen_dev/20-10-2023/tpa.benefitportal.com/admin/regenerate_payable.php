...
if($isJoin){
	$joinIncr = "LEFT JOIN orders o ON(o.id=ro.order_id)
...
$options = array(
	...
	'url' => 'regenerate_payable.php?' . $query_string,
	'db_handle' => $pdo->dbh,
...
if ($is_ajaxed) {
	...
	// Read Commisssions Payables activity code start
	  $description['ac_message'] =array(
	    ...
	    'ac_message_1' => ' read regenerate payables page',
	  ); 

	  activity_feed(3, $_SESSION['admin']['id'], 'Admin',0, 'regenerated_payable','Regenerate Payables', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($description));
	...
	try {
		$sel_sql = "SELECT 
					...
					rc.debit,
					rc.status,
					COUNT(DISTINCT(ro.order_id)) as orderCounts,
					a.display_id as adminDispId,
					CONCAT(a.fname,' ',a.lname) as adminName
				FROM regenerated_payable rc
				JOIN admin a ON(rc.admin_id=a.id) 
				LEFT JOIN regenerated_order_payable ro ON(rc.id=ro.request_id)
				$joinIncr
	...
	include_once 'tmpl/regenerate_payable.inc.php';
	exit;
}
...
$template = 'regenerate_payable.inc.php';
