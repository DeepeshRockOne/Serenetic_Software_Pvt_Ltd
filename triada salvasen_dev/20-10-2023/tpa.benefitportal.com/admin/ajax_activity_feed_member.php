...
if(isset($_REQUEST['export']) && $_REQUEST['export'] == 'export_activity') {
    ...
    $extra_params['activit_feed']['entity_type'] = 'Customer';
    $job_id = add_export_request_api('EXCEL',$_SESSION['admin']['id'],'Admin',"Member History","member_history",$tmp_incr,$sch_params,$extra_params,'member_history');
    $reportDownloadURL = $AWS_REPORTING_URL['member_history']."&job_id=".$job_id;
...
try {
    ...
    $sel_sql = "SELECT af.* FROM activity_feed af 
                JOIN customer c on ((c.id=af.entity_id AND af.entity_type='Customer') OR (c.id=af.user_id AND af.user_type='Customer')) 
                WHERE c.id=:id  AND c.type='customer' AND af.is_deleted ='N' $incr $key_incr ORDER BY $SortBy $currSortDirection $limit_incr";
    $sch_params[':id'] = $id;
