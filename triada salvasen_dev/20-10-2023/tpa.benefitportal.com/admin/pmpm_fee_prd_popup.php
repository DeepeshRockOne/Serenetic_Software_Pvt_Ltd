...
$products = $pdo->select("SELECT p.name,p.product_code,p.id,p.status 
						  ...
						  WHERE pcrap.rule_id = :id AND pcrap.is_deleted = 'N' AND p.is_deleted = 'N' group by p.id order by p.name ASC",array(':id' => $pmpm_fee_id));
...
$template = 'pmpm_fee_prd_popup.inc.php';
