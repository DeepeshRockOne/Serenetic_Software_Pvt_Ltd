...
has_access(6);
    ...
    $group_sql = "SELECT md5(c.id) as id,c.id as _id,c.business_name,c.address,c.address_2,c.city,c.state,c.zip,c.business_phone,c.business_email,c.fname,c.lname,c.cell_phone,c.email,c.public_name,c.public_email,c.public_phone,c.user_name,cs.display_in_member,cs.tpa_for_billing,cs.is_valid_address,cgs.group_size,cgs.group_in_year,cgs.ein,cgs.business_nature,cgs.sic_code,cgs.employer_company_common_owner,cgs.invoice_broken_locations,cs.signature_file,cgs.automated_communication,cs.is_2fa,cs.via_sms,cs.via_email,cs.send_otp_via,cs.is_ip_restriction,cs.allowed_ip,cs.agent_contract_file,c.status
        FROM customer c
    ...
    if(!empty($group_row)){ 
        ...
        $automated_communication = !empty($group_row['automated_communication']) ? explode(",",$group_row['automated_communication']) : array();
