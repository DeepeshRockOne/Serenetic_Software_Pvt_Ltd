...
if($is_ajaxed && $is_delete){
    ...
    if($action == 'Delete' && !empty($dep_rows['id'])){
        ...
        activity_feed(3,$_SESSION['admin']['id'], 'Admin' , $rows['id'], 'customer','Admin Deleted Dependent',$_SESSION['admin']['name'],"",json_encode($ac_desc));
...
if(!empty($customer_id) && $is_dep_ajaxed){
    ...
    if ($validate->isValid()) {
        ...
        if($action == 'Edit'){
            ...
            $ac_action = 'Admin Update Dependent Detail';
        }else{
            $display_id = 
            $ac_ms_1 = ' Added new Dependent For Member ';
            $ac_action = 'Admin Added New Dependent';
        }
        ...
        if($activity_feed){
            ...
            activity_feed(3,$_SESSION['admin']['id'], 'Admin' , $customer_id, 'customer', $ac_action,$_SESSION['admin']['name'],"",json_encode($ac_desc));
