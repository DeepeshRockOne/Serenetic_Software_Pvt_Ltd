...
$options = array(
    ...
    'url' => 'weekly_pending_hrm_payments.php?' . $query_string,
    'db_handle' => $pdo->dbh,
...
if ($is_ajaxed) {
    try {
        $sel_sql = "SELECT hrmp.group_id AS groupId,hrmp.status,COUNT(DISTINCT hrmp.payer_id) AS memberId,
						s.rep_id AS groupRepId,hrmp.pay_period,c.is_compliant,
						CONCAT(s.fname,' ',s.lname) AS groupName,hrmp.pay_date,
						...
						WHERE hrmp.hrm_payment_duration='weekly' AND hrmp.is_deleted='N' AND hrmp.status IN('Pending') $incr
    ...
    include_once 'tmpl/weekly_pending_hrm_payments.inc.php';
    exit;
}

$template = 'weekly_pending_hrm_payments.inc.php';
