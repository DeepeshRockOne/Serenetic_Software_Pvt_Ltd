...
function filterHTML() {
  ...
  $tmp.find('*[contenteditable]').each(function() {
    $(this).removeAttr('contenteditable')
  });
  $tmp.find('.delete').each(function() {
    $(this).remove();
  });
  $tmp.find('#edit_link').each(function() {
    $(this).remove();
  });
  $tmp.find('[class=""]').each(function() {
...
function openPopup() {
  ...
  if (typeof icon === 'undefined') {
    ...
    $tmp.append('<div style="text-align: center; padding-bottom: 40px;"><img src="img/icons/' + icon + '"></div>')
  }
...
function generateThumbs() {
  ...
  if (count > el) {
    ...
    else if (the_thumb.indexOf('http') !== -1) {
      $('#modules_widgets [data-id="' + the_module + '"]').append('<img src="' + the_thumb + '">');
    }
