...
$breadcrumbes[1]['link'] = 'admin_fees.php';
$breadcrumbes[2]['title'] = 'Manage Admin Fees';

$page_title = "Manage Admin Fees";
...
if($fee_id > 0){
	$sqlFeeProduct="SELECT group_concat(id) as feeProduct FROM prd_main where find_in_set(:fee_id,admin_fee_ids) AND is_deleted='N'";
...
$sqlProduct="SELECT * FROM prd_main where id=:id and product_type='Admin Fee'";
...
if($fee_id > 0){
	if($resProduct){
		...
		$admin_fee_ids_products = array();

		$checkAdminFee="SELECT group_concat(id) as productList FROM prd_main where is_deleted='N' AND FIND_IN_SET(:fee_id,admin_fee_ids)";
		$resAdminFee=$pdo->selectOne($checkAdminFee,array(":fee_id"=>$fee_id));

		$admin_fee_ids_products = explode(",", $resAdminFee['productList']);
	}else{
		setNotifyError("Admin Fee Not Found");
		redirect("admin_fees.php");
	}
}
...
if($iframe==1){
	$template = "manage_admin_fees.inc.php";
