...
$options = array(
	...
	'url' => 'monthly_pending_commission.php?' . $query_string,
	'db_handle' => $pdo->dbh,
...
if ($is_ajaxed) {
	try {
		 $sel_sql = "SELECT cs.customer_id,cs.id as commId,cs.type as commType,cs.status,
				    ...
				    WHERE cs.commission_duration='monthly' AND s.type !='Customer' AND cs.is_deleted='N' AND cs.status IN('Pending') $incr 
	...
	include_once 'tmpl/monthly_pending_commission.inc.php';
	exit;
}

$template = 'monthly_pending_commission.inc.php';
