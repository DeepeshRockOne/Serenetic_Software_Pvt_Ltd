...
// step-2 variables
    ...
    $eligiblity_sql = "SELECT * FROM eligibility_schedule WHERE is_deleted='N' AND file_id=:id";
    $where_id = array(':id' => $file_id);
    $eligibility_res = $pdo->selectOne($eligiblity_sql, $where_id);
    ...
    if($cancel_processing == 'Y'){
        if($eligibility_res){
            ...
            $pdo->update("eligibility_schedule",$req_data,$req_where); 
        }
        $req_where = array(
            "clause"=>"id=:id",
            "params"=>array(
...
if ($validate->isValid()) {
        $file_name = "";
        if($file_id){
            $file_name = getname('eligibility_files',$file_id,'file_name','id');
        }
        ...
        if (!empty($eligibility_res) && is_array($eligibility_res)) {
            if(($eligibility_res['schedule_type'] != $schedule_type) || ($eligibility_res['schedule_end_type'] != $schedule_end_type)){
            ...
            $pdo->update("eligibility_schedule", $ins_params, $up_where);

            $next_schedule_date = next_eligibility_schedule($eligibility_res['id'],date('Y-m-d'));
            ...
            $activityFeedDesc['ac_message'] =array(
              ...
              'ac_message_1' =>' Updated Eligibility Schedule ',
              'ac_red_2'=>array(
                'href'=>'',
                'title'=>$file_name,
              ),
            ...
            if(!empty($activityFeedDesc) && !empty($activityFeedDesc['key_value']['desc_arr'])){ 
                activity_feed(3, $_SESSION['admin']['id'], 'Admin', $file_id, 'eligibility_files','Admin Updated Eligibility Schedule', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($activityFeedDesc));
            }

        }else{
            $scheduled_id = $pdo->insert("eligibility_schedule",$ins_params);    
            generate_eligibility_request($scheduled_id);
 
            //  next scheduled date code start
            $next_schedule_date = next_eligibility_schedule($scheduled_id,date('Y-m-d'));
            if($next_schedule_date != ''){
                $req_where = array(
                "clause"=>"id=:id",
                "params"=>array(
            ...
            $description['ac_message'] =array(
              ...
              'ac_message_1' =>' Scheduled Eligibility File ',
              'ac_red_2'=>array(
                //'href'=>  '',
                'title'=>$file_name,
              ),
            ); 
            activity_feed(3, $_SESSION['admin']['id'], 'Admin', $file_id, 'eligibility_files','Admin Scheduled Eligibility File', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($description));
        }
    $response['status'] = 'success';
} else {
