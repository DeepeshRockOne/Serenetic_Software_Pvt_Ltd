...
$file_row = $pdo->selectOne("SELECT * FROM csv_agent_leads WHERE md5(id)=:id",$file_where);
if(!empty($file_row)) {
    if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete') {
        ...
        activity_feed(3,$_SESSION['admin']['id'],'Admin',$file_row['id'],'csv_agent_leads', 'Deleted CSV Leads Import Request','','', $desc);
        ...
        if($file_row['status'] == "Processed") {
            ...
            $desc['total_leads'] = "Total Leads : ".$file_row['total_leads'];
            $desc = json_encode($desc);
            activity_feed(3,$_SESSION['admin']['id'],'Admin',$file_row['id'],'csv_agent_leads', 'Cancelled CSV Leads Import','','', $desc);
