<?php 
  ...
  $specific_user_group = (isset($_POST['specific_user_group']) && $_POST['specific_user_group'] == 'on') ? 'Y' : 'N';
    ...
    $specific_admin = checkIsset($_POST['specific_admin'],'arr');
      ...
      if($user_group == 'Admins'){
        if($specific_user_group == 'N'){
          ...
          if(!empty($specific_admin)){
            ...
            $insert_param['lead_tags'] = NULL;
            $insert_param['specific_user_ids'] = implode(',', $specific_admin);
            $insert_param['product_ids'] = NULL;
            $insert_param['product_status'] = NULL;
          }
        }
            ...
            foreach ($messages as $key => $value) {
              if(in_array($key, $updMsgid)){
                ...
                if(!empty($activity)){
                    ...
                    $link = $ADMIN_HOST.'/add_sms_broadcast.php?broadcaster_id='. md5($updBroadcasterId);
                  
                    $actFeed=$functionsList->generalActivityFeed($tmp,$tmp2,$link,$broadcaster_res['display_id'],$key,'broadcaster_message','Admin Updated SMS Broadcaster','updated message on SMS Broadcaster');
                }
                ...
                $actFeed=$functionsList->generalActivityFeed($tmp,$tmp2,$link,$broadcaster_res['display_id'],$key,'broadcaster_message','Admin Updated SMS Broadcaster','added message on SMS Broadcaster');
      ...
      if(isset($updBroadcasterId) && !empty($updBroadcasterId) && $is_clone == 'N'){
        ...
        if(!empty($checkDiff)){
          ...
          $activityFeedDesc['ac_message']['ac_red_2']=array(
            'href'=>$ADMIN_HOST.'/add_sms_broadcast.php?broadcaster_id='.md5($updBroadcasterId),
            'title'=>$broadcaster_res['display_id']
          ); 
          
          activity_feed(3, $_SESSION['admin']['id'], 'Admin', $updBroadcasterId, 'broadcaster','Admin Updated SMS Broadcaster', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($activityFeedDesc),'',json_encode($extraJson));
        }
      } else {
        $description['ac_message'] =array(
          ...
          'ac_red_2'=>array(
              'href'=>$ADMIN_HOST.'/add_sms_broadcast.php?broadcaster_id='.md5($broadcaster_id),
              'title'=>$display_id,
          ),
        );

        activity_feed(3, $_SESSION['admin']['id'], 'Admin', $broadcaster_id, 'broadcaster','Admin Created SMS Broadcaster', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($description));
      }
    ...
    if(!empty($action_type) && $action_type == 'send_sms' && !empty($send_user_sms)){
      ...
      if(!empty($messages)){
        foreach ($messages as $key => $msg) {
          ...
          send_sms_to_phone($send_user_sms,$msg);
        }
      }
    }
