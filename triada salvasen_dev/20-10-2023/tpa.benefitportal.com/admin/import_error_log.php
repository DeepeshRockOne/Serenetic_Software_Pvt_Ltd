...
$file_row = $pdo->selectOne("SELECT * FROM import_requests WHERE md5(id)=:id AND is_deleted='N'", array(':id' => $id));
...
$options = array(
    ...
    'results_per_page' => 50,
    'url' => 'import_error_log.php?' . $query_string,
    'db_handle' => $pdo->dbh,
...
try {
    ...
    $sel_sql = "SELECT * FROM import_csv_log WHERE is_deleted='N' AND id>0 " . $incr . " ";
...
if (isset($_GET["export"]) && (trim($_GET["export"]) == "export_excel")) {
    if ($_GET['export'] == 'export_excel') {
        ...
        if (count($fails_report_rows) > 0) {
            ...
            redirect('import_summary.php', true);
        }

        if ($content) {
            $csv_filename = "import_error_log_" . date("Ymd", time()) . ".csv";
...
$template = 'import_error_log.inc.php';
