...
$breadcrumbes[1]['link'] = 'enrollment_fees.php';
$breadcrumbes[2]['title'] = 'Manage Application Fees';
$prdIncr = '';
$page_title = "Manage Application Fees";
...
if($fee_id > 0){
	$sqlFeeProduct="SELECT group_concat(id) as feeProduct FROM prd_main where find_in_set(:fee_id,enrollment_fee_ids) AND is_deleted='N'";
...
$sqlProduct="SELECT * FROM prd_main where id=:id and product_type='Enrollment Fee'";
...
if($fee_id > 0){
	if($resProduct){
		...
		$enrollment_fee_ids_products = array();

		$checkEnrollmentFee="SELECT group_concat(id) as productList FROM prd_main where is_deleted='N' AND FIND_IN_SET(:fee_id,enrollment_fee_ids)";
		$resEnrollmentFee=$pdo->selectOne($checkEnrollmentFee,array(":fee_id"=>$fee_id));

		$enrollment_fee_ids_products = explode(",", $resEnrollmentFee['productList']);
	}else{
		setNotifyError("Enrollment Fee Not Found");
		redirect("enrollment_fees.php");
	}
}


$productListSql="SELECT id,name,product_code,record_type 
				FROM prd_main where id!=:id AND (status='Active' $prdIncr) AND is_deleted='N' and type NOT IN('Kit','Fees','Association') 
				ORDER BY name ASC";
...
if($iframe==1){
	$template = "manage_enrollment_fees.inc.php";
