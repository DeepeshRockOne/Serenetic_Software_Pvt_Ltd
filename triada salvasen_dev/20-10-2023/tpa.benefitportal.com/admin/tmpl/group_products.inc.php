        ...
        <table class="<?=$table_class?> mn">
            ...
            <?php if($totalProduct > 0 && !empty($fetchProduct)) { ?>
                <?php foreach($fetchProduct as $product) { ?>
                    ...
                    <?php } ?>
                    <td><a href="javascript:void(0)" data-href="<?=$HOST?>/agents_pricing.php?agent_id=<?=$_GET['id']?>&product_id=<?=$product['pid']?>&from=admin" class="red-link agents_pricing"><strong>View</strong></a></td>
...
function group_product_ajax_submit() {
    ...
    function product_billing_type($billing_type,$id) {
        ...
        swal({
          text: "<strong>Change billing Product to "+$txt+" :</strong> Are you sure?",
