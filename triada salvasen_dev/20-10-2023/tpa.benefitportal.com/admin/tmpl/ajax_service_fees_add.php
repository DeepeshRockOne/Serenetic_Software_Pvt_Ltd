<?php 
	...
	//********** step1 varible intialization code start **********************
		...
		$is_fee_on_renewal  = $_POST['is_fee_on_renewal'];
		$fee_renewal_type  = $_POST['fee_renewal_type'];
		$fee_renewal_count  = $_POST['fee_renewal_count'];

		$is_fee_on_commissionable  = $_POST['is_fee_on_commissionable'];
		
		$price  = $_POST['price'];
		$non_commissionable_price  = $_POST['non_commissionable_price'];
		$commissionable_price  = $_POST['commissionable_price'];
	...
	if ($validate->isValid()) {
			...
			//****  service fees id code start *************
				$checkServiceFee="SELECT id,service_fee_ids FROM prd_main where is_deleted='N' AND FIND_IN_SET(:fee_id,service_fee_ids)";
				$resServiceFee=$pdo->select($checkServiceFee,array(":fee_id"=>$fee_id));

				if(!empty($resServiceFee)){
					foreach ($resServiceFee as $key => $product) {
						if(!in_array($product['id'],$product_array)){
							$service_fee_ids_list=explode(",", $product['service_fee_ids']);
							if (($key = array_search($fee_id, $service_fee_ids_list)) !== false) {
							    unset($service_fee_ids_list[$key]);
							}
							$service_fee_ids = implode(",",$service_fee_ids_list);
							$updParams=array("service_fee_ids"=>trim($service_fee_ids,","));
							$updWhere=array(
								'clause'=>'id=:id',
				...
				if(!empty($product_array)){
					foreach ($product_array as $key => $product) {
						$sqlProduct="SELECT id,service_fee_ids FROM prd_main where is_deleted='N' AND id=:id";
						$resProduct=$pdo->selectOne($sqlProduct,array(":id"=>$product));

						if($resProduct){
							$service_fee_ids_list=explode(",",$resProduct['service_fee_ids']);
							if(!in_array($fee_id,$service_fee_ids_list)){
								array_push($service_fee_ids_list, $fee_id);

								$service_fee_ids = implode(",", $service_fee_ids_list);
								$updParams=array("service_fee_ids"=>trim($service_fee_ids,","),'is_service_fee'=>'Y');
								$updWhere=array(
									'clause'=>'id=:id',
