<!-- $account_manager variable defined at agents_add_account_managers.inc.php file -->

<?php if(checkIsset($account_manager) !='account_manager') {?>
<form id="create_new_admin_level_frm"   method="POST">
<?php } ?>
  <input type="hidden" name="id" value="<?=checkIsset($agent_id)?>">
  <div class="add_level_panelwrap create_admin_popup">
    <div class="panel panel-default panel-block Font_Roboto ">
      <div class="panel-heading" <?=checkIsset($account_manager) == 'account_manager' ? 'style="display:none"' : '';?>>
        <h4 class="mn fw500"><img src="images/icons/add_level_icon.svg" height="20px" />&nbsp;&nbsp; Edit Level</h4>
      </div>
      <div class="<?=checkIsset($account_manager) != 'account_manager' ? 'panel-body' : '';?> theme-form">
        <div class="form-group has-value" <?=checkIsset($account_manager) == 'account_manager' ? 'style="display:none"' : '';?>>
          <input type="text" name="access_name" class="form-control has-value" readonly="readonly" value="<?=checkIsset($_GET['lvl_name'])?>" />
          <label>Name Level<sup class="text-red">*</sup></label>
          <p class="error" id="error_access_name"></p>
        </div>
        <p class="text-gray fs12" <?=checkIsset($account_manager) == 'account_manager' ? 'style="display:none"' : '';?>>Choose the modules this agent will have access to<sup class="text-red">*</sup></p>
        <table class="table  access_table text-center theme-form">
          <?php if(checkIsset($account_manager) == 'account_manager' ){ ?>
            <thead>
              <tr class="gray_head">
                <th colspan="2" class="fs16"><i class="fa fa-info-circle text-blue fs18" aria-hidden="true"></i> &nbsp;Feature Access</th>
              </tr>
              <tr>
        ...
        <table class="<?= $table_class ?>">
          ...
          <tbody>
            <?php if(!empty($displayDirectEnrollList)) { ?>
              <?php foreach ($displayDirectEnrollList as $key => $value) { ?>
                  <tr>
                      <td><?= $value ?></td>
                      <td>
                        <div class="custom-switch">
                          <label class="smart-switch">
                              <input type="checkbox" name="displayDirectEnroll[]" class="js-switch displayDirectEnroll" data-user-group="<?= $value ?>" id="displayDirectEnroll_<?= $value ?>" value="<?= $value ?>" <?= !empty($displayDirectEnrollArr) && in_array($value, $displayDirectEnrollArr) ? 'checked' : '' ?>/>
        ...
        <table class="<?= $table_class ?>">
          <thead>
              <tr>
                  <th>Agent’s Admin Performance Roles</th>
                  <th>Status</th>
              </tr>
          </thead>
          <tbody>
            <?php if(!empty($additionalAccessList)) { ?>
              <?php foreach ($additionalAccessList as $key => $value) { ?>
                  <tr>
                      <td><?= $value['descriptions'] ?></td>
                      <td>
                        <div class="custom-switch">
                          <label class="smart-switch">
                              <input type="checkbox" name="additionalAccess[]" class="js-switch additionalAccess" data-user-group="<?= $key ?>" id="additionalAccessArr_<?= $key ?>" value="<?= $key ?>" <?= !empty($additionalAccessArr) && in_array($key, $additionalAccessArr) ? 'checked' : '' ?>/>
                              <div class="smart-slider round"></div>
                          </label>
                        </div>
                      </td>
        ...
        <div class="form-group text-center m-t-30" <?=checkIsset($account_manager) == 'account_manager' ? 'style="display:none"' : '';?>> 
...
$(document).off("click", "#save_btn");
  $(document).on("click", "#save_btn", function(e) {
    ...
    $(".error").html('');
    $.ajax({
      url: 'ajax_agents_access_edit.php',
      dataType: 'JSON',
      ...
      success: function(res) {
        ...
        if (res.status == "success") {
            window.parent.location.href = "agent_detail_v1.php?id=<?=checkIsset($_GET['agent_id'])?>";
        } else {
          var is_error = true;
...
var acl_name = "<?=checkIsset($acl_name)?>";
  if (acl_name != '') {
    ...
    var acl_names = <?=json_encode(checkIsset($acl_names));?>;
    var acl_features = <?=json_encode(checkIsset($acl_features));?>;
