    ...
    <div id="health_details_section" class="thumbnail theme-form">
        ...
        <div class="row enrollment_auto_row">
            <?php 
                ...
                $gap_excluded_fields = array("in_patient_benefit","out_patient_benefit","monthly_income","monthly_benefit");
                ...
                if($is_gap_product == 'Y' && in_array($row['label'],$gap_excluded_fields) && empty($control_value)){
                    //remove fields if product is gap plus as per CRM-174
                    continue;
                }
                ?>
                    ...
                    <?php } else if ($control_type == 'select_multiple') { ?>
                        ...
                        <?php } else if ($control_type == 'textarea') { ?>                                                                                                                                      
                            <div class="col-sm-12 form-inline">
                                    ...
                                    <textarea name="<?= $custom_name ?>[<?= $prd_question_id ?>]" rows="3" class="form-control" maxlength="300"  readonly="" disabled=""><?= $control_value ?></textarea>      
                                    <p class="error" id="error_<?= $control_name ?>"></p>    
                            </div>  
                    <?php } ?>
                <?php } ?>
            ...
            <?php if(!empty($prd_benefit_amount['Primary'])) { ?>
                ...
                foreach ($prd_benefit_amount['Primary'] as $product_id => $benefit_amount_res) {
                 ...
                 foreach($benefit_amount_res as $benefitKey => $benefitValue){ ?>
                    <?php 
                        if($is_gap_product == 'Y' && in_array($benefitKey,$gap_excluded_fields)){
                            continue;
                        }
                    ?>
                    <div class="col-md-12 col-sm-12"><p class="font-bold m-b-15"><?=ucwords(str_replace('_',' ',$benefitKey))?></p></div>
                    <?php if($benefitKey == 'monthly_benefit'){ ?>
                        <div class="col-md-4 col-sm-6">
                            <div class="form-group">
                                <input type="text" name="primary_<?=$benefitKey?>[<?= $product_id ?>]" id="primary_<?=$benefitKey?>_<?= $product_id ?>" class="form-control <?=!empty($primary_benefit_amount[$benefitKey][$product_id])?"has-value":""?>" value="<?=displayAmount($primary_benefit_amount[$benefitKey][$product_id])?>" disabled>
