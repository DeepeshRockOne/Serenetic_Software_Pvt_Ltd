<div class="white-box">
    <div class="clearfix tbl_filter">
        <div class="pull-left">
            <h4 class="">Weekly Commissions</h4>
        </div>
       <?php if ($total_rows > 0) { ?>
        <div class="pull-right">
          <div class="form-inline" id="DataTables_Table_0_length top_paginate_cont">
            ...
            <div class="form-group mn height_auto">
              <select size="1" id="pages" name="weekly_pages" class="form-control select2 placeholder" onchange="$('#per_pages').val(this.value);$('#nav_page').val(1);weeklyCommissionStatement();">
            ...
            <tbody>
                <?php if ($total_rows > 0) {
                    ...
                    foreach ($fetch_rows as $rows) { 
                        ...
                        ?>
                        <tr>
                            <td class="text-nowrap"><a href="weekly_commissions_status.php?pay_period=<?=$rows['payPeriod']?><?php if(!empty($agent_id)){ ?>&agent_id=<?=$agent_id?><?php } ?>" class="fw500 text-action"><?= $startPayPeriod.' - '.$endPayPeriod; ?></a></td>
...
<script type="text/javascript">
    $(document).off('click', '#weeklyCommissionDiv ul.pagination li a');
    $(document).on('click', '#weeklyCommissionDiv ul.pagination li a', function (e) {
        e.preventDefault();
        $('#ajax_loader').show();
