<!-- $account_manager variable defined at groups_add_account_managers.inc.php file -->

<?php if(checkIsset($account_manager) !='account_manager') {?>
<form id="create_new_admin_level_frm"   method="POST">
<?php } ?>
  <input type="hidden" name="id" value="<?=checkIsset($group_id)?>">
  <div class="add_level_panelwrap create_admin_popup">
    ...
    <!-- Group Communication method update code start -->
      <p class="fw500 m-b-20">How would group like to handle automated communications (email/sms) from system?</p>
      <div class="row">
        <div class="col-xs-4">
          <div class="form-group height_auto">
            <label>Group Adminstration</label>
    ...
    <!-- Group Communication method update code ends -->
        <p class="text-gray fs12" <?=checkIsset($account_manager) == 'account_manager' ? 'style="display:none"' : '';?>>Choose the modules this group will have access to<sup class="text-red">*</sup></p>
              ...
              <td>
                <a role="button" data-toggle="collapse" data-parent="#accordion" href=".c<?=$feature["id"]?>" aria-expanded="true" aria-controls="c<?=$feature['id']?>">
                  <span class="link">
                    ...
                    <i class="<?= !empty($feature['child']) ? 'fa fa-sort ' : '' ?> text-action pull-right"></i>
...
$(document).off("click", "#save_btn");
  $(document).on("click", "#save_btn", function(e) {
    ...
    $(".error").html('');
    $.ajax({
      url: 'ajax_group_access_edit.php',
      dataType: 'JSON',
      ...
      success: function(res) {
        ...
        if (res.status == "success") {
            window.parent.location.href = "groups_details.php?id=<?=checkIsset($_GET['group_id'])?>";
        } else {
          var is_error = true;
