    ...
    <div class="custom_notes_area" style="display : <?= empty($note_id) ? 'block' : 'none' ?>">
      <form name="frm_note_add_main" id="frm_note_add_main" method="POST"
        ...
        <div class="form-group m-b-0">
          <textarea name="note_text" class="form-control resizable" rows="17" placeholder="Type something..." id="add_note_text"></textarea>
        ...
        <div class="row m-t-30">
          <div class="col-xs-6"><a href="javascript:void(0)" id="post_note" class="btn btn-action">
            <?=($reply_id != '') ? 'Reply' : 'Post' ?>
            </a> </div>
          <div class="col-xs-6">
          ...
          <div id="fileList" class="file_name_div" style="display:none;">
            <div class="input-group" >
                <input type="text" name="note_file_display" id="note_file_display" placeholder="File Name" class="form-control " value="<?= $note_res['file_name'] ?>">
      ...
      <form name="frm_note_edit_main" id="frm_note_edit_main" method="POST" action="ajax_account_note_operation.php?id=<?= $customer_id; ?>&operation=edit_note"
        ...
        <div class="panel-body">
          ...
          <div class="form-group">
            <textarea name="note_text" cols="3" class="form-control resizable" rows="3" placeholder="Notes" <?php echo $show =='show' ? 'readonly="readonly"' : ''?> id="note_edit_text"><?= $note_res['description'] ?></textarea>
            <p class="error" id="error_description"></p>
          ...
          <div class="file_name_div form-group" style="display: <?php echo ($note_res['file_name'] != '') ? 'block' : 'none'; ?>">
            <div class="input-group">
                    ...
                    <span class="input-group-addon"><a href="<?php echo $show!='show' ? 'javascript:void(0)' : $ADMIN_HOST.'/download_note_file.php?file_name='.$note_res['file_name'];?>" class="<?php if($show!='show') { ?>delete_file <?php } ?>"><i class="fa <?php echo $show!='show' ? 'fa-close' : 'fa-download'?>"></i></a>
                    <input type="hidden" name="delete_file_value" value="N" class="delete_file_value">
                    </span>
              </div>
          </div>
          ...
          <div class="row">
            <div class="col-xs-6">
              <?php if($show!='show') { ?>
              <a href="javascript:void(0)" id="edit_post_note" class="btn btn-action ">Save</a> 
    ...
    $(document).ready(function () {
            ...
            success: function (res) {
                ...
                if (res.status == 'success') {
                    ...
                    if(res.from_page == 'agent_notes') {
                        ...
                        // if(!(res.from_page == 'all_popup')){
                        //   window.opener.location.href = '<?=$ADMIN_HOST?>/'+ url +'?id=<?=$customer_id?>';
                        //   window.close();
                        // }
        ...
        $('#frm_note_edit_main').ajaxForm({
            ...
            success: function (res) {
                $("#ajax_loader").hide();
                if (res.status == 'success') {
                    parent.setNotifySuccess("Note updated successfully.");
                    if(res.from_page == 'agent_notes') {
                        window.location.href = 'agent_notes.php?id=<?=$customer_id?>&user_type=' + res.user_type;
                    } else {
                        if(res.user_type == 'Admin'){
                            ...
                            url = "lead_details.php";
                        }else if(res.user_type == 'Participants'){
                            url = "participants_details.php";
                        }
                    }
                    if(!(res.from_page == 'all_popup')){
                        parent.$.colorbox.close();
                        // parent.location = '<?=$ADMIN_HOST?>/'+ url +'?id=<?=$customer_id?>';
                        parent.interactionUpdate('<?=$customer_id?>','notes',url);
                        // window.opener.location.href = '<?=$ADMIN_HOST?>/'+ url +'?id=<?=$customer_id?>';
                        // window.close();
                    }
                } else if (res.status == 'fail') {
...
function refreshParent(url) 
{
    ...
    window.opener.interactionUpdate('<?=$customer_id?>','notes',url);
}
</script>