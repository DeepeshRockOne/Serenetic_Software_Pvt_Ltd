        ...
        <?php if ($total_rows > 0) {
          ...
          <tr>
            ...
            <td class="text-center">
              <a href="effective_orders_payable.php?id=<?=md5($rows["id"])?>" class="viewEffectedOrders text-action fw500" data-id="<?=$rows["id"]?>">
                <?=$rows["orderCounts"]?>
              </a>
            </td>
            <td><a href="javascript:void(0)" class="text-action fw500"><?=$rows["adminDispId"]?></a><br><?=$rows["adminName"]?></td>
            <td><?=$rows['debit'] != 0 ? displayAmount(abs($rows['debit'])) : '-';?></td>
            <td><strong><?=$rows['credit'] != 0 ? '('.displayAmount(abs($rows['credit'])) .')' : '-';?></strong></td>
            <td><?=$rows["status"]?></td>
            <td class="icons text-right">
              ...
              <?php } ?>
              <a href="effective_orders_payable.php?id=<?=md5($rows["id"])?>" class="viewEffectedOrders"><i class="fa fa-eye"></i></a>
              <?php if($rows["status"] == "Completed"){ ?>
                <a href="javascript:void(0)" class="exportCSV" data-regeneratedPayableId="<?=md5($rows['id'])?>"><i class="fa fa-download"></i></a>
              <?php } ?>
            </td>
          </tr>
        <?php }
...
<?php }else{ ?>
  ...
  <form id="regenerateFrm" action="regenerate_payable.php" method="GET">
    ...
    <div class="panel panel-default panel-block">
      <div class="panel-body">
        <div class="clearfix m-b-15 tbl_filter">
          <div class="pull-left">
            <h4 class="m-t-7">Regenerated Payables</h4>
          </div>
          <div class="pull-right ">
              ...
              <a href="add_regenerate_payable.php" class="btn btn-action m-l-5">+ Regenerate Payable</a>
          </div>
        </div>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $(document).on('click', '.exportCSV', function(e) {
      var regeneratedPayableId = $(this).attr("data-regeneratedPayableId");
      
      parent.confirm_export_data(function() {
        $('#ajax_loader').show();
        $.ajax({
            url: "regenerate_payables_export_csv.php",
            type: 'GET',
            data: {"regeneratedPayableId":regeneratedPayableId},
            dataType: 'json',
            success: function(res) {
                $('#ajax_loader').hide();
                if(res.status == "success") {
