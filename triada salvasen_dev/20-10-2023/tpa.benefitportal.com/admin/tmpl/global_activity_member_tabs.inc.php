...
<div class="panel panel-default panel-block panel-title-block global_activity">
   <form id="frm_search_members" action="global_activity_member_tabs.php" method="GET" >
         ...
         <div class="panel-wrapper collapse in">
            <div class="panel-body theme-form">
               ...
               <div class="panel-footer">
                  <button type="button" class="btn btn-info" name="search" id="btn_ajax_submit_members"> <i class="fa fa-search"></i> Search </button>
                  <?php
                  ?>
                  <input type="hidden" name="search_type" id="search_type" value="" />
                  <input type="hidden" name="is_ajaxed_members" id="is_ajaxed_members" value="1" />
...
<script type="text/javascript">  
    $(document).ready(function() {
      initSelectizeDropDown('activity_by','globalActivityMember',<?php echo $_BOOTSTRAP_TAGS_MIN_LENGTH; ?>);
      initSelectizeDropDown('impacted_name','globalActivityMember',<?php echo $_BOOTSTRAP_TAGS_MIN_LENGTH; ?>);
      initSelectizeDropDown('ip_address','IPaddressMember',<?php echo $_BOOTSTRAP_TAGS_MIN_LENGTH; ?>);
      //ajax_submit_members();
      $(document).off("click","#btn_ajax_submit_members");
     $(document).on("click","#btn_ajax_submit_members",function(){
  ...
  function ajax_submit_members() {
      $('#ajax_loader').show();
      $('#ajax_data').hide();
      $('#is_ajaxed_members').val('1');

      var params = $('#frm_search_members').serialize();
      var all_usersFrm = $('#all_usersFrm').serialize();
      params += '&'+all_usersFrm;
      $.ajax({
          url: $('#frm_search_members').attr('action'),
          type: 'GET',
          data: params,
