   ...
   <table class="<?=$table_class?>">
      ...
      <?php if(!empty($billing_info) && count($billing_info) > 0) {
            ...
            <tr>
               ...
               <td class="icons">
                  <a href="<?=$HOST?>/add_billing_profile.php?location=admin&id=<?=$customer_id?>&action=Edit&bill_id=<?=$info['id']?>" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="Edit" class="edit_billing_profile"><i class="fa fa-edit" aria-hidden="true"></i></a>
                  <?php if($info['is_default'] !='Y' && count($billing_info) > 1) { 
                     //OP29-738 Remove ability for Admin to do delete billing method if no other exists
                  ?>
...
<div class="text-right">
   <a href="<?=$HOST?>/add_billing_profile.php?location=admin&id=<?=$customer_id?>&action=Add" class="add_billing_profile btn btn-action">+ Billing Method</a>
</div>
<hr>
   ...
   $(document).on('click','.delete_billing_profile',function(e){
      var $bill_id = $(this).attr('data-bill_id');
         swal({
            ...
            $.ajax({
               url:'<?=$HOST?>/add_billing_profile.php?location=admin',
               method : 'POST',
               data : {
                  id:'<?=$customer_id?>',
                  bill_id:$bill_id,
