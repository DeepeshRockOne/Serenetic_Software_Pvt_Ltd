<div class="panel panel-default  panel-space">
  <form  name="agentGlobalRule" id="agentGlobalRule" method="post">
  <div class="panel-heading">
    <div class="panel-title">
      <p class="fs16 mn"><strong class="fw500">Advance Commission -</strong> <span class="fw300">Charged to Agents</span></p>
    </div>
  </div>
  <div class="panel-body theme-form">
    ...
    <div class="step_btn_wrap m-t-30 text-right"> 
        <input type="submit" id="addAgentGlobalFee" class="btn btn-action" value="Save">
        ...
        <input type="hidden" name="chargedTo" id="chargedTo" value="Agents">
...
$(document).ready(function(){
  ...
  $('#agentGlobalRule').on('submit', function(e) {
    e.preventDefault();
    $("#ajax_loader").show();
    $.ajax({
      url: '<?= $ADMIN_HOST ?>/ajax_manage_advance.php',
      type: 'POST',
      data: $('#agentGlobalRule').serialize(),
      dataType: 'json',
  ...
  $(document).on("click", ".delAdvanceFee", function(e) {
    ...
    swal({
      ...
      $("#ajax_loader").show();
      $.ajax({
        url: "add_agent_advance_global_rule.php",
        dataType: 'JSON',
        type: 'GET',
        data: {
          feeId: $feeId,
