            ...
            <tr>
              <td>
                <?php if($rows['status'] != 'Processing') { ?>
                  <a href="javascript:void(0)" class="fw500 text-red" onclick="parent.redirect_page('add_sms_broadcast.php?broadcaster_id=<?= md5($rows['id']); ?>')"><?=$rows['display_id']; ?></a>
                <?php }else{ ?>
              ...
              <td class="icons">
                ...
                <?php } if($rows['status'] == 'Processing') { ?>
                  <a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" title="Cancel" onclick="parent.change_status_sms_broadcaster('<?= md5($rows['id']); ?>', 'Cancelled')"><i class="fa fa-close" aria-hidden="true"></i></a>
                <?php } ?>
              </td>
            </tr>
          <?php } ?>
...
<?php } else { ?>
  ...
  <div class="panel panel-default panel-block panel-title-block" style="display: none;">
    <form id="frm_search" action="sms_broadcaster_detail_page.php" method="GET" class="sform">
      <div class="panel-wrapper collapse in">
        <div class="panel-footer clearfix">
          <button type="submit" class="btn btn-info" name="search" id="search" > <i class="fa fa-search"></i> Search </button>
          <button type="button" class="btn btn-info" name="viewall" id="viewall" onClick="window.location = 'sms_broadcaster_detail_page.php'"> <i class="fa fa-search-plus"></i> View All </button>
