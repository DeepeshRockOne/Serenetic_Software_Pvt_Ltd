      ...
      <tbody>
        <?php if ($total_rows > 0) {
          foreach ($fetch_rows as $rows) { ?>
            <tr>
              <td><a href="javascript:void(0)" class="fw500 text-red" onclick="parent.redirect_page('add_sms_broadcast.php?broadcaster_id=<?= md5($rows['id']); ?>')"><?php echo $rows['display_id']; ?></a><br /><?php echo date('m/d/Y', strtotime($rows['created_at'])); ?></td>
              <td><?=$rows['admin_name'] . ' ('.$rows['admin_display_id'] . ')' ;?></td>
              <td><?=$rows['brodcast_name']; ?></td>
              <td class="icons text-center">
                <a href="javascript:void(0);" class="open_detail_page" data-id="<?=md5($rows['id'])?>" data-toggle="tooltip" data-placement="top" title="Message"><i class="fa fa-eye" aria-hidden="true"></i></a>
              </td>
              <td class="text-center"><?=$rows['total_users'] . '/' . $rows['total_sent']?></td>
              <td class="text-center" width="10%"><?php echo $rows['status']; ?> </td>
              <td class="icons">
                <a href="javascript:void(0)" data-toggle="tooltip" data-placement="top" title="Duplicate" onclick="parent.redirect_page('add_sms_broadcast.php?broadcaster_id=<?= md5($rows['id']); ?>&is_clone=Y')"><i class="fa fa-clone" aria-hidden="true" ></i></a>
                <!-- <a href="javascript:void(0)" data-toggle="tooltip" data-placement="top" title="Edit" onclick="parent.redirect_page('add_sms_broadcast.php?broadcaster_id=<?= md5($rows['id']); ?>')"><i class="fa fa-pencil-square-o" aria-hidden="true" ></i></a> -->
                <a href="javascript:void(0);" data-toggle="tooltip" data-placement="top" title="Delete" onclick="parent.delete_sms_broadcaster('<?= md5($rows['id']); ?>')"><i class="fa fa-trash" aria-hidden="true"></i></a>
              </td>
            </tr>
          <?php } ?>
        <?php } else {?>
...
<?php } else { ?>
  ...
  <div class="panel panel-default panel-block panel-title-block">
    <form id="frm_search" action="sms_broadcaster_history_page.php" method="GET" class="sform">
      <div class="clearfix"></div>
      <div class="clearfix tbl_filter m-t-5">
        <div class="pull-left">
          <h4 class="m-t-7">History</h4>
        </div>
      ...
      <div class="panel-wrapper collapse in" style="display: none;">
        <div class="panel-footer clearfix">
          ...
          <button type="button" class="btn btn-info" name="viewall" id="viewall" onClick="window.location = 'sms_broadcaster_history_page.php'"> <i class="fa fa-search-plus"></i> View All </button>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      dropdown_pagination('ajax_histroy_data')
      ajax_history_submit();
    ...
    $(document).off('click','.open_detail_page');
    $(document).on('click','.open_detail_page', function(e){
      parent.parent_window_colorbox({href : 'broadcaster_sms_preview.php?id='+$(this).attr('data-id'), iframe : true, width:'900px', height:'377px'});
    });
