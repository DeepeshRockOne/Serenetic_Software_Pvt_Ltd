<div class="clearfix order_tab_info">
   ...
   <div class="pull-right text-right">
      ...
      <a href="<?=$HOST?>/make_payment.php?location=admin&id=<?=$customer_id?>" class="btn btn-success make_payment">Make Payment</a>
   </div>
</div>
<!-- Tab panes -->
...
$(document).on('change', '.order_action', function(e) {
    ...
    if (this_val == 'Receipt') {
      ...
      Postdate(id);
       ...
       $href= "add_payment_reversal.php?orderId="+id;
...
function Reprocess(id) {
    $.colorbox({
      href: '<?=$HOST?>/reprocess_order.php?location_from=memberDetail&customer_id=<?=$customer_id?>&orderId='+id,
      iframe: true,
      width: '1020px',
      height: '600px',
      fastIframe: false,
...
function Regenerate(id) {
    $.colorbox({
      href: 'regenerate_order.php?location_from=memberDetail&customer_id=<?=$customer_id?>&orderId='+id,
      iframe: true,
      width: '1020px',
      height: '600px',
      fastIframe: false,
...
function Postdate(id) {
    $.colorbox({
      href: '<?=$HOST?>/edit_order_post_date.php?location=admin&orderId='+id,
      iframe: true,
      width: '400px',
      height: '420px',
      fastIframe: false,
