      ...
      <tbody>
        <?php if ($total_rows > 0) {
          foreach ($fetch_rows as $rows) { ?>
            <tr>
              <td><a href="javascript:void(0)" class="fw500 text-red" onclick="parent.redirect_page('add_email_broadcast.php?broadcaster_id=<?= md5($rows['id']); ?>')"><?php echo $rows['display_id']; ?></a><br /><?php echo date('m/d/Y', strtotime($rows['created_at'])); ?></td>
              ...
              <td class="icons">
                ...
                <a href="javascript:void(0)" data-toggle="tooltip" data-placement="top" title="Duplicate" onclick="parent.redirect_page('add_email_broadcast.php?broadcaster_id=<?= md5($rows['id']); ?>&is_clone=Y')"><i class="fa fa-clone" aria-hidden="true" ></i></a>
...
<?php } else { ?>
  ...
  <div class="panel panel-default panel-block panel-title-block m-t-5">
    <form id="frm_search" action="email_broadcaster_history_page.php" method="GET" class="sform">
      <div class="clearfix"></div>
      <div class="clearfix tbl_filter">
        <h4 class="pull-left m-t-7">History</h4>
      ...
      <div class="panel-wrapper collapse in" style="display: none;">
        <div class="panel-footer clearfix">
          ...
          <button type="button" class="btn btn-info" name="viewall" id="viewall" onClick="window.location = 'email_broadcaster_history_page.php'"> <i class="fa fa-search-plus"></i> View All </button>
  ...
  <script type="text/javascript">
    ...
    dropdown_pagination(execute,'ajax_histroy_data');
    ...
    $(document).on('click','.open_detail_page', function(e){
      parent.parent_window_colorbox({href : 'emailer_content.php?id='+$(this).attr('data-id'), iframe : true, width:'900px', height:'377px'});
    });
