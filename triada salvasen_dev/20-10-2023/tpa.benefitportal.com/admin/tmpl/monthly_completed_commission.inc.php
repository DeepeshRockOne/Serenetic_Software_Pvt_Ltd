  ...
  <div class="table-responsive">
    <table class="<?=$table_class?>">
        ...
        <?php if ($total_rows > 0) {
          foreach ($fetch_rows as $rows) { ?>
            <tr>
              ...
              <td><a href="agent_monthly_com_popup.php?pay_period=<?=$pay_period?>&agent_id=<?=md5($rows['customer_id'])?>&status=<?=$rows['status']?>" class="fw500 text-action agent_monthly_com_popup"><?=$rows["agentRepId"]?></a><br><?php if(!empty($agent_id)){ ?><?=$rows["age_name"]?><?php }else{ ?><?=$rows["agentName"]?><?php } ?></td>
              ...
              <td class="icons">
                <a href="agent_monthly_com_popup.php?pay_period=<?=$pay_period?>&agent_id=<?=md5($rows['customer_id'])?>&status=<?=$rows['status']?>" data-toggle="tooltip" title="View Statement" data-placement="top" class="agent_monthly_com_popup"><i class="fa fa-eye" aria-hidden="true"></i></a>
                <?php if($module_access_type == "rw") { ?>
                <a href="javascript:void(0);" class="exportAgentCSV" data-status="<?=$rows['status']?>" data-agent_id="<?=$rows['customer_id']?>" data-toggle="tooltip" title="Export Excel" data-placement="top"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>
                <a href="ajax_commission_payperiod_operations.php?agentIds=<?=$rows['customer_id']?>&pay_period=<?=$rows['pay_period']?>&status=<?=$rows['status']?>&action=revCompletedComm&commission_duration=monthly" class="revCompletedComm" data-toggle="tooltip" title="Reverse Completed" data-placement="top"><i class="fa fa-angle-double-left" aria-hidden="true"></i></a>
                <?php } ?>
      ...
      <?php if ($total_rows > 0) {?>
        <tfoot>
          <tr>
            <td colspan="11"><?php echo $paginate->links_html; ?></td>
          </tr>
        </tfoot>
      <?php }?>
    </table>
    ...
    $(document).ready(function() {
      ...
      $('.paidCommPopover').popover({
        ...
        content: function() {
          ...
          getPopoverData($agentId,$payPeriod,'monthly');
...
<?php } else { ?>
  <?php include_once 'notify.inc.php';?>
   ...
   <form id="completedCommFrm" action="monthly_completed_commission.php" method="GET" class="sform">
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      $(document).off("click", ".agent_monthly_com_popup");
        ...
        parent.confirm_export_data(function() {
          $('#ajax_loader').show();
            $.ajax({
                ...
                type: 'GET',
                data: {"commission_duration":"monthly","agentIds":agentIds,"pay_period":pay_period,"status":"CompleteDecline"},
                dataType: 'json',
                success: function(res) {
                    $('#ajax_loader').hide();
                    if(res.status == "success") {
      ...
      $(document).on('click', '.exportAgentCSV', function(e) {
        ...
        parent.confirm_export_data(function() {
            ...
            $.ajax({
                url: "commission_export_csv.php",
                type: 'GET',
                data: {"commission_duration":"monthly","agentIds":agentIds,"pay_period":pay_period,"status":status},
                dataType: 'json',
                success: function(res) {
                    $('#ajax_loader').hide();
                    if(res.status == "success") {
