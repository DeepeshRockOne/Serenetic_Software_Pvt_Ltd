<div class="add_level_panelwrap add_fee_wrap">
  <div class="panel panel-default panel-block">
    ...
    <form name="carrier_fee_frm" method="Post" id="carrier_fee_frm" class="theme-form ">
...
$(document).on("change", ".fee_method", function() {
  $val = $(this).val();
  if ($val == "Percentage") {
...
$(document).on("click", "#add_fee", function() {
  $("#ajax_loader").show();
  $.ajax({
    url: 'ajax_add_carrier_fee.php',
    dataType: 'JSON',
    data: $("#carrier_fee_frm").serialize(),
    type: 'POST',
    success: function(res) {
      $("#ajax_loader").hide();
      $('.error').html('');
      if (res.status == "success") {
        window.parent.$("#carrier_fee_id").val(res.carrier_fee_id);
        window.parent.load_carrier_fee_div();
