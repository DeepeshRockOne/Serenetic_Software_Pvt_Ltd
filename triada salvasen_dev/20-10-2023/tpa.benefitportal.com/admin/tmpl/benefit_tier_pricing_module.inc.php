            ...
            <?php if ($total_rows > 0) {
                foreach ($fetch_rows as $mainKey => $rows) {?>
                    ...
                    <tr>
                        ...
                        <td><?= !empty($rows['matrixPlanType']) ? $prdPlanTypeArray[$rows['matrixPlanType']]['title'] . $rows['product_detail']: '-' ?></td>
                        <?php if(!empty($prdPricingQuestionRes)) { ?>
                            <?php foreach ($prdPricingQuestionRes as $key => $value) { ?>
                                <td class="<?= $value['id'] ?>PriceRow" style="<?= !empty($price_control) && in_array($value['id'], $price_control) ? '' : 'display: none' ?>">
                                    <?= $value['control_class']=='text_amount' ? '$' : '' ?><?= !empty($rows[$value['id']]['matrix_value']) ? $rows[$value['id']]['matrix_value'] : '-' ?></td>
                            <?php } ?>
                        <?php } ?>
                    </tr>
                <?php }?>
