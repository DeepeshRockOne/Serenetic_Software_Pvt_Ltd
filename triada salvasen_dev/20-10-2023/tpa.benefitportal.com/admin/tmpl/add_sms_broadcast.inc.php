        ...
        <p class="fs20 m-b-20">+ Text Message (SMS) Broadcast</p>
            ...
            <div class="form-group">
              <select class="form-control" name="user_group" id="userGroupSel">
                <option></option>
                <option value="Admins" <?=($userGroup == 'Admins') ? "selected='selected'" : '' ?>>Admins</option>
                <option value="Agents" <?=($userGroup == 'Agents') ? "selected='selected'" : '' ?>>Agents</option>
                <option value="Employer Groups" <?=($userGroup == 'Employer Groups') ? "selected='selected'" : '' ?>>Groups</option>
    ...
    <div class="theme-form">
      ...
      <hr>
        ...
        <div class="text-right m-b-10">
          ...
          <?php } ?>
          <a href="sms_broadcaster.php" class="btn red-link">Cancel</a>
        </div>
...
<script type="text/javascript">
  ...
  ajaxCall = function(){
    $("#ajax_loader").show();
    $.ajax({
      url:"<?= $ADMIN_HOST ?>/ajax_add_sms_broadcast.php",
      ...
      success: function(res) {
        ...
        if (res.status == 'success') {
          ...
          if(res.action_type == 'send_sms'){
            ...
            setTimeout(function(){ 
              window.location.href = "sms_broadcaster.php";
            }, 1000);
          }
          $("#ajax_loader").hide();
