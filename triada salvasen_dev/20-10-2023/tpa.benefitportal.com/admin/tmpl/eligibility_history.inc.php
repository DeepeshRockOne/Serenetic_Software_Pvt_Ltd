<?php if ($is_ajaxed) { ?>
  ...
  <div class="panel-body">
    <h4 class="m-t-0">Eligibility History </h4>
      ...
      <table class="<?=$table_class?>">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center"><a href="eligibility_processed_file.php?id=<?=$rows['id']?>" class="fw500 text-action"><?=$rows['total_files']?></a></td>
            <td><?=strtotime($rows['last_processed']) > 0 ? date('m/d/Y h:i A',strtotime($rows['last_processed'])) . " EST" : "-" ?></td>
            <td class="icons text-center">
              <a href="eligibility_processed_file.php?id=<?=$rows['id']?>" data-toggle="tooltip" data-trigger="hover" title="Download"><i class="fa fa-list"></i></a>
            </td>
          </tr>
          <?php } ?>
          <?php }else{ ?>
...
<form id="frm_search" action="eligibility_history.php" method="GET" class="sform">
