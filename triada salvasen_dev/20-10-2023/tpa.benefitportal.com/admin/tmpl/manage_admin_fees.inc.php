<div class="white-box manage_fees_wrap"> 
   <div class="panel panel-default panel-block">
      <div class="panel-heading">
         <h4 class="fees_bar_name mn"><?= ($resProduct) ? 'Manage Admin Fees' :'New Admin Fees ' ?></h4>
      </div>
                                    ...
                                    <select name="product[]" id="product" multiple="multiple">
                                        <?php if(!empty($productListRes)){?>
                                          <?php foreach ($productListRes as $key => $product) { ?>
                                              ...
                                              <option value="<?= $product['id'] ?>" <?=((!empty($admin_fee_ids_products) && in_array($product['id'],$admin_fee_ids_products)) || $product_id == $product['id'])?'selected="selected"':''?>><?= $product['name'] .' ('. $product['product_code'].')' ?></option>  
         ...
         success: function (res) {
            ...
            if(res.status=="success"){
               if($iframe==1){
                  ...
                  window.location.href="admin_fees.php";
               }
