        ...
        <tr>
          ...
          <th width="7%">
            <div class="checkbox checkbox-custom mn">
              <input type="checkbox" id="payToAllAgentBox" class="js-switch" data-toggle="tooltip" data-container="body" title="Select All"/>
...
<?php } else { ?>
  <?php include_once 'notify.inc.php';?>
   ...
   <form id="pendingCommFrm" action="monthly_pending_commission.php" method="GET" class="sform">
    ...
    $(document).ready(function() {
      ...
      // Agent Commission Pay Code Start
        ...
        $(document).on('click', '#approveToPayBtn', function(e) {
          ...
          if(selAgent > 1){
               window.parent.$.colorbox({href: 'approve_to_pay_commissions.php?commission_duration=monthly&selType=allAgent&pay_period=' + pay_period + '&type=approved_to_pay&agentIds=' + agentIds, width: '750px', height: '500px', iframe: true});
          }else{
               window.parent.$.colorbox({href: 'approve_to_pay_commissions.php?commission_duration=monthly&selType=singleAgent&pay_period=' + pay_period + '&type=approved_to_pay&agentIds=' + agentIds, width: '750px', height: '500px', iframe: true});
          }
        });
        ...
        $(document).on('click', '#denyToPayBtn', function(e) {
          ...
          parent.swal({
            ...
            $.ajax({
              ...
              type: 'GET',
              dataType:'JSON',
              data: {action:'denyToPay',agentIds:agentIds,pay_period:pay_period,commission_duration:'monthly'},
              beforeSend: function () {
                $("#ajax_loader").show();
              },
        ...
        $(document).on('click', '#exportCsvBtn', function(e) {
          ...
          $('#ajax_loader').show();
            $.ajax({
                ...
                type: 'GET',
                data: {"commission_duration":"monthly","agentIds":agentIds,"pay_period":pay_period,"status":"Pending"},
                dataType: 'json',
                success: function(res) {
                    $('#ajax_loader').hide();
                    if(res.status == "success") {
