<div class="white-box manage_fees_wrap"> 
   <div class="panel panel-default panel-block">
      <div class="panel-heading">
         <h4 class="fees_bar_name mn"><?= ($resProduct) ? 'Manage Service Fees' :'New Service Fees ' ?></h4>
      </div>
      <div class="panel-body">
         <form  method="POST" id="manage_fees" enctype="multipart/form-data"  autocomplete="off">
            ...
            <div class="section_space">
               <h4 class="h4_title">Fee Settings</h4>
               <div class="row">
                  <div class="col-sm-6">
                  ...
                  <div class="form-is-this-fee clearfix">
                     <div class="col-sm-6 col-md-4">
                        ...
                        <div class="m-l-25" id="renewal_div" style="<?= $is_fee_on_renewal=="Y" ? '' :'display:none' ?>">
                           <div class="radio-v">
                              <label>
                              <input name="fee_renewal_type" <?= $fee_renewal_type=="Continuous" ? 'checked' : '' ?> type="radio" value="Continuous" > Continuous
                              </label>
                           </div>
                           <div class="radio-v">
                              <label>
                              <input name="fee_renewal_type" <?= $fee_renewal_type=="Set Renewals" ? 'checked' : '' ?> type="radio" value="Set Renewals"> Set Numbers of Renewals
                              </label>
                           </div>
                           <p class="error" id="error_fee_renewal_type"></p>
                           <div class="form-group mn" id="fee_renewal_type_div" style="<?= $fee_renewal_type=="Set Renewals" ? '' :'display:none' ?>">
                              <select class="form-control max-w50 m-l-25" name="fee_renewal_count">
                                 <?php for($i=0;$i<=12;$i++) { ?>
                                    <option value="<?= $i ?>" <?= $i == $fee_renewal_count ? 'selected=selected' : '' ?>><?= $i ?></option>
                                 <?php } ?>
                              </select>
               ...
               <div class="row">
                  ...
                  <div class="">
                     <table class="<?=$table_class;?> table-responsive">
                        <thead>
                           <tr>
                                    ...
                                    <select name="product[]" id="product" multiple="multiple">  
                                        <?php if(!empty($productListRes)){?>
                                          <?php foreach ($productListRes as $key => $product) { ?>
                                              <option value="<?= $product['id'] ?>" <?=((!empty($service_fee_ids_products) && in_array($product['id'],$service_fee_ids_products)) || $product_id == $product['id'])?'selected="selected"':''?>><?= $product['name'] .' ('. $product['product_code'].')' ?></option>  
                                          <?php } ?>
                                        <?php } ?>
                                    </select>
    ...
    success: function(res) {
      ...
      if (res.status == "success") {
        if ($iframe == 1) {
          ...
          window.location.href = "service_fees.php";
        }
