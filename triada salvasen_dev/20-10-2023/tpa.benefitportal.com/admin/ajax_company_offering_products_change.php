...
if (!empty($offering_company)) {
	foreach ($offering_company as $product_id => $company_id) {

		$checksqlProduct="SELECT id,company_id,product_code FROM prd_main WHERE is_deleted='N' AND id=:id";
		$checkresProduct=$pdo->selectOne($checksqlProduct,array(":id"=>$product_id));

			if(!empty($checkresProduct)){

			$updateParams = array("company_id" => $company_id);
			$updateWhere = array(
				"clause" => "id=:id",
				...
				if(!empty($checkDiff)){
					...
					if(!empty($checkDiff)){
						foreach ($checkDiff as $key1 => $value1) {
							if($key1=='company_id'){
								$oldVaArray[$key1]=getname('prd_company',$oldVaArray[$key1],'company_name','id');
								$NewVaArray[$key1]=getname('prd_company',$NewVaArray[$key1],'company_name','id');
							}
