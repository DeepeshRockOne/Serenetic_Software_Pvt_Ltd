...
$options = array(
  ...
  'url' => 'ajax_get_billing_requests.php?' . $query_string,
  'db_handle' => $pdo->dbh,
...
$sel_sql = "SELECT br.*,bf.file_name,bh.file_name as processed_file
        FROM billing_requests br
        JOIN billing_files bf on(bf.id = br.file_id)
        LEFT JOIN billing_history bh on(bh.req_id = br.id)
        WHERE br.is_deleted='N'" . $incr . " GROUP BY br.id ORDER BY $SortBy $currSortDirection";
			...
			<?php if($fetch_rows){
				foreach ($fetch_rows as $k => $rows) { ?>
					<tr>
						...
						<?php }else{ ?>
								<a href="billing_export_requests.php?is_download=Y&file_name=<?=urlencode($rows['processed_file_name']);?>" data-toggle="tooltip" data-placement="top" title="Download"><i class="fa fa-download"></i></a>
