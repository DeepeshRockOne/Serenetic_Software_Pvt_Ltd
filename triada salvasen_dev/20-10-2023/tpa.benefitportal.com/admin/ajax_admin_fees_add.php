<?php
	...
	if ($validate->isValid()) {
			...
			//****  admin fees id code start *************
				$checkAdminFee="SELECT id,admin_fee_ids FROM prd_main where is_deleted='N' AND FIND_IN_SET(:fee_id,admin_fee_ids)";
				$resAdminFee=$pdo->select($checkAdminFee,array(":fee_id"=>$fee_id));

				if(!empty($resAdminFee)){
					foreach ($resAdminFee as $key => $product) {
						if(!in_array($product['id'],$product_array)){
							$admin_fee_ids_list=explode(",", $product['admin_fee_ids']);
							if (($key = array_search($fee_id, $admin_fee_ids_list)) !== false) {
							    unset($admin_fee_ids_list[$key]);
							}
							$admin_fee_ids = implode(",",$admin_fee_ids_list);
							$updParams=array("admin_fee_ids"=>trim($admin_fee_ids,","));
							$updWhere=array(
								'clause'=>'id=:id',
				...
				if(!empty($product_array)){
					foreach ($product_array as $key => $product) {
						$sqlProduct="SELECT id,admin_fee_ids FROM prd_main where is_deleted='N' AND id=:id";
						$resProduct=$pdo->selectOne($sqlProduct,array(":id"=>$product));

						if($resProduct){
							$admin_fee_ids_list=explode(",",$resProduct['admin_fee_ids']);
							if(!in_array($fee_id,$admin_fee_ids_list)){
								array_push($admin_fee_ids_list, $fee_id);

								$admin_fee_ids = implode(",", $admin_fee_ids_list);
								$updParams=array("admin_fee_ids"=>trim($admin_fee_ids,","),'is_admin_fee'=>'Y');
								$updWhere=array(
									'clause'=>'id=:id',
