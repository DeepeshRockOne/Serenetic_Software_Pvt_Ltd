...
$is_ajaxed_groups = isset($_REQUEST['is_ajaxed_groups']) ? $_REQUEST['is_ajaxed_groups'] : '';
$is_all_activity_popup = isset($_REQUEST['is_all_activity_popup']) ? $_REQUEST['is_all_activity_popup'] : '';
$type = isset($_REQUEST['type']) ? $_REQUEST['type'] : '';

if($type=='Group' && $is_all_activity_popup=='Y')
{	
	...
	try {
		...
		JOIN customer c ON (c.type='Group' AND ((c.id=af.entity_id AND af.entity_type='Group') OR (c.id=af.user_id AND af.user_type='Group'))) 
...
if(!$is_ajaxed_groups)
{
	$sel_activity_type = "SELECT DISTINCT entity_action FROM activity_feed WHERE user_type='Group'";
	$activity_type_res = $rpdo->select($sel_activity_type);
}

include_once 'tmpl/global_activity_groups_tabs.inc.php';
?>