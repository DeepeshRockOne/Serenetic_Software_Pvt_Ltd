...
$is_ajaxed_members = isset($_REQUEST['is_ajaxed_members']) ? $_REQUEST['is_ajaxed_members'] : '';
...
if($type=='Member' && $is_all_activity_popup=='Y'){
 ...
 try {
	...
	$limit_incr = ' LIMIT '.$from_limit.','.$limit;
	 ...
	 $sel_sql = "SELECT af.* FROM activity_feed af 
		JOIN customer c ON (c.type='Customer' AND ((c.id=af.entity_id AND af.entity_type='customer') OR (c.id=af.user_id AND af.user_type='Customer'))) 
...
if(!$is_ajaxed_members){
	$sel_activity_type_user = $rpdo->select("SELECT DISTINCT entity_action FROM activity_feed WHERE user_type='Customer' and user_type!='Agent'");
	$sel_activity_type_ent = $rpdo->select("SELECT DISTINCT entity_action FROM activity_feed WHERE entity_type='Customer' and user_type!='Agent'");
	$activity_type_res = array_merge($sel_activity_type_user,$sel_activity_type_ent);
}

include_once 'tmpl/global_activity_member_tabs.inc.php';
?>