...
$agentSql = "SELECT id,CONCAT(fname,' ',lname) as name ,rep_id FROM customer WHERE md5(id) = :agent_id and is_deleted='N'";
...
// Read Commisssions Page activity code start
  $description['ac_message'] =array(
    ...
    'ac_message_1' =>' read commissions for ',
    'ac_red_2'=>array(
				'href'=>$ADMIN_HOST.'/agent_detail_v1.php?id='.md5($agentRow['id']),
				'title'=>"(".$agentRow['rep_id'].")",
			),
  ); 

  activity_feed(3, $_SESSION['admin']['id'], 'Admin',$agentRow['id'], 'Agent','Read Commission', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($description));
...
$statusIncr = "";
if(!empty($status)){
	$statusIncr .= " AND cs.status IN('".$status."')";
}
...
foreach ($display_commission as $ckey => $cvalue){
	...
	$pdf_sel_sql = "SELECT cs.id as commId,c.rep_id as mbrRepId,c.fname,c.lname,CONCAT(c.fname,' ',c.lname) as mbrName,
	                ...
	                WHEN cs.is_fee_comm = 'Y' THEN 'Fee'
	                ELSE 'Earned'
	                END AS revType
...
// Weekly Commission pay Period Summary Code Start
	$weeklyCommSql = "SELECT 
						...
						FROM commission cs
						WHERE cs.commission_duration='weekly' AND cs.is_deleted='N'
						AND cs.customer_id = :agentId 
						AND cs.pay_period=:pay_period $statusIncr  
...
$template = "agent_weekly_com_popup.inc.php";
