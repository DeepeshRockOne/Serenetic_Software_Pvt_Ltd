...
//$incr .= " AND status IN('Pending','Running','Processed')";
...
$sel_sql = "SELECT er.*,ad.display_id as requester_rep_id,CONCAT(ad.fname,' ',ad.lname) as requester_name,TIMESTAMPDIFF(SECOND,er.created_at,now()) as difference,TIMESTAMPDIFF(SECOND,er.reprocess_at,now()) as rep_difference
        FROM $REPORT_DB.export_requests er
		JOIN admin ad ON(ad.id = er.user_id AND er.user_type='Admin')
							...
							<td>
								...
								if($status == "Processed") {
									if($rows['user_type'] == "Admin") {
										$requester_detail_page = 'admin_profile.php?id='.md5($rows['user_id']);
									} else {
										$requester_detail_page = 'agent_detail_v1.php?id='.md5($rows['user_id']);
									}
									/*<a href="<?=$requester_detail_page?>" target="_blank" class="text-action"><?=$rows['requester_rep_id']?></a>*/
									?>
