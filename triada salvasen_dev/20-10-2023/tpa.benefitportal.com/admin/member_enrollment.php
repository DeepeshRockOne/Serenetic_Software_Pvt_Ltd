...
if($customer_id){
	$customer_sql = "SELECT c.id,c.sponsor_id,s.type as sponsor_type 
					FROM customer c 
...
$breadcrumbes[3]['title'] = 'Add Product';
...
$enrollmentLocation ="adminSide";
$display_default_billing = 'Y';
$display_add_enrollee = false;
...
if(isset($lead_quote_row) && !empty($lead_quote_row)){
	...
	if($group_billing_method=='individual'){
		$order_sql = "SELECT id,status,future_payment,post_date FROM orders WHERE id =:id";
		$order_res = $pdo->selectOne($order_sql,array(":id"=>$order_id));
	}else{
		$order_sql = "SELECT id,status,future_payment,post_date FROM group_orders WHERE id =:id";
		$order_res = $pdo->selectOne($order_sql,array(":id"=>$order_id));
	}
...
$def_bill_sql = "SELECT payment_mode,id,card_no,card_type,AES_DECRYPT(ach_routing_number,'" . $CREDIT_CARD_ENC_KEY . "')as ach_routing_number,last_cc_ach_no,is_default FROM customer_billing_profile WHERE is_direct_deposit_account='N' AND customer_id=:customer_id AND is_deleted='N'";
