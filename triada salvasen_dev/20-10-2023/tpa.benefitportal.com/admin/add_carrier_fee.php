...
$carrier_fee_id = checkIsset($_GET['carrier_fee_id']);
...
if (!empty($_GET['fee_id'])) {
    ...
    if(!empty($feeRow)){
        ...
        if($is_clone == 'Y'){
            $display_fee_id = get_carrier_fee_id();
            $fee_name = "";
        }else{
            //************* Activity Code Start *************
                ...
                activity_feed(3, $_SESSION['admin']['id'], 'Admin', $feeRow['id'], 'Carrier',"Admin Read Carrier Fee", $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($description));
            //************* Activity Code End *************
        }
...
if(!empty($carrier_id)){
    $productSql="SELECT p.id,p.name,p.product_code,p.type,c.title 
            ...
            where p.type!='Fees' AND p.parent_product_id=0 AND p.is_deleted='N' AND md5(p.carrier_id) = :carrier_id AND (pbv.errorJson IS NULL OR pbv.errorJson = '[]') ORDER BY name ASC";
    $productRes = $pdo->selectGroup($productSql,array(":carrier_id" => $carrier_id),'title');
}
...
$template = 'add_carrier_fee.inc.php';
