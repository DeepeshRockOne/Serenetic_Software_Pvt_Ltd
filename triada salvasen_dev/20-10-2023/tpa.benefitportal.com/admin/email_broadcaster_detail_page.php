...
$options = array(
	...
	'url' => 'email_broadcaster_detail_page.php?' . $query_string,
	'db_handle' => $pdo->dbh,
...
if ($is_ajaxed) {
	try {
		...
		$sel_sql = "SELECT b.id, b.brodcast_name, b.status, b.display_id, b.created_at, CONCAT(a.fname, ' ',a.lname) as admin_name, a.display_id as admin_display_id, a.id as admin_id, b.total_sent, GROUP_CONCAT(CONCAT(bss.schedule_date, ' ', bss.schedule_hour, ':00 ')) as schedule_details,bss.time_zone as schedule_time_zone
					...
					WHERE b.sender_type='Admin' AND b.type='email' AND b.status NOT IN ('Completed','Cancelled') AND b.is_deleted = 'N' " . $incr . "
	...
	include_once 'tmpl/email_broadcaster_detail_page.inc.php';
	exit;
}
...
$template = 'email_broadcaster_detail_page.inc.php';
