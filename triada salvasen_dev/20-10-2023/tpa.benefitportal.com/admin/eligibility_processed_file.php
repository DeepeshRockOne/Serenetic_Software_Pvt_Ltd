...
$breadcrumbes[2]['link'] = 'eligibility_processed_file.php';
...
$options = array(
	...
	'url' => 'eligibility_processed_file.php?' . $query_string,
	'db_handle' => $pdo->dbh,
...
if(!$is_ajaxed){
	$description['ac_message'] =array(
	  ...
	  'ac_message_1' =>' read processed eligibility files ',
	  'ac_red_2'=>array(
	    'title'=> $file_name,
	  ),
	); 
	activity_feed(3, $_SESSION['admin']['id'], 'Admin', 0, 'eligibility_files','Admin read processed eligibility files', $_SESSION['admin']['fname'],$_SESSION['admin']['lname'],json_encode($description));
}
...
if ($is_ajaxed) {
	try {
		...
		$sel_sql = "SELECT eh.*,ef.file_name,eh.file_name as processed_file,CONCAT(a.fname,' ',a.lname) as admin_name,a.display_id 
		FROM eligibility_files ef
		JOIN eligibility_history eh on eh.service_group_id = ef.id
	...
	include_once 'tmpl/eligibility_processed_file.inc.php';
	exit;
}
...
$template = 'eligibility_processed_file.inc.php';
