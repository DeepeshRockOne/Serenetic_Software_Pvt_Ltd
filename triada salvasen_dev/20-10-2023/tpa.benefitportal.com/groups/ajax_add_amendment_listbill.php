			...
			foreach ($categoryData as $category_id => $amount) {
				...
				if(empty($amendment[$tmpKey][$customer_id][$category_id])){
					...
					if(!empty($product_code)){
						...
						activity_feed(3, $_SESSION['groups']['id'],'Group',$amendment_id , 'list_bill_amendment', 'List Bill Amendment Created', $_SESSION['groups']['fname'], $_SESSION['groups']['lname'], json_encode($ac_description));
					}
				...
				if(!isset($category[$tmpKey][$customer_id][$category_id]) && !empty($amendment_id)){
					...
					if(!empty($resAmendment)){
						...
						if(!empty($product_code)){
							...
							activity_feed(3, $_SESSION['groups']['id'],'Group',$amendment_id , 'list_bill_amendment', 'List Bill Amendment Removed', $_SESSION['groups']['fname'], $_SESSION['groups']['lname'], json_encode($ac_description));
						}
