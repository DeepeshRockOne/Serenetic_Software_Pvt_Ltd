            ...
            foreach ($messages as $key => $value) {
              if(in_array($key, $updMsgid)){
                ...
                if(!empty($activity)){
                    ...
                    $link = $GROUP_HOST.'/add_sms_broadcast.php?broadcaster_id='. md5($updBroadcasterId);
                  
                    $actFeed=$functionsList->generalActivityFeed($tmp,$tmp2,$link,$broadcaster_res['display_id'],$key,'broadcaster_message','Group Updated SMS Broadcaster','updated message on SMS Broadcaster');
                }
                ...
                $actFeed=$functionsList->generalActivityFeed($tmp,$tmp2,$link,$broadcaster_res['display_id'],$key,'broadcaster_message','Group Updated SMS Broadcaster','added message on SMS Broadcaster');
      ...
      if(isset($updBroadcasterId) && !empty($updBroadcasterId) && $is_clone == 'N'){
        ...
        if(!empty($checkDiff)){
          $activityFeedDesc['ac_message'] =array(
            ...
            'ac_message_1' =>' updated message on SMS Broadcaster ',
            'ac_red_2' => array(
              'href'=>$GROUP_HOST.'/add_sms_broadcast.php?broadcaster_id='.md5($updBroadcasterId),
          ...
          activity_feed(3, $_SESSION['groups']['id'], 'Group', $updBroadcasterId, 'broadcaster','Group Updated SMS Broadcaster', $_SESSION['groups']['fname'],$_SESSION['groups']['lname'],json_encode($activityFeedDesc),'',json_encode($extraJson));
        }
      } else {
        $description['ac_message'] =array(
          ...
          'ac_red_2'=>array(
              'href'=>$GROUP_HOST.'/add_sms_broadcast.php?broadcaster_id='.md5($broadcaster_id),
              'title'=>$display_id,
          ),
        );

        activity_feed(3, $_SESSION['groups']['id'], 'Group', $broadcaster_id, 'broadcaster','Group Created SMS Broadcaster', $_SESSION['groups']['fname'],$_SESSION['groups']['lname'],json_encode($description));
      }
