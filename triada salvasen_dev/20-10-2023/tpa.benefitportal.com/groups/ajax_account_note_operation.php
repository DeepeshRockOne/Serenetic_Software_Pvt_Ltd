...
if (!empty($_REQUEST['operation']) && $_REQUEST['operation'] == 'edit_note') {
    ...
    if ($validate->isValid()) {
        ...
        //$ac_description['description'] = $_SESSION['groups']['rep_id'].' updated note on '. $name['fname'].$name['lname'].' ('.$name['rep_id'].')' ;
        ...
        activity_feed(3, $_SESSION['groups']['id'],'Group', $c_user_id,$user_type , 'Note Updated', $_SESSION['groups']['fname'], $_SESSION['groups']['lname'], json_encode($ac_description));
    } else {
        $res['status'] = 'fail';
...
if (!empty($_REQUEST['operation']) && $_REQUEST['operation'] == 'add_note') {
    ...
    if ($validate->isValid() && $agent_id != '') {
        ...
        /* Code for activity feed*/
        if($reply_id == ''){
            $extra['user_fname'] = $_SESSION['groups']['fname'];
            $extra['user_lname'] = $_SESSION['groups']['lname'];
            $extra['user_display_id'] = $_SESSION['groups']['rep_id'];
            $extra['user_photo'] = $_SESSION['groups']['rep_id'];
            $extra['user_type'] = 'Group';
            ...
            if($user_type == 'Lead') {
                ...
                // $ac_description['note_description'] = addslashes($description);
                $ac_description["ac_description_link"] = array(
                    'Description '=>array('href'=>'#javascript:void(0)','class'=>'descriptionPopup red-link','title'=>'Description','data-desc'=>checkIsset($description),'data-encode'=>'no')
                );
            }
            
            activity_feed(3,$user_row['id'],$user_type,$note_id, 'note', 'Note Created', $_SESSION['groups']['fname'], $_SESSION['groups']['lname'], json_encode($ac_description),'',json_encode($extra));

            activity_feed(3,$_SESSION['groups']['id'],'Group',$_SESSION['groups']['id'],'customer', 'Note Created', $_SESSION['groups']['fname'], $_SESSION['groups']['lname'], json_encode($ac_description));
        }
