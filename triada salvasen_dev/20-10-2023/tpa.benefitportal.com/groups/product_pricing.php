...
group_has_access(10);
    ...
    $prd_main = $pdo->selectOne("SELECT name,pricing_model from prd_main where md5(id)=:id",array(":id"=>$product_id));

    if($prd_main['pricing_model'] !='VariableEnrollee'){
        $sqlCheckPricingQue="SELECT prd_pricing_question_id from prd_pricing_question_assigned where is_deleted='N' AND md5(product_id)=:product_id";
        ...
        $prd_sql = "SELECT $incr pm.price,pm.plan_type from prd_matrix pm $tbl_incr where md5(pm.product_id) = :product_id  
    ...
    // pre_print($prdPlanTypeArray,false);
    // pre_print($products);
$template = 'product_pricing.inc.php';
