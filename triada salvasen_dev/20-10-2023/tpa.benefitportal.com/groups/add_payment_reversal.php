...
$orders = $pdo->select("SELECT o.id,o.display_id from orders o join customer c on(c.id = o.customer_id) where o.status in ('Payment Approved','Pending Settlement') and c.status not in('Customer Abandon','Pending Quote','Pending Validation','Post Payment') order by id desc");
