...
class pagination {
	...
	/**
	 * True, If implement dropdown pagination, False if not
	 */

	public $is_dropdown_pagination=true;
	/**
	...
	public function run($current_page = 1, $query = '', $options = null) {		
		...
		$this->curr_page_no = $current_page;
		/*
	...
	protected function build_links() {
		...
		$this->links_html = '<div class="row table-footer-row">'. PHP_EOL;
		$this->links_html .= ' <div class="col-sm-12">'. PHP_EOL;
		$this->links_html .= '	<div class="pull-left">'. PHP_EOL;
		...
		$this->links_html .= '	<div class="pull-right">'. PHP_EOL;
		$this->links_html .= '		<div class="dataTables_paginate paging_bs_normal paging_bs_new">'. PHP_EOL;
		$this->links_html .= '			<ul class="' . $this->options['class_ul'] . '">' . PHP_EOL;

		$this->build_links_first_prev();
		$this->records_info = $start.' to '.$end.' of '.$this->total_results.' Records';
	...
	protected function build_links_first_prev() {
		...
		if ($this->options['show_links_first_last'] == true) {
			if ($this->current_page == 1 && $this->options['show_links_first_last_if_dead'] == true) {
				$this->links_html .= '<li class="prev ' . $this->options['class_dead_links'] . '"><span>' . $this->options['text_first'] . '</span></li>' . PHP_EOL;
		...
		if ($this->options['show_links_prev_next'] == true) {
			if ($this->current_page == 1 && $this->options['show_links_prev_next_if_dead'] == true) {
				$this->links_html .= '<li class="prev ' . $this->options['class_dead_links'] . '"><span>' . $this->options['text_prev'] . '</span></li>' . PHP_EOL;
			} elseif ($this->current_page != 1) {
			...
			$this->prev_page_url = ($this->current_page != 1 ? $this->build_link_url($this->current_page - 1) : $this->build_link_url(1));
		}
	}
	...
	protected function loop_through_links($start, $finish, $array_block_label = '') {
		...
		while ($counter <= $finish) {
			if ($this->options['current_page_is_link'] == false && $counter == $this->current_page) {
				$this->links_html .= '<li class="' . $this->get_current_or_normal_class($counter) . '">' . $counter . '</li>' . PHP_EOL;
			} else {

				if($this->is_dropdown_pagination)
				{

					$this->links_array['links'][1][] = array(
						'page_number' => $counter,
					...
					$this->links_html.='<li class="live-link theme-form pr">'.PHP_EOL;
					$this->links_html.='<select class="form-control pagination_select" data-size="3">'.PHP_EOL;
					for ($i=1; $i <= $this->total_pages ; $i++) {
						$is_selected=($this->curr_page_no==$i)?'selected':'';
						$this->links_html.='<option value="'.$i.'" '.$is_selected.' data-page_url="'.str_replace('page='.$this->curr_page_no,'page='.$i,$this->links_array['links'][1][0]['link_url']).'">'.$i.'</option>'.PHP_EOL;
					}
					$this->links_html.='</select></li>'.PHP_EOL;
				}
				else
				{
	...
	protected function build_links_next_last() {
		...
		if ($this->options['show_links_prev_next'] == true) {
			...
			$this->next_page_url = ($this->current_page != $this->total_pages ? $this->build_link_url($this->current_page + 1) : $this->build_link_url($this->total_pages));
		}
