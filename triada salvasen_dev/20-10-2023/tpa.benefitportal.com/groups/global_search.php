...
// 	$incr .= " AND (c.fname LIKE :name OR c.lname LIKE :name OR CONCAT(c.fname,' ',c.lname) LIKE :name OR CONCAT(c.lname,' ',c.fname) LIKE :name OR c.email LIKE :email OR c.rep_id=:rep_id OR c.display_id=:user_id OR c.cell_phone LIKE :cell_phone) AND c.upline_sponsors LIKE :sponsor_id";
...
//                   FROM customer c
//                   WHERE (c.fname LIKE :name OR c.lname LIKE :name OR CONCAT(c.fname,' ',c.lname) LIKE :name OR CONCAT(c.lname,' ',c.fname) LIKE :name OR c.email LIKE :email OR c.rep_id=:rep_id  OR c.cell_phone LIKE :cell_phone) AND c.type='Group' AND c.is_deleted ='N' AND c.id>0 AND c.upline_sponsors LIKE :sponsor_id";
// 	$group_params = array(
...
if ($gsearch != "" || ($gsearch_area_type == '' || $gsearch_area_type == 'Leads')) {
	$leads_sql = "SELECT COUNT(DISTINCT l.id) AS `lead`
                FROM leads l
                WHERE (l.fname LIKE :name OR l.lname LIKE :name OR CONCAT(trim(l.fname),' ',trim(l.lname)) LIKE :name OR CONCAT(trim(l.lname),' ',trim(l.fname)) LIKE :name OR l.email LIKE :email OR l.lead_id=:lead_id OR l.cell_phone LIKE :cell_phone) AND l.id>0 AND l.is_deleted = 'N' AND l.sponsor_id=:sponsor_id";
	$leads_params = array(
		':fname' => '%' . makeSafe($gsearch) . '%',
		':lname' => '%' . makeSafe($gsearch) . '%',
		':name' => '%' . makeSafe($gsearch) . '%',
