          ...
          <div class="phone-control-wrap">
            ...
            <div class="phone-addon theme-form text-left">
              ...
              <div class="row">
                <div class="col-md-3">
                  <div class="form-group ">
                    <select class="form-control" name="user_group" id="user_group">
                      ...
                      <option value="Leads" <?= (!empty($broadcaster_res) && $user_type == 'Leads') || !empty($enrollee_id) ? "selected='selected'" : '' ?>>Enrollee</option>
                    </select>
                ...
                <div class="filter_div leads_filter_div specific_value_div" id="filter_div_specific_leads" style="<?=(!empty($broadcaster_res) && $user_type == 'Leads' && $specific_user_group == 'Y') || !empty($enrollee_id)  ? '' : 'display: none;'?>">
                  <div class="col-md-7">
                      <div class="form-group ">
                        <select name="specific_leads[]" class="se_multiple_select" multiple="multiple">
                          <?php  if(!empty($lead_res)) { ?>
                            <?php foreach ($lead_res as $key => $value) { ?>
                              <option value="<?= $value['id'] ?>" <?= (!empty($broadcaster_res) && !empty($specific_user_ids_array) && in_array($value['id'], $specific_user_ids_array)) || $enrollee_id == $value['id'] ? "selected='selected'" : '' ?>><?= $value['lead_name'] . ' - (' . $value['lead_id'] . ') ' ?></option>
                            <?php } ?>
                          <?php } ?>
                        </select>
                        <label>Select Specific Enrollees</label>
...
<script type="text/javascript">
  ...
  $(document).on("change", "#user_group", function(){
    if($(this).val() != '' && $(this).val() != undefined){
      ...
      if($user_group_name == "Leads"){
        $user_group_name = "Enrollees";
      }      
  ...
  $('#broadcast_form').ajaxForm({
      ...
      url:"<?= $GROUP_HOST ?>/ajax_add_email_broadcast.php",
      method: 'POST',
      dataType: 'json',
      success: function(res) {
        $("#ajax_loader").hide();
