        ...
        <div class="panel-body">
          ...
          <div class="file_name_div form-group" style="display: <?php echo ($note_res['file_name'] != '') ? 'block' : 'none'; ?>">
            <div class="input-group">
                    ...
                    <span class="input-group-addon"><a href="<?php echo $show!='show' ? 'javascript:void(0)' : $GROUP_HOST.'/download_note_file.php?file_name='.$note_res['file_name'];?>" class="<?php if($show!='show') { ?>delete_file <?php } ?>"><i class="fa <?php echo $show!='show' ? 'fa-close' : 'fa-download'?>"></i></a>
                    <input type="hidden" name="delete_file_value" value="N" class="delete_file_value">
                    </span>
              </div>
          </div>
        ...
        $('#frm_note_add_main').ajaxForm({
            ...
            success: function (res) {
                ...
                if (res.status == 'success') {
                    ...
                    // window.opener.location.href = '<?=$GROUP_HOST?>/'+ url +'?id=<?=$customer_id?>';
...
function refreshParent(url) 
{
    ...
    window.opener.interactionUpdate('<?=$customer_id?>','notes',url,'groups');
}
</script>