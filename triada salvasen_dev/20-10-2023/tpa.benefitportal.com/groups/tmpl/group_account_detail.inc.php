    ...
    <p class="agp_md_title">Account</p>
       <div class="theme-form">
            ...
            <div class="row">
                ...
                <?= generate2FactorAuthenticationUI($group_row); ?>
            </div>
            <hr>
        </div>
    </div>
    <div role="tabpanel" class="tab-pane" id="gp_attributes">
       ...
       <div class="form-group attributes_btn height_auto clearfix">
            ...
            <?php if(!empty($group_row) && $group_row['status'] == 'Active'){ ?>
                <a href="<?=$HOST?>/downloads3bucketfile.php?file_path=<?=urlencode($GROUP_AGREEMENT_CONTRACT_FILE_PATH)?>&file_name=<?=urlencode($group_row['agent_contract_file'])?>&user_id=<?=$group_id?>&location=group_profile" class="btn btn-action btn-outline">Group Agreement</a>
            <?php } ?>
       </div>
...
<script type="text/javascript">
    ...
    /*  groups_add_resource.php file not exist in group area so when this need then add this file and releated file in group in need
