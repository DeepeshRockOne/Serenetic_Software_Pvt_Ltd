...
$options = array(
	...
	'url' => 'all_broadcast_group.php?' . $query_string,
	'db_handle' => $pdo->dbh,
...
if ($is_history_ajaxed) {
	try {
		...
		$sel_sql = "SELECT b.id, b.brodcast_name, b.status, b.display_id, b.created_at, CONCAT(a.fname, ' ',a.lname) as agent_name, a.rep_id as agent_display_id, a.id as agent_id, b.total_users,b.total_sent, GROUP_CONCAT(CONCAT(bss.schedule_date, ' ', bss.schedule_hour, ':00 ')) as schedule_details,bss.time_zone as schedule_time_zone,b.updated_at,b.user_type,b.type
				...
				WHERE b.sender_type='Group' AND b.status NOT IN ('Draft') AND b.type IN('email','sms') 
	...
	include_once 'tmpl/all_broadcast_group.inc.php';
	exit;
}
$exStylesheets = array("thirdparty/jquery_ui/css/front/jquery-ui-1.9.2.custom.css", 'thirdparty/multiple-select-master/multiple-select.css', 'thirdparty/sweetalert/sweetalert.css','thirdparty/bootstrap-datepicker-master/css/datepicker.css'.$cache);
$exJs = array('thirdparty/multiple-select-master/jquery.multiple.select.js', 'thirdparty/clipboard/clipboard.min.js','thirdparty/bootstrap-datepicker-master/js/bootstrap-datepicker.js');

$template = 'all_broadcast_group.inc.php';
