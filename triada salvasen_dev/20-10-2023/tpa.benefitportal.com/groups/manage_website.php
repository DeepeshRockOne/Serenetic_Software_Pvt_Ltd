...
if(isset($_POST['id']) && isset($_POST['operation'])) {
	...
	if($_POST['operation'] == "delete") {
        ...
        if(!empty($pb_row)) {
            ...
            $check_image = $pdo->select("SELECT * FROM page_builder_images WHERE page_builder_id=:page_builder_id AND is_deleted='N'",array(":page_builder_id"=>$pb_row['id']));
            if (!empty($check_image)) {
                foreach ($check_image as $key => $value) {
                    if(!empty($value["image_name"]) && file_exists($PAGE_COVER_DIR.DIRECTORY_SEPARATOR.$value["image_name"])){
                        unlink($PAGE_COVER_DIR.DIRECTORY_SEPARATOR.$value["image_name"]);
                    }
            ...
            activity_feed(3, $_SESSION['groups']['id'], 'Group', $pb_row['id'], 'page_builder', 'Website Deleted', $_SESSION['groups']['fname'], $_SESSION['groups']['lname'], $desc);
            $res['status'] = 'success';
	...
	if($_POST['operation'] == "change_publish_status") {
        ...
        if(!empty($pb_row)) {
            ...
            $desc = json_encode($desc);
            activity_feed(3, $_SESSION['groups']['id'], 'Group', $pb_row['id'], 'page_builder', 'Website Publish Status Changed', $_SESSION['groups']['fname'], $_SESSION['groups']['lname'], $desc);
...
if ($is_ajaxed) {
	try {
	    $sel_sql = "SELECT pb.*,md5(pb.id) as id
	              ...
	              WHERE pb.is_deleted = 'N' AND pb.agent_id='" . $group_id . "' " . $incr . " 
...
$description['ac_message'] =array(
	...
	'ac_red_2'=>array(
		'href'=>$GROUP_HOST.'/manage_website.php',
