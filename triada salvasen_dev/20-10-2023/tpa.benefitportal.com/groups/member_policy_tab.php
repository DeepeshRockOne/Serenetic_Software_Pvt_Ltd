...
if(!empty($_POST['showing_pass']) || (!empty($_POST['type']) && $_POST['type'] == 'encrypt')){
    $salaryRes = salaryEncryptDecrypt('group','customer');
    ...
    if($validate->isValid()){
        ...
        if(!empty($lead_update_activity)){
            ...
            $description['ac_message'] = array(
                'ac_red_1'=>array(
                    'href'=> 'groups_detail.php?id='.md5($_SESSION['groups']['id']),
...
if($is_address_ajaxed){
    ...
    $validate->digit(array('required' => true, 'field' => 'zip', 'value' => $zip,'min'=> 5,'max'=>5 ), array('required' => 'Zip Code is required'));
...
if($is_ajaxed && $is_update){
    ...
    if($validate->isValid()){
        ...
        if($acUPdate){
            activity_feed(3,$_SESSION['groups']['id'], 'Group',$rows['id'],'customer','Group Update Member Detail',"","",json_encode($ac_desc));
        }
