...
if(!empty($product_ids)) {
    $prd_sql = "SELECT p.id as product_id,p.name as product_name,p.category_id,pc.title as category_name,min(pm.price) as price,pc.short_description,pc.category_image
        ...
        p.product_type IN ('Group Enrollment') AND
        p.id IN(".$product_ids.")
        GROUP BY p.id 
...
$spon_sql = "SELECT CONCAT(c.fname,' ',c.lname) as name,c.rep_id,c.email,c.cell_phone,cs.display_in_member,c.public_name,c.public_phone,c.public_email,cs.is_branding,cs.brand_icon,c.user_name 
            FROM customer c 
...
$website_enroll_link = $GROUP_ENROLLMENT_WEBSITE_HOST . '/' . $pb_row['user_name'];
