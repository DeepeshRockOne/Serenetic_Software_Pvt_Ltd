    ...
    if ($validate->isValid()) {
        if (!empty($profile)) {
            ...
            if ($res == "fail") {
                ...
                $o_data = $pdo->selectOne('SELECT fname,lname,email from customer where id = :id',array(":id"=>$id));
                ...
                if(isset($_REQUEST['is_my_profile'])) {
                    $_SESSION['groups']['brand_icon'] = $image_name;
                    $desc = array();
                    ...
                    $desc['ac_message'] = array(
                        ...
                        'ac_message_1' =>' Updated Brand Logo For Group '.$o_data['fname'].' '.$o_data['lname'].' ',
                        'ac_red_2'=>array(
                            'href'=> $ADMIN_HOST.'/groups_details.php?id='.md5($id),
                            'title'=> $o_data['rep_id'],
                        ),
                    );
                    $desc=json_encode($desc);
                    activity_feed(3,$_SESSION['groups']['id'],'Group',$id,$o_data['type'],$o_data['type'].' Profile Updated',"","",$desc);
                }

                if(isset($_REQUEST['personal_branding'])) {
                    setNotifySuccess('Brand logo saved successfully');
                }
            }
        }
    }
