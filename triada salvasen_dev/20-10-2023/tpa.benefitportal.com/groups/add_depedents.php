...
if($is_ajaxed && $is_delete){
    ...
    if($action == 'Delete' && !empty($dep_rows['id'])){
        ...
        activity_feed(3,$_SESSION['groups']['id'], 'Group' , $rows['id'], 'customer','Group Deleted Dependent',$_SESSION['groups']['fname'],"",json_encode($ac_desc));
        // activity_feed(3,$rows['id'], 'customer' , $rows['id'], 'customer', 'Admin Deleted Dependent',$rows['name'],"",json_encode($ac_desc));
...
if(!empty($customer_id) && $is_dep_ajaxed){
    ...
    if ($validate->isValid()) {
        ...
        if($action == 'Edit'){
            ...
            $ac_action = 'Group Update Dependent Detail';
        }else{
            $display_id = 
            $ac_ms_1 = ' Added new Dependent For Member ';
            $ac_action = 'Group Added New Dependent';
        }
        ...
        if($activity_feed){
            ...
            activity_feed(3,$_SESSION['groups']['id'], 'Group' , $customer_id, 'customer', $ac_action,$_SESSION['groups']['fname'],"",json_encode($ac_desc));
