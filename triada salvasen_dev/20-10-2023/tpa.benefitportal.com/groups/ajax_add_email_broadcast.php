<?php include_once dirname(__FILE__) . '/layout/start.inc.php';
  ...
  // $specific_agents = isset($_POST['specific_agents']) ? $_POST['specific_agents'] : array();
  // $specific_group = isset($_POST['specific_group']) ? $_POST['specific_group'] : array();
  $specific_member = isset($_POST['specific_member']) ? $_POST['specific_member'] : array();
  $specific_leads = isset($_POST['specific_leads']) ? $_POST['specific_leads'] : array();
  ...
  if($upload_type=="file"){
    ...
    if ($validate->isValid()) {
      if(empty($action_type) || (!empty($action_type)) && $action_type != 'send_email'){
        ...
        if($user_group == 'Leads'){
          ...
          if($specific_user_group == 'N'){
            ...
            //     $insert_param['specific_user_ids'] = implode(',', $specific_agents);
            //   }
        ...
        if(isset($update_broadcaster_id) && !empty($update_broadcaster_id) && $is_clone == 'N'){
          ...
          if(!empty($checkDiff)){
            ...
            $activityFeedDesc['ac_message']['ac_red_2']=array(
              'href'=>$GROUP_HOST.'/add_email_broadcast.php?broadcaster_id='.md5($update_broadcaster_id),
              'title'=>$broadcaster_res['display_id']
            ); 
            
            activity_feed(3, $_SESSION['groups']['id'], 'Group', $update_broadcaster_id, 'email_broadcaster','Group Updated Email Broadcaster', $_SESSION['groups']['fname'],$_SESSION['groups']['lname'],json_encode($activityFeedDesc),'',json_encode($extraJson));
          }
          ...
          $description['ac_message'] =array(
            ...
            'ac_red_2'=>array(
                'href'=>$GROUP_HOST.'/add_email_broadcast.php?broadcaster_id='.md5($broadcaster_id),
                'title'=>$display_id,
            ),
          );

          activity_feed(3, $_SESSION['groups']['id'], 'Group', $broadcaster_id, 'email_broadcaster','Group Created Email Broadcaster', $_SESSION['groups']['fname'],$_SESSION['groups']['lname'],json_encode($description));
        }
      }
      ...
      if(!empty($action_type) && $action_type == 'send_email'){
        ...
        $params['EMAILER_SETTING']['from_mail_name'] = $DEFAULT_SITE_NAME;
