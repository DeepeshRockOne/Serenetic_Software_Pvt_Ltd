...
if ($is_ajaxed) {
    ...
    $options = array(
        ...
        'url' => 'members_import_summary.php?' . $query_string,
        'db_handle' => $pdo->dbh,
    ...
    try {
        ...
        LEFT JOIN import_csv_log ilog on(ilog.file_id = ir.id AND ilog.is_deleted='N')
        WHERE ir.is_deleted='N' AND user_type='Group' AND user_id='" . $_SESSION['groups']['id'] . "' GROUP BY ir.id ORDER BY id DESC";
        $paginate = new pagination($page, $sel_sql, $options);
        if ($paginate->success == true) {
            $fetch_rows = $paginate->resultset->fetchAll();
            $total_rows = count($fetch_rows);
        }
        include_once 'tmpl/members_import_summary.inc.php';
        exit();
...
$page_title = "Import Members Summary";
$template = 'members_import_summary.inc.php';
