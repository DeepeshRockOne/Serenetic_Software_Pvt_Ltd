...
$options = array(
    ...
    'url' => 'member_pending_hrm_payments.php?' . $query_string,
    'db_handle' => $pdo->dbh,
...
if ($is_ajaxed) {
    try {
        $sel_sql = "SELECT s.rep_id AS grpRepId,
                    ...
                    WHERE hrmp.is_deleted='N' AND
                    hrmp.status IN('Pending') AND c.is_compliant = 'Y' $incr ORDER BY $SortBy $currSortDirection";
    ...
    include_once 'tmpl/member_pending_hrm_payments.inc.php';
    exit;
}
