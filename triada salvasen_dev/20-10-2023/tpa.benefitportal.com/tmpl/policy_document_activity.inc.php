...
var maxid = "<?=checkIsset($minmum)?>";
var allNumRows = "<?=checkIsset($allNumRows)?>";
</script>
<?php
if(isset($_SESSION['admin'])) {
    $tz = new UserTimeZone('m/d/Y g:i A T',$_SESSION['admin']['timezone']);

} elseif (isset($_SESSION['agents'])) {
    ...
    $tz = new UserTimeZone('m/d/Y g:i A T','CST6CDT');
}
...
$iconArr = array(
    ...
    'triggers' => array('class' => 'icon-envelope', 'color' => 'danger'),
    'sms' => array('class' => 'icon-screen-smartphone', 'color' => 'danger'),
    'customer' => array('class' => 'type-e', 'color' => 'danger'),
);
...
$global_heading = [
    ...
    'is_benefit_tier' => 'Vary by plan tier?',
    'price' => 'Price',
    ...
    'is_fee_by_benefit_tier' => 'Amount vary by plan tier?',
...
<div class="policy_activity">
   <?php
      if ($total_rows > 0) {
      foreach ($fetch_rows as $row) {?>
   <div class="policy_activity_block"  data-user_type="<?=$row['user_type']?>" data-action="<?=$row['entity_action']?>">
      ...
      <div class="policy_activity_head">
         <h4><?=$row['entity_action']?></h4>
         <p><?php echo $tz->getDate($row['changed_at']); ?></p>
      </div>
      <div class="policy_activity_body">
         ...
         <p>
            ...
            <a href="<?= $evalue['href'] ?>" data-desc="<?= checkIsset($evalue['data-desc']) ?>" class="text-action <?= checkIsset($evalue['class']) ?>" data-encode="<?= checkIsset($evalue['data-encode']) ?>"><?= $evalue['title'] ?></a>
            <?php }else{ ?>
            <span class="text-action"><?= $evalue['title'] ?></span>
            <?php } ?>
         ...
         <p>
            ...
            <?= $desHeading ?> <a href="#javascript:void(0)" data-desc="<?= $descValue['data-desc'] ?>" class="text-action <?= $descValue['class'] ?>" data-encode="<?= checkIsset($descValue['data-encode']) ?>"><?= $descValue['title'] ?></a>
            <?php } ?>
         </p>
      ...
      <div class="policy_activity_footer">
         <p><?=$row['ip_address'];?></p>
      </div>
   </div>
   <?php
   ...
   <div class="policy_activity_block text-center">
