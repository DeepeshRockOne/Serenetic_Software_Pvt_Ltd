            ...
            <div id="product_tab" class="tab-pane fade in active">
                <div class="panel-body">
                    <div class="text-center">
                        <h4 class="m-b-30">COBRA benefits for Member ID: <span class="text-action"><?= $rep_id ?></span>
                        </h4>
                        <p class="fs16 m-b-30">Please choose which product(s) the member wants to enroll in COBRA for:</p>
                            ...
                            <div class="btn-group colors" data-toggle="buttons">
                                ...
                                foreach ($terminated_subscriptions as $key => $subscription_row) {
                                    ...
                                    <label class="btn btn-info btn-outline m-b-5 btn_reinstate_subscription" data-ws_id="<?= $subscription_row['ws_id'] ?>">
                                        <input type="checkbox" name="reinstate_subscriptions[<?=$subscription_row['product_id']?>]" class="js-switch reinstate_subscriptions" autocomplete="off" value="<?=$subscription_row['ws_id'] ?>"><?=$subscription_row['product_name'].' '.$subscription_row['product_code']?>
            ...
            <div id="reinstate_tab" class="tab-pane fade ">
                <div class="reinstate_choose_period">
                    <div class="panel-body">
                        ...
                        <div class="panel-footer text-right">
                            <button type="button" class="btn btn-action btn_submit" data-step="3">Confirm
                            </button>
                            <a href="javascript:void(0);" class="btn red-link" onclick="window.parent.$.colorbox.close()">Cancel</a>
                        </div>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('change','.benefit_tier_change_drpdwn',function(){
            $new_plan_id = $(this).val();
            $customer_id = $(this).data('customer_id');
            $product_id = $(this).data('product_id');
            if($new_plan_id == 1){
                $('.dependents_div_' + $product_id).slideUp();
            }else{
                $('.dependents_div_' + $product_id).slideDown();
            }
            $.ajax({
                url: '<?= $HOST ?>/cobra_reinstate_products.php',
                data: {new_plan_id:$new_plan_id,customer_id:$customer_id,product_id:$product_id},
                type: 'POST',
                dataType: 'json',
                success: function (res) {
                    if(res.content == '' && $new_plan_id > 1){
                        swal({
                          text: "To make this change, you must first have qualified dependent(s) added.",
                    ...
                    $('#dependents_' + $product_id).html(res.content);
                    $('#dependents_' + $product_id).multipleSelect('refresh');
                    $('#full_premium_amount_' + $product_id).text(res.price);
                    $('#cobra_product_cost_' + $product_id).text(res.total_amount);
                    $('#cobra_service_fee_' + $product_id).text(res.cobra_service_fee + '%');
                }
            });
            return false;
        });

        // $(document).on('change','.dependents',function(){
        //     $customer_id = $(this).data('customer_id');
        //     $product_id = $(this).data('product_id');
        //     $ws_id = $(this).data('web_id');
        //     $new_plan_id = $('#prd_plan_type' + $product_id).val();
        //     $dependents = $(this).val();

        //     $.ajax({
        //         url: '<?=$HOST?>/ajax_get_product_price_detail.php',
        //         method: 'POST',
        //         data: {customer_id:$customer_id,new_prd_id:$product_id,ws_id:$ws_id,new_plan_type:$new_plan_id,dependant:$dependents},
        //         dataType: 'json',
        //         success: function (res) {
        //             $("#ajax_loader").hide();
        //             console.log(res);
    ...
    function reinstate_product(btn_obj) {
        ...
        $.ajax({
            url: '<?= $HOST ?>/ajax_cobra_reinstate_product.php',
            ...
            success: function (res) {
                ...
                if (typeof (res.coverage_periods_html) !== 'undefined') {
                    ...
                    // $(".temp_drpdown").selectpicker();
                    $(".dependents").multipleSelect({});   
                    common_select();
                    fRefresh();
                }
                ...
                if (res.status == "success") {
                    ...
                    $.each(res.errors, function (index, value) {
                        ...
                        if (is_error) {
                            $("[href='#" + $('#err_' + index).parents(".tab-pane").attr("id") + "']").trigger("click");
                            is_error = false;
