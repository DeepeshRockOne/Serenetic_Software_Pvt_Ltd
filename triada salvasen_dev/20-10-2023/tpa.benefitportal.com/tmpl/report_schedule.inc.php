<style type="text/css">
.filter_fields.bootstrap-select .dropdown-menu .inner.open{ max-height:120px!important; overflow-y: auto!important; }
</style>
<div class="panel panel-default panel-block schedule_popup">
	<form name="frmSchedule" id="frmSchedule" action="" method="POST">
		<input type="hidden" name="report_id" value="<?=$report_row['id']?>">
		<input type="hidden" name="user_id" value="<?=$user_id?>">
		<input type="hidden" name="user_type" value="<?=$user_type?>">
		<input type="hidden" name="hdn_columns" id="hdn_columns">
		<div class="panel-heading">
			<div class="panel-title">
				<h4 class="mn">Schedule - <span class="fw300"><?=$report_name?></span></h4>
			</div>
		</div>
			...
			<div class="report_schedule">
				...
				<div class="row theme-form">
					<div class="col-xs-6">
						<div class="form-group">
							...
							<p class="error"><span id="error_schedule_type"></span></p>
						</div>
					</div>
						...
						<div class="phone-control-wrap">
							...
							<div class="phone-addon">
								<div class="form-group">
									...
									<label>Select</label>
									<p class="error"><span id="error_schedule_frequency"></span></p>
								</div>
							</div>
							<div class="phone-addon w-30">
				...
				<div class="form-group ">
					<select class="form-control" name="generate_via" id="generate_via1">
	                    ...
	                    <?php /*<option value="FTP" <?=$generate_via == 'FTP' ? 'selected="selected"' : ''?>>FTP</option>*/ ?>
	                  </select>
					...
					<div class="col-sm-12">
						<div class="form-group">
							<select class="form-control" name="ftp_name">
								<option value=""></option>
								<option value="LHH">LHH FTP</option>
							</select>
				...
				<?php if(!empty($report_columns)) { ?>
					<div class="row theme-form generate_report_panel">
						<div class="col-xs-12">
				        	<select id='pre-selected-options' multiple='multiple'>
				        		<?php foreach ($report_columns as $column_key=> $field) { ?>
				...
				if(!empty($filter_fields)) {
					...
					<div class="row theme-form">
					<?php
					foreach ($filter_fields as $field_data) {
						include('reports_fields.inc.php');
					}
					?>
					</div>
					<?php
	...
	$('#cancel_processing').on('click',function(){
	 ...
	 $('input[name="schedule_end_type"]').on('click', function(e) {
	  ...
	  $(document).on("click","#schedule",function(){
	    report_schedule($(this));
	  });

	});

function report_schedule(btn_obj) {
    ...
    $.ajax({
        url: '<?=$HOST?>/ajax_report_schedule.php',
