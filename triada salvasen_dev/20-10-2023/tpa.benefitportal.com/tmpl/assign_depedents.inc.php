	...
	<div class="panel-body">
			...
			<?php if($dep_res){ ?>
				...
				<?php } else { ?>
					<div class="form-group text-center">
						<a href="javascript:void(0);" class="text-action fw500"><i class="fa fa-exclamation-circle"></i>  In order to add dependent(s), you must change benefit tier.</a>
		...
		<div class="text-center m-t-20">
			<?php if($tier_change_required == false && !empty($dep_res)){ ?>
				<button class="btn btn-action" type="submit" id="btn_submit">Save</button>
			<?php } ?>
...
$(document).ready(function() {
	...
	$('#frm_add_dep').off('submit');
	$('#frm_add_dep').on('submit',function(e){
	  e.preventDefault();
	  parent.disableButton($("#btn_submit"));
      $('#ajax_loader').show();
