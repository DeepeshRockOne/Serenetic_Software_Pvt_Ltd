      ...
      <?php if ($total_rows > 0) { ?>
        <?php foreach ($fetch_rows as $rows) { ?>
          <tr>
            ...
            <?php $reason = json_decode($rows['response'],true); ?> 
            <td><?= (!empty($reason['reason']))?$reason['reason']:'-'; ?></td>
          </tr>
        <?php } ?>
      <?php } else { ?>
        <tr>
...
<div class="panel panel-default panel-block">
  <form id="popup_frm_search" action="send_email_activity.php" method="GET">
    <div class="panel-heading">
      <div class="panel-title">
        <h4 class="mn">
           <strong class="fw500">Email Activity - <span class="fw300"><?=(!empty($email)?'('.$email.')':'')?>  </span></strong>
        </h4>
      </div>
    </div>
