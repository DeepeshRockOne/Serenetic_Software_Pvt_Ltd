...
$contingent_beneficiary_number = !empty($_POST['contingent_beneficiary_field_number']) ? explode("_", $_POST['contingent_beneficiary_field_number']) : array();

$number = checkIsset($contingent_beneficiary_number[0]);
$display_number = checkIsset($contingent_beneficiary_number[1]);
$response['number'] = $number;

$dependent_information = array();
$dependent_count = 0;

$contingent_existing_dependent = checkIsset($_POST['contingent_existing_dependent'],'arr');
...
$postArray = array(
	...
	'api_key' => 'getContingentBeneficiary'
);
$apiResponse = $ajaxApiCall->ajaxApiCall($postArray,true);
$contingent_beneficiary_field = checkIsset($apiResponse["data"],'arr');
...
if(!empty($contingent_beneficiary_field)){
	ob_start();
	?>
	<div id="inner_contingent_beneficiary_field_<?= $number ?>" class="inner_contingent_beneficiary_field">
	<div class="clearfix m-b-25">
	    <h5 class="mn pull-left">Contingent Beneficiary <span data-display_number="<?= $display_number ?>" data-id="<?= $number ?>" id="contingent_beneficiary_number_<?= $number ?>" class="display_contingent_beneficiary_number"><?= $display_number ?></span>
	    <a href="javascript:void(0);" class="red-link removeContingentBeneficiaryField" data-id="<?= $number ?>">Remove</a>
	    </h5>
  	</div>
 	<div class="row enrollment_auto_row">
 		<div class="col-sm-3">
	      <div class="form-group">
	      	<input type="hidden" name="contingent_beneficiary_id[<?= $number ?>]" id="contingent_beneficiary_id_<?= $number ?>" value="0">
	      	<select id="contingent_existing_dependent_<?= $number ?>"  name="contingent_existing_dependent[<?= $number ?>]" class="contingent_beneficiary_select_<?= $number ?> contingent_beneficiary_select" data-id="<?= $number ?>" data-select-val="">
	      		<option value="" ></option>
	      		<?php if(!empty($dependent_information)){ ?>
	      			<?php foreach ($dependent_information as $key => $row) { ?>
	      				<?php if(!empty($row['fname'])){ ?>
	      					<option value="<?= $row['id'] ?>" data-key="<?= $row['key'] ?>" data-full-name="<?= $row['fname'] .' '.$row['lname'] ?>" data-type="<?= $row['type'] ?>" data-fname = "<?= $row['fname'] ?>" data-lname = "<?= $row['lname'] ?>" data-phone = "<?= $row['phone'] ?>" data-email = "<?= $row['email'] ?>" data-ssn = "<?= $row['ssn'] ?>" data-address="<?= $row['address'] ?>" style="<?= !empty($contingent_existing_dependent) && in_array($row['id'],$contingent_existing_dependent) ? 'display: none' : '' ?>"><?= $row['fname'] .' '.$row['lname'] .' ('.$row['type'].')' ?></option>
	      				<?php } ?>	
	      			<?php } ?>
	      		<?php } ?>
	      	</select>
	        ...
	        ?>
	      	<?php if($control_type=='text'){?>
				<div class="col-lg-3 col-md-6">
					...
					<?php }else {?>
						<div class="form-group">
							<input type="text" id="<?= $control_name ?>_<?= $number ?>" maxlength="<?= $maxlength ?>" name="<?= $control_name ?>[<?= $number ?>]" value="" class="form-control <?= ($control_name == "contingent_queBeneficiaryEmail") ? "no_space" : "" ?> <?= $class ?>"  required data-id="<?= $number ?>" >
							<label><?= $label ?><?php if( $is_required == 'Y') { ?><span class="req-indicator">*</span><?php } ?></label>
							<p class="error" id="error_<?= $control_name ?>_<?= $number ?>"></p>
						</div>
					<?php } ?>
	      	...
	      	<?php }else if($control_type=='select'){ ?>
	    		<div class="col-lg-3 col-md-6">
              		<div class="form-group">
		              	<select id="<?= $control_name ?>_<?= $number ?>"  name="<?= $control_name ?>[<?= $number ?>]" class="contingent_beneficiary_select_<?= $number ?>" required data-live-search="true" data-id="<?= $number ?>">
	      	...
	      	<?php }else if($control_type=='select_multiple'){ ?>
	        	<div class="col-lg-3 col-md-6">
              		<div class="form-group">
		              	<select id="<?= $control_name ?>_<?= $number ?>"  name="<?= $control_name ?>[<?= $number ?>][]" class="se_multiple_select contingent_beneficiary_multiple_select_<?= $number ?>" required data-live-search="true" data-id="<?= $number ?>" multiple="multiple">
		              		<?php if($control_name=='contingent_product'){  ?>
