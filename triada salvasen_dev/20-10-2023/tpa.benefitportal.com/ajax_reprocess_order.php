...
if ($validate->isValid()){
    ...
    $insert_billing = true;
    $order_res = $pdo->selectOne('SELECT id,display_id,is_renewal,subscription_ids,customer_id,grand_total FROM orders WHERE id = :id AND customer_id=:customer_id AND status NOT IN("Pending Settlement","Payment Approved")', array(':id' => $order_id,":customer_id"=>$customer_id));
        ...
        if(strtotime(date('Y-m-d',strtotime($orderdate))) == strtotime(date('Y-m-d')))
        {
            ...
            if(!empty($order_res['id'])){
                ...
                if(!empty($order_billing_res['customer_billing_id'])){
                    $billing_id = $order_billing_res['id'];
           ...
           $customer_billing_sql = "SELECT *,AES_DECRYPT(ach_account_number,'".$CREDIT_CARD_ENC_KEY."') as decrypt_ach_account_number,
            ...
            if($billing_id > 0) {
                unset($order_billing_info_params['created_at']);
                $order_billing_id = $billing_id;
                $pdo->update("order_billing_info", $order_billing_info_params, array("clause" => "id=:id", "params" => array(":id" => $billing_id)));
            }else{
            ...
            if ($payment_mode == "ACH") {
                ...
                if ($payment_res['status'] == 'Success') {
                    ...
                    $decline_log_id = $functionLst->credit_card_decline_log($customer_id, $cc_params, $payment_res);
                }
            ...
            if ($paymentApproved) {
                ...
                foreach ($order_ws_result as $order_ws_row) {
                    ...
                    $update_ws_where = array("clause" => "id=:id", "params" => array(":id" => $order_ws_row['id']));
                    ...
                    $extra_params['message'] = $msg;
                    $extra_params['transaction_id'] = $txn_id;
                    $extra_params['order_id'] = $order_id;
                    $policySetting->removeTerminationDate($order_ws_row["id"],$extra_params);
                    ...
                    // activity_feed(3, $customer_row['sponsor_id'], $customer_row['sponsor_type'], $customer_row['id'], 'customer', $msg , $prd_row['name'], "", 0);
                }
                ...
                $functionLst->checkJoinderAgreement($order_id);
                ...
                $customer_status = $member_setting['member_status'];
                $res['status'] = 'success';
            } else {
                $order_params = array(
                    'transaction_id' => makeSafe($txn_id),
                ...
                $other_params=array("transaction_id"=>$txn_id,'transaction_response'=>$payment_res,'reason'=>checkIsset($payment_error),'cc_decline_log_id'=>checkIsset($decline_log_id));
                ...
                foreach ($order_ws_result as $order_ws_row) {
                    ...
                    $member_setting = $memberSetting->get_status_by_payment($paymentApproved,"","","",array('attempt' => $order_ws_row['total_attempts'] + 1));
                    if (!empty($attemptRow)) {
                        ...
                        $customer_status = $updateArr['status'] = $member_setting['member_status'];
            ...
            if($location == "admin") {
                ...
                activity_feed(3, $_SESSION['agents']['id'], 'Agent', $customer_id, 'Customer', 'Reprocess Order','','',json_encode($activity_feed_data));
                ...
                activity_feed(3, $_SESSION['groups']['id'], 'Group', $customer_id, 'Customer', 'Reprocess Order','','',json_encode($activity_feed_data));
            }
        }else{
            $customer_id = $order_res['customer_id'];
            $customer_sql = "SELECT fname,lname,rep_id,sponsor_id,id,status FROM customer WHERE id=:customer_id";
            $customer_where = array(":customer_id" => $customer_id);
            $customer_row = $pdo->selectOne($customer_sql, $customer_where);
            ...
            $member_setting = $memberSetting->get_status_by_payment(false,"",true,$customer_row['status']);
