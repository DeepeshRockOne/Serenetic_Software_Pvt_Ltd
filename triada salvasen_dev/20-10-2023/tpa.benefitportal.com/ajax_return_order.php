...
if(in_array($reversal_type, array('Chargeback','Payment Return'))){
	...
	if ($validate->isValid()) {
		...
		$member_setting = $memberSetting->get_status_by_order_status($reversal_type);
	    ...
	    if ($reversal_type == 'Chargeback') {
	        ...
	        if($location == "admin") {
		        ...
		        $desc = json_encode($description);
	  			activity_feed(3, $_SESSION['agents']['id'], 'Agent', $order_row['customer_id'], 'Customer', 'Chargebacked Order','', "", $desc);
		        ...
		        $desc = json_encode($description);
	  			activity_feed(3, $_SESSION['groups']['id'], 'Group', $order_row['customer_id'], 'Customer', 'Chargebacked Order','', "", $desc);
	        }
	        ...
	        if($location == "admin") {
		        ...
		        $desc = json_encode($description);
	  			activity_feed(3, $_SESSION['agents']['id'], 'Agent', $order_row['customer_id'], 'Customer', 'Payment Returned','', "", $desc);
	        } elseif($location == "group") {
        		$description['ac_message'] = array(
				    'ac_red_1' => array(
			      		'href'=> $ADMIN_HOST .'/groups_details.php?id='.md5($_SESSION['groups']['id']),
		        ...
		        $desc = json_encode($description);
	  			activity_feed(3, $_SESSION['groups']['id'], 'Group', $order_row['customer_id'], 'Customer', 'Payment Returned','', "", $desc);
	        }
	    }
			...
			if($reversal_type=='Refund'){
				...
				if($location == "admin") {
	        		...
	        		if($is_partial_refund){
						...
						$description['descriptions'][] = 'Transaction ID : '.$txn_id;
				        $desc = json_encode($description);
				        activity_feed(3, $_SESSION['agents']['id'], 'Agent', $order_row['customer_id'], 'Customer', 'Partial Refund','', "", $desc);
					}else{
						$description['ac_message'] = array(
						    'ac_red_1' => array(
						    	'href'=> 'agent_detail_v1.php?id='.md5($_SESSION['agents']['id']),
						...
						$description['descriptions'][] = 'Transaction ID : '.$txn_id;
				        $desc = json_encode($description);
			  			activity_feed(3, $_SESSION['agents']['id'], 'Agent', $order_row['customer_id'], 'Customer', 'Full Refund','', "", $desc);
					}
		        }elseif($location == "group") {
					if($is_partial_refund){
						$description['ac_message'] = array(
						...
						$description['descriptions'][] = 'Transaction ID : '.$txn_id;
				        $desc = json_encode($description);
				        activity_feed(3, $_SESSION['groups']['id'], 'Group', $order_row['customer_id'], 'Customer', 'Partial Refund','', "", $desc);
					}else{
						$description['ac_message'] = array(
						    'ac_red_1' => array(
						    	'href'=> $ADMIN_HOST .'groups_details.php?id='.md5($_SESSION['groups']['id']),
						...
						$description['descriptions'][] = 'Transaction ID : '.$txn_id;
				        $desc = json_encode($description);
			  			activity_feed(3, $_SESSION['groups']['id'], 'Group', $order_row['customer_id'], 'Customer', 'Full Refund','', "", $desc);
					}
				}
			}else{
				if($location == "admin") {
			        ...
			        $desc = json_encode($description);
			        activity_feed(3, $_SESSION['agents']['id'], 'Agent', $order_row['customer_id'], 'Customer', 'Void Order','', "", $desc);
				}elseif($location == "group") {
					$description['ac_message'] = array(
						'ac_red_1' => array(
							  'href'=> $ADMIN_HOST . '/groups_details.php?id='.md5($_SESSION['groups']['id']),
					...
					$desc = json_encode($description);
					  activity_feed(3, $_SESSION['groups']['id'], 'Group', $order_row['customer_id'], 'Customer', 'Void Order','', "", $desc);
				}
			}
