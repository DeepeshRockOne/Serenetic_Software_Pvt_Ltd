...
switch ($_POST['function']) {
    case 'is-online':
        $result = 'offline';
        $user = sb_db_get('SELECT last_activity, user_type,app_user_type FROM sb_users WHERE id = "' . $_POST['user_id'] . '" LIMIT 1');
    ...
    case 'update-user':
    case 'add-user':
        $values = ['first_name', 'last_name', 'email', 'profile_image', 'password', 'user_type', 'department','app_user_type'];
        $settings = [];
