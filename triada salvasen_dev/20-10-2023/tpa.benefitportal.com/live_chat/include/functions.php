...
const  SELECT_FROM_USERS = 'SELECT id, first_name, last_name, email, profile_image, user_type, creation_time, last_activity, department, token, app_user_id';
...
function sb_login($email = '', $password = '', $user_id = '', $user_token = '') {
    ...
    if ($email != '' && $password != '') {
        ...
        $result = sb_db_get('SELECT id, app_user_id, app_user_type, profile_image, first_name, last_name, email, user_type, token, department, password FROM sb_users WHERE email = "' . $email . '" LIMIT 1');
        ...
        // Login for visitors
        $result = sb_db_get('SELECT id, app_user_id, app_user_type, profile_image, first_name, last_name, email, user_type, token FROM sb_users WHERE id = "' . $user_id . '" AND token = "' . $user_token . '" LIMIT 1');
        if (sb_is_error($result)) return $result;
    ...
    if ($valid_login) {
        $settings =  ['id' => $result['id'], 'profile_image' => $result['profile_image'], 'first_name' => $result['first_name'], 'last_name' => $result['last_name'], 'email' => $result['email'], 'user_type' => $result['user_type'], 'token' => $result['token'], 'app_user_id' => $result['app_user_id'], 'app_user_type' => $result['app_user_type']];
...
function sb_login_custom($app_user_id,$app_user_type,$app_user_data = array()) {
    global $SB_LOGIN;
    $valid_login = false;
    $result = null;

    $result = sb_db_get('SELECT id, app_user_id, app_user_type, profile_image, first_name, last_name, email, user_type, token, department, password,app_user_type FROM sb_users WHERE app_user_id = "' . $app_user_id . '" AND app_user_type = "' . $app_user_type . '" LIMIT 1');
    if (isset($result) && $result != '' && isset($result['user_type'])) {
        $valid_login = true;
        $user_settings = array(
            'profile_image'=> array($app_user_data['profile_image']),
            'first_name'=> array($app_user_data['first_name']),
            'last_name'=>array($app_user_data['last_name']),
            'password'=>array($app_user_data['password']),
            'email'=>array($app_user_data['email']),
            'user_type'=>array($app_user_data['user_type']),
            'app_user_type'=>array($app_user_data['app_user_type']),
        );
        sb_update_user($result['id'],$user_settings);

        foreach ($result as $key => $value) {
            if(isset($app_user_data[$key])) {
                $result[$key] = $app_user_data[$key];
            }
        }
    } else {
        $user_settings = array(
            'profile_image'=> $app_user_data['profile_image'],
            'first_name'=> $app_user_data['first_name'],
            'last_name'=>$app_user_data['last_name'],
            'password'=>$app_user_data['password'],
            'email'=>$app_user_data['email'],
            'user_type'=>$app_user_data['user_type'],
            'app_user_id'=>$app_user_id,
            'app_user_type'=>$app_user_type,
        );
        $user_id = sb_add_user($user_settings);

        $result = sb_db_get('SELECT id, app_user_id, app_user_type, profile_image, first_name, last_name, email, user_type, token, department, password FROM sb_users WHERE id="' . $user_id . '" LIMIT 1');
        if (isset($result) && $result != '' && isset($result['user_type'])) {
            $valid_login = true;
        }
    }
...
function sb_js_global() {
    ...
    $code .= 'var SITE_CACHE = "' . SITE_CACHE . '";';
    $code .= 'var SB_URL = "' . SB_URL . '";';
    $code .= 'var DEFAULT_CHAT_HEADER_IMAGE = "' . DEFAULT_CHAT_HEADER_IMAGE . '";';
...
function sb_add_user($settings = [], $settings_extra = [], $login_app = false) {
    ...
    //REMOVE THIS DUE TO ADMIN AND AGENTS HAVE SAME EMAIL ID
    ...
    if ($settings['profile_image'] == '') {
        $settings['profile_image'] = $LIVE_CHAT_HOST.'/media/user.svg';
    }
    ...
    if (empty($settings['app_user_id'])) {
        $settings['app_user_id'] = NULL;
    }
    if (empty($settings['app_user_type'])) {
        $settings['app_user_type'] = 'Website';
    }
...
function sb_update_user($user_id, $settings, $settings_extra = []) {
    ...
    $app_user_type = sb_isset($settings, 'app_user_type', ['NULL'])[0];
    ...
    if ($active_user != false && $active_user['id'] == $user_id) {
        $result = sb_update_login($profile_image, $first_name, $last_name, $email, $department, $user_type,$app_user_type);
    }
    return $result;
}
...
function sb_get_online_admins($exclude_id = -1, $sorting = 'creation_time') {
    $users = sb_db_get(SELECT_FROM_USERS . ' FROM sb_users WHERE user_type = "admin" AND last_activity > "' . gmdate('Y-m-d H:i:s', time() - 15) . '"' . ($exclude_id > 0 ? ' AND id <> "' . $exclude_id . '"' : '') . ' ORDER BY ' . $sorting . ' DESC', false);
    if (isset($users) && is_array($users)) {
        return $users;
    } else {
        return new SBError('db-error', 'sb_get_online_admins', $users);
    }
}
...
function sb_get_idle_admins($exclude_id = -1, $sorting = 'creation_time') {
    $qry = 'SELECT sb.* FROM sb_users sb 
            LEFT JOIN sb_conversations sb_c ON (sb.id = sb_c.assign_id)
            WHERE sb_c.id is null AND sb.user_type = "admin" AND sb.last_activity > "' . gmdate('Y-m-d H:i:s', time() - 15) . '"' . ($exclude_id > 0 ? ' AND sb.id <> "' . $exclude_id . '"' : '') . ' ORDER BY ' . $sorting . ' DESC';
    $users = sb_db_get($qry, false);
    
    if (isset($users) && is_array($users)) {
        return $users;
    } else {
        return new SBError('db-error', 'sb_get_idle_admins', $users);
    }
}
...
function sb_get_conversations_users($conversations) {
    if (count($conversations) > 0) {
        ...
        $qry='SELECT sb_u.id, sb_u.first_name, sb_u.last_name, sb_u.profile_image, sb_u.user_type, sb_c.id as conversation_id, sb_c.status_code,sb_u.app_user_type,c.rep_id,sb_u.app_user_id,md5(sb_u.app_user_id) encryptId
            FROM sb_users sb_u
            JOIN sb_conversations sb_c ON (sb_u.id = sb_c.user_id)
            LEFT JOIN '.$tmpDb.'.customer c ON (c.id = sb_u.app_user_id AND sb_u.app_user_type IN ("Agent","Group","Customer"))
            WHERE sb_u.id = sb_c.user_id AND sb_c.id IN ' . $code;
        $result = sb_db_get($qry, false);
        ...
        for ($i = 0; $i < count($conversations); $i++) {
            ...
            for ($j = 0; $j < count($result); $j++) {
                if ($conversation_id == $result[$j]['conversation_id']) {
                    ...
                    $conversations[$i]['app_user_type'] = $result[$j]['app_user_type'];
                    $conversations[$i]['rep_id'] = $result[$j]['rep_id'];
                    $conversations[$i]['app_user_id'] = $result[$j]['app_user_id'];
                    $conversations[$i]['encryptId'] = $result[$j]['encryptId'];
                    break;
                }
            }
        }
...
function sb_get_conversations($pagination = 0, $status_code = 0, $routing = false) {
    ...
    $result = sb_db_get('SELECT sb_messages.*, sb_users.user_type as message_user_type,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message FROM sb_messages, sb_users, sb_conversations WHERE sb_users.id = sb_messages.user_id ' . $status_code . ' AND sb_conversations.id = sb_messages.conversation_id' . (sb_get_agent_department() !== false ? ' AND sb_conversations.department = ' . sb_get_agent_department() : '') . sb_routing_db($routing) . ' AND sb_messages.creation_time IN (SELECT max(creation_time) latest_creation_time FROM sb_messages GROUP BY conversation_id) GROUP BY conversation_id ORDER BY sb_conversations.status_code DESC, sb_messages.creation_time DESC LIMIT ' . ($pagination * 100 + $i) . ',100', false);
    if (isset($result) && is_array($result)) {
...
function sb_get_agent_conversations($agent_id=0,$pagination = 0, $status_code = 0, $routing = false) {
    
    //$agent_id = !empty($agent_id) ? 'AND sb_conversations.agent_id = '.$agent_id : '';
    $agent_id = !empty($agent_id) ? 'AND (sb_conversations.agent_id = '.$agent_id.' OR sb_conversations.assign_id = '.$agent_id.' OR sb_conversations.assist_id = '.$agent_id .')' : '';
    //print_r($agent_id);exit;
    $status_code = $status_code == 0 ? 'AND sb_conversations.status_code <> 3 ' : 'AND sb_conversations.status_code = ' . $status_code;
    $result = sb_db_get('SELECT sb_messages.*, sb_users.user_type as message_user_type,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message FROM sb_messages, sb_users, sb_conversations WHERE sb_users.id = sb_messages.user_id '. $agent_id.' ' . $status_code . ' AND sb_conversations.id = sb_messages.conversation_id' . (sb_get_agent_department() !== false ? ' AND sb_conversations.department = ' . sb_get_agent_department() : '') . sb_routing_db($routing) . ' AND sb_messages.creation_time IN (SELECT max(creation_time) latest_creation_time FROM sb_messages GROUP BY conversation_id) GROUP BY conversation_id ORDER BY sb_messages.creation_time DESC', false);
...
function sb_get_new_agent_conversations($agent_id,$datetime, $routing = false) {

    $agent_id = !empty($agent_id) ? ' AND (c.agent_id = '.$agent_id.' OR c.assign_id = '.$agent_id.' OR c.assist_id = '.$agent_id .')' : '';

    $result = sb_db_get('SELECT m.*, u.user_type as message_user_type,AES_DECRYPT(m.message,"Gserw54sf533sS") as message FROM sb_messages m, sb_users u, sb_conversations c WHERE m.creation_time IN (SELECT max(creation_time) latest_creation_time FROM sb_messages WHERE creation_time > "' . $datetime . '" GROUP BY conversation_id) AND u.id = m.user_id AND c.id = m.conversation_id '.$agent_id . (sb_get_agent_department() !== false ? ' AND c.department = ' . sb_get_agent_department() : '') . sb_routing_db($routing, 'c') . ' GROUP BY conversation_id ORDER BY m.creation_time DESC', false);
    if (isset($result) && is_array($result)) {
        if (count($result) > 0) {
            return sb_get_conversations_users($result);
        } else {
        ...
        return new SBError('db-error', 'sb_get_new_agent_conversations', $result);
    }
}
...
function sb_get_conversation($user_id, $conversation_id) {
    $messages = sb_db_get('SELECT sb_messages.*, sb_users.first_name, sb_users.last_name, sb_users.profile_image, sb_users.user_type,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message FROM sb_messages, sb_users, sb_conversations WHERE sb_conversations.status_code <> 3 AND sb_conversations.status_code <> 4 AND sb_messages.conversation_id = "' . $conversation_id . '" AND sb_conversations.user_id = "' . $user_id . '" AND sb_messages.conversation_id = sb_conversations.id AND sb_users.id = sb_messages.user_id ORDER BY sb_messages.creation_time ASC', false);
    if (isset($messages) && is_array($messages)) {
        $details = sb_db_get('SELECT sb_users.id as user_id, sb_users.first_name, sb_users.last_name, sb_users.profile_image, sb_users.user_type, sb_conversations.id, sb_conversations.title, sb_conversations.creation_time, sb_conversations.status_code as conversation_status_code, sb_conversations.department, sb_conversations.agent_id, sb_conversations.display_id,sb_conversations.is_chat_waiting FROM sb_users, sb_conversations WHERE sb_conversations.status_code <> 3 AND sb_conversations.status_code <> 4 AND sb_conversations.id = "' . $conversation_id . '" AND sb_users.id = "' . $user_id . '" AND sb_users.id = sb_conversations.user_id LIMIT 1');
        return ['messages' => $messages, 'details' => $details];
    } else {
        return new SBError('db-error', 'sb_get_conversation', $messages);
    }
}
function sb_get_agent_conversation($user_id, $conversation_id) {
    $messages = sb_db_get('SELECT sb_messages.*, sb_users.first_name, sb_users.last_name, sb_users.profile_image, sb_users.user_type,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message FROM sb_messages, sb_users, sb_conversations WHERE sb_conversations.status_code <> 3  AND sb_messages.conversation_id = "' . $conversation_id . '" AND sb_conversations.user_id = "' . $user_id . '" AND sb_messages.conversation_id = sb_conversations.id AND sb_users.id = sb_messages.user_id ORDER BY sb_messages.creation_time ASC', false);
    if (isset($messages) && is_array($messages)) {
        $details = sb_db_get('SELECT sb_users.id as user_id, sb_users.first_name, sb_users.last_name, sb_users.profile_image, sb_users.user_type, sb_conversations.id, sb_conversations.title, sb_conversations.creation_time, sb_conversations.status_code as conversation_status_code, sb_conversations.department, sb_conversations.agent_id, sb_conversations.display_id,sb_users.app_user_type,sb_conversations.is_chat_waiting FROM sb_users, sb_conversations WHERE sb_conversations.status_code <> 3  AND sb_conversations.id = "' . $conversation_id . '" AND sb_users.id = "' . $user_id . '" AND sb_users.id = sb_conversations.user_id LIMIT 1');
        return ['messages' => $messages, 'details' => $details];
    } else {
        return new SBError('db-error', 'sb_get_agent_conversation', $messages);
    }
}
...
function sb_get_new_messages($user_id, $conversation_id, $datetime) {
    $qry='SELECT sb_messages.*, sb_users.first_name, sb_users.last_name, sb_users.profile_image, sb_users.user_type,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message,sb_conversations.status_code as conversation_status_code FROM sb_messages, sb_users, sb_conversations WHERE sb_messages.creation_time > "' . $datetime . '" AND sb_messages.conversation_id = "' . $conversation_id . '" AND sb_users.id = sb_messages.user_id AND sb_conversations.user_id = "' . $user_id . '" AND sb_messages.conversation_id = sb_conversations.id ORDER BY sb_messages.creation_time ASC';
    $result = sb_db_get($qry, false);
...
function sb_is_conversation_assigned($conversation_id) {
    $qry='SELECT sb_conversations.is_chat_waiting  FROM sb_conversations WHERE sb_conversations.id = "' . $conversation_id . '"';
    $result = sb_db_get($qry, true);
    
    if (isset($result) && is_array($result)) {
        return $result;
    } else {
        return new SBError('db-error', 'sb_is_conversation_assigned', $result);
    }
}
...
function sb_search_conversations($search, $routing) {
    ...
    $result = sb_db_get('SELECT sb_messages.*, sb_users.user_type as message_user_type,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message FROM sb_messages, sb_users' . ($department !== false || $routing ? ', sb_conversations' : '') . ' WHERE sb_users.id = sb_messages.user_id' . ($department !== false ? ' AND sb_conversations.id = sb_messages.conversation_id AND sb_conversations.department = ' . $department : '') . sb_routing_db($routing) . ($routing && $department === false ? ' AND sb_conversations.id = sb_messages.conversation_id' : '') .' AND (LOWER(sb_messages.message) LIKE "%' . $search . '%" OR LOWER(sb_messages.attachments) LIKE "%' . $search . '%" OR LOWER(sb_users.first_name) LIKE "%' . $search . '%" OR LOWER(sb_users.last_name) LIKE "%' . $search . '%" OR LOWER(sb_users.email) LIKE "%' . $search . '%") GROUP BY sb_messages.conversation_id ORDER BY sb_messages.creation_time DESC', false);
    if (isset($result) && is_array($result)) {
        return sb_get_conversations_users($result);
    } else {
...
function sb_new_conversation($user_id, $status_code = 0, $title = '', $department = -1, $agent_id = -1, $routing = false) {
    ...
    $display_id = get_conversation_display_id();
    $conversation_id = sb_db_query('INSERT INTO sb_conversations(user_id, display_id, title, status_code, creation_time, department, agent_id) VALUES ("' . $user_id . '", "' . $display_id . '", "' . sb_db_escape(htmlentities(ucfirst($title), ENT_NOQUOTES)) . '", "' . $status_code . '", "' . gmdate('Y-m-d H:i:s') . '", ' . (sb_isset_num($department) ? $department : 'NULL') . ', ' . (sb_isset_num($agent_id) ? $agent_id : 'NULL') . ')', true);
...
function get_conversation_display_id()
{
    $display_id = rand(10000000,99999999);
    if(sb_db_get('SELECT count(*) as count FROM sb_conversations WHERE display_id = "LC' . $display_id . '"')['count'] == 0) {
        return 'LC'.$display_id;
    } else {
        return get_conversation_display_id();       
    }
}
...
function sb_get_user_conversations($user_id, $exclude_id = -1) {
    ...
    return sb_db_get('SELECT sb_messages.*, sb_users.first_name, sb_users.last_name, sb_users.profile_image, sb_users.user_type, sb_conversations.status_code AS conversation_status_code, sb_conversations.department,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message FROM sb_messages, sb_users, sb_conversations WHERE sb_conversations.status_code <> 3 AND sb_conversations.status_code <> 4 AND sb_users.id = sb_messages.user_id' . (sb_get_agent_department() !== false ? ' AND sb_conversations.department = ' . sb_get_agent_department() : '') . ' AND sb_messages.conversation_id = sb_conversations.id AND sb_messages.creation_time IN (SELECT max(sb_messages.creation_time) AS latest_creation_time FROM sb_messages, sb_conversations WHERE sb_messages.conversation_id = sb_conversations.id AND sb_conversations.user_id = "' . $user_id . '"' . $exclude . ' GROUP BY conversation_id) GROUP BY conversation_id ORDER BY creation_time DESC', false);
}
...
function sb_get_new_user_conversations($user_id, $datetime) {
    return sb_db_get('SELECT sb_messages.*, sb_users.first_name, sb_users.last_name, sb_users.profile_image, sb_users.user_type, sb_conversations.status_code AS conversation_status_code, sb_conversations.department,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message FROM sb_messages, sb_users, sb_conversations WHERE sb_conversations.status_code <> 3 AND sb_conversations.status_code <> 4 AND sb_users.id = sb_messages.user_id AND sb_messages.conversation_id = sb_conversations.id AND sb_messages.creation_time IN (SELECT max(sb_messages.creation_time) AS latest_creation_time FROM sb_messages, sb_conversations WHERE sb_messages.creation_time > "' . $datetime . '" AND sb_messages.conversation_id = sb_conversations.id AND sb_conversations.user_id = "' . $user_id . '" GROUP BY conversation_id) GROUP BY conversation_id ORDER BY creation_time DESC', false);
}
 ...
 * -----------------------------------------------------------
 *
 * Return the conversation by status_code
 * status_code: live = 0, pending = 1, pending user = 2, archive = 3, trash = 4.
 *
 */

function sb_get_live_conversations($routing = false) {
    $status_code = 'AND (sb_conversations.status_code IN(0,1))';
    $incr = " AND DATE(sb_messages.creation_time)='".date("Y-m-d")."'";
    $result = sb_db_get('SELECT sb_messages.*, sb_users.user_type as message_user_type,sb_conversations.agent_id as assignedId,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message FROM sb_messages, sb_users, sb_conversations WHERE sb_users.id = sb_messages.user_id ' . $status_code . $incr .' AND sb_conversations.id = sb_messages.conversation_id' . (sb_get_agent_department() !== false ? ' AND sb_conversations.department = ' . sb_get_agent_department() : '') . sb_routing_db($routing) . ' AND sb_messages.creation_time IN (SELECT max(creation_time) latest_creation_time FROM sb_messages GROUP BY conversation_id) GROUP BY conversation_id ORDER BY sb_conversations.status_code DESC, sb_messages.creation_time DESC', false);
    if (isset($result) && is_array($result)) {
        return sb_get_conversations_users($result);
    } else {
        return new SBError('db-error', 'sb_get_live_conversations', $result);
    }
}
...
function sb_get_in_queue_conversations($routing = false) {
    //$incr = ' AND DATE(sb_conversations.creation_time)="'.date("Y-m-d").'"';
    $qry = 'SELECT sb_conversations.id as conversation_id FROM sb_users,sb_conversations WHERE sb_users.id = sb_conversations.user_id AND sb_conversations.assign_id is null AND sb_conversations.status_code NOT IN (3,4) ORDER BY sb_conversations.status_code DESC,sb_conversations.creation_time DESC';
    $result = sb_db_get($qry, false);

    if (isset($result) && is_array($result)) {
        return sb_get_conversations_users($result);
    } else {
        return new SBError('db-error', 'sb_get_in_queue_conversations', $result);
    }
}
...
function sb_get_served_conversations($routing = false) {
    
    $incr = ' AND DATE(sb_conversations.creation_time)="'.date("Y-m-d").'"';
    $qry = 'SELECT sb_conversations.id as conversation_id FROM sb_users,sb_conversations WHERE sb_users.id = sb_conversations.user_id AND sb_users.app_user_type!="Admin" '.$incr.'  ORDER BY sb_conversations.status_code DESC,sb_conversations.creation_time DESC';
    $result = sb_db_get($qry, false);
    
    if (isset($result) && is_array($result)) {
        return sb_get_conversations_users($result);
    } else {
        return new SBError('db-error', 'sb_get_served_conversations', $result);
    }
}
        ...
        if ($security) {
            ...
            if ($conversation_status_code != -1) {
                if (in_array($conversation_status_code, [0, 1, 2, 3, 4])) {
                    $agent_id_str = '';
                    if(sb_is_agent()) {
                        $agent_id_str = ',agent_id='.$user_id.',is_chat_waiting="A"';
                    }
                    sb_db_query('UPDATE sb_conversations SET status_code = ' . $conversation_status_code . $agent_id_str.' WHERE id = "' . $conversation_id . '" LIMIT 1');
                    if(sb_is_agent()) {
                        $assignCheck = sb_db_get('SELECT assign_id FROM sb_conversations WHERE id = "' . $conversation_id . '" LIMIT 1');

                        if(!empty($assignCheck) && empty($assignCheck['assign_id'])){
                            sb_db_query('UPDATE sb_conversations SET assign_id = ' . $user_id .' WHERE id = "' . $conversation_id . '" LIMIT 1');
                        }
                    }
...
function sb_is_online($datetime) {
    return strtotime($datetime) > strtotime(gmdate('Y-m-d H:i:s', time() - 60));
}
...
function sb_set_typing($user_id, $conversation_id) {
    return sb_db_query('UPDATE sb_users SET typing = ' . $conversation_id . ',last_activity = "' . gmdate('Y-m-d H:i:s') . '" WHERE id = "' . $user_id . '"');
}
...
function sb_csv_conversations($conversation_id) {
    if ($conversation_id != -1) {
        $conversation = sb_db_get('SELECT *,AES_DECRYPT(sb_messages.message,"Gserw54sf533sS") as message FROM sb_messages WHERE conversation_id="' . $conversation_id . '"', false);
...
function sb_user_autodata($user_id) {
    ...
    if (!defined('SB_API')) {
        ...
        // IP and related data
        if (strlen($real_ip_address) > 6) {
            $settings['ip'] = [$real_ip_address, 'IP'];
...
function sb_update_bot($name = '', $profile_image = '') {
    global $LIVE_CHAT_HOST;
    $bot = sb_db_get('SELECT id, profile_image, first_name, last_name,app_user_type FROM sb_users WHERE user_type = "bot" LIMIT 1');
    if ($name == '') {
        $name = 'Bot';
    }
    if ($profile_image == '') {
    ...
    $settings = ['profile_image' => [$profile_image], 'first_name' => [$name], 'user_type' => ['bot'],'app_user_type' => ''];
    if ($bot == '') {
...
function sb_push_notification($title = '', $message = '', $icon = '', $interest = false, $conversation_id = false) {
    if (sb_get_multi_setting('push-notifications', 'push-notifications-active')) {
        ...
        $query = '{"interests":["' . $interest . '"],"web":{"notification":{"title":"' . str_replace('"', '', $title) . '","body":"' . str_replace('"', '', $message) . '","icon":"' . $icon . '","deep_link":"' . SB_URL . '/index.php' . $attributes . '","hide_notification_if_site_has_focus":true}}}';
