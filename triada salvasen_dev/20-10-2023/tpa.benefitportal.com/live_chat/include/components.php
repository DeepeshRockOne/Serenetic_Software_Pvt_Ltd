...
function sb_component_chat() {
    global $DEFAULT_CHAT_TITLE,$DEFAULT_CHAT_DESCRIPTION,$LIVE_CHAT_HOST;
    sb_js_global();
    sb_css();
...
?>
    <div class="sb-main sb-chat sb-no-conversations<?php echo $css ?>" style="display: none; transition: none;">
        <div class="sb-body">
            <div class="sb-scroll-area">
                <div class="sb-header sb-header-main" <?php if ($background != '') echo 'style="background-image: url(' . $background . ')"' ?>>
                    <?php if (!$disable_dashboard) echo '<i class="sb-icon-close sb-dashboard-btn close_conversation_reps"></i>'; ?>
                ...
                <div id="defaultWelcomeMessage" class="sb-default-message" >Hello! Thank you for visiting, how may we help you?</div>
                  <div id="conversationWaiting" style="display: none">
                        <div class="sb-default-message">
                            One moment please while we connect you to a live chat representative
                        </div> 
                        <div class="sb-waiting-message">
                            <div>Connecting to a live representative...</div>
        ...
        <div class="sb-chat-btn">
            ...
            <img class="sb-icon" alt="" src="<?= $LIVE_CHAT_HOST ?>/media/chat_icon.svg" />
            <img class="sb-close" alt="" src="<?= $LIVE_CHAT_HOST ?>/media/minimize.svg" />
        </div>
        <i class="sb-icon sb-icon-close sb-responsive-close-btn"></i>
        <audio id="sb-audio" preload="auto">
            <source src="<?= $LIVE_CHAT_HOST ?>/media/sound.mp3" type="audio/mpeg">
        </audio>
        <audio id="sb-audio-out" preload="auto">
            <source src="<?= $LIVE_CHAT_HOST ?>/media/sound-out.mp3" type="audio/mpeg">
        </audio>
...
function sb_component_editor($admin = false) { global $LIVE_CHAT_HOST; ?>
    <div class="sb-editor">
        ...
        <div class="sb-bar">
            <div class="sb-bar-icons">
                 <div class="sb-btn-saved-replies" data-sb-tooltip="<?php sb_e('Add a saved reply') ?>"><a class="btn quick_reply_btn">Quick Reply</a></div>
                <?php if ($admin || !sb_get_setting('disable-uploads')) echo '<div class="sb-btn-attachment attach_btn" data-sb-tooltip="' . sb_('Attach a file') . '"></div>'; ?> 
               
                <div class="sb-btn-emoji emoji_btn" data-sb-tooltip="<?php sb_e('Add an emoji') ?>"></div>
                <div class="sb-icon-send sb-submit submit_btn" data-sb-tooltip="<?php sb_e('Send message') ?>"></div>
            </div>
            
            <img class="sb-loader" src="<?= $LIVE_CHAT_HOST ?>/media/loader.svg" alt="" />
        </div>
...
function sb_profile_box() { global $LIVE_CHAT_HOST; ?>
    <div class="sb-profile-box sb-lightbox">
        <div class="sb-top-bar">
            <div class="sb-profile">
                <img src="<?= $LIVE_CHAT_HOST ?>/media/user.svg" />
                <span class="sb-name"></span>
            </div>
            <div>
                <?php /*<a class="sb-edit sb-btn sb-icon" data-button="toggle" data-hide="sb-profile-area" data-show="sb-edit-area">
...
function sb_profile_edit_box() { global $LIVE_CHAT_HOST;?>
...
function sb_login_box() { 
    global $DEFAULT_CHAT_SITE_NAME,$HOST,$DEFAULT_CHAT_LOGO,$LIVE_CHAT_HOST,$ADMIN_HOST;
    
    ?>
    <form class="sb sb-rich-login sb-admin-box">
        <div class="sb-info"></div>
        <div class="sb-top-bar">
            <img src="<?= $HOST.'/'.$DEFAULT_CHAT_LOGO ?>" />
            <div class="sb-title"><?php sb_e('Sign into') ?></div>
            <div class="sb-text"><?= 'To continue to '.$DEFAULT_CHAT_SITE_NAME ?></div>
        </div>
        <div class="sb-main">
 ...
 * ----------------------------------------------------------
 *
 * Display the form to install Live Chat
 *
 */

function sb_installation_box($error = false) {
    global $SB_LANGUAGE,$LIVE_CHAT_HOST;
...
?>
    <div class="sb-main sb-admin sb-admin-start">
        <form class="sb-intall sb-admin-box">
            ...
            <div class="sb-top-bar">
                <img src="<?= $LIVE_CHAT_HOST ?>/media/logo.svg" />
    ...
    <div class="sb-main <?php echo $logged ? 'sb-admin' : 'sb-admin-start' ?>" style="opacity: 0">
        ...
        <div class="sb-header">
            <div class="sb-admin-nav">
                <img src="<?= $LIVE_CHAT_HOST ?>/media/logo_white.svg" />
                <div>
                    ...
                    <?php if ($is_admin) {
                        echo '<a id="sb-users" style="display:none;"><span>' . sb_('Users') . '</span></a>';
            ...
            <div class="sb-admin-nav-right sb-menu-mobile">
                ...
                <div class="sb-desktop">
                    ...
                    <?php if ($is_admin) {
                        //echo '<a href="http://board.support/docs/" target="_blank" class="sb-docs"><i class="sb-icon-help"></i></a><a href="#" class="sb-version">' . SB_VERSION . '</a>'
                    } ?>
                </div>
                <div class="sb-mobile">
...
function sb_apps_area($apps) {
    ...
    for ($i = 1; $i < count($apps); $i++) { 
        $code .= '<div data-app="' . $apps[$i][1] . '">' . (defined($apps[$i][0]) ? '<i class="sb-icon-check"></i>' : '' ) . ' <img src="' . $LIVE_CHAT_HOST . '/media/apps/' . $apps[$i][1] . '.svg" /><h2>' . $apps[$i][2] . '</h2><p>' . sb_($apps[$i][3]) . '</p></div>';
    }
    echo $code . '</div></div>';
}
