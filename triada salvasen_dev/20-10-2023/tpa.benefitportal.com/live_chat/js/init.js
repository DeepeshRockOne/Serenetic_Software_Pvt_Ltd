...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        var url = url_full.substr(0, url_full.lastIndexOf('live_chat'));
        var xhr = sbCorsRequest('GET', url + 'live_chat/include/init.php' + ('lang' in parameters ? '?lang=' + parameters['lang'] : '') + ('mode' in parameters ? ('lang' in parameters ? '&' : '?') + 'mode=' + parameters['mode'] : ''));
        ...
        if (!xhr) {
            ...
            xhr.onload = function () {
                ...
                sbCreateStyle(url + 'live_chat/css/min/' + (tickets ? 'tickets' : 'main') + '.min.css', head);
                sbCreateScript(url + 'live_chat/js/main.js', head);

                if (tickets) {
                    sbCreateScript(url + 'live_chat/apps/tickets/tickets.js', head);
                }
            };
    ...
    function getScriptUrl(a) {
        ...
        for (var i = 0; i < b.length; i++) {
            if (b[i].src.indexOf('/live_chat/js/init.js') > -1 || b[i].src.indexOf('/live_chat/js/min/init.min.js') > -1) {
                return b[i].src;
            }
        }
        return '';
