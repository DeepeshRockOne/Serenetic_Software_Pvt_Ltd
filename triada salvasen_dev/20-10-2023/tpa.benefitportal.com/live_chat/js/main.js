    ...
    */
    ...
    class SBMessage {
        ...
        getCode() {
            ...
            let media_code = '';
            let thumb = admin || (agent && !CHAT_SETTINGS['hide-agents-thumb']) || (!agent && CHAT_SETTINGS['display-users-thumb']) ? `<div class="sb-thumb"><img src="${this.details['profile_image']}"></div>` : '';
            let css = (!agent ? 'sb-right' : 'sb-left') + (thumb == '' ? '' : ' sb-thumb-active');
            let type = '';
            ...
            // Message creation
            //karan
            $("#defaultWelcomeMessage").hide();
            return `<div data-id="${this.details['id']}" class="${css}" ${type}>${thumb}<div class="sb-cnt"><div class="sb-message${media_code != '' && message == '' ? ' sb-message-media' : ''}"><div class='chat_person'><strong>${this.details['user_type'] == 'admin' ? this.details['first_name'] : this.details['full_name']}  </strong></div>${message}${media_code}</div>${attachments_code}<div class="sb-time">${SBF.beautifyTime(this.details['creation_time'], true)}</div></div>${admin_menu}</div>`;
        }
    ...
    var SBChat = {
            ...
            if (!this.initialized) {
                ...
                if (!admin && !tickets) {
                    ...
                    //if (!mobile && window.innerHeight < 760) {
                        ...
                        //$(main).find(' > .sb-body').css('max-height', '300px');
                    }
                }
        ...
        update: function () {
            if (this.conversation != false && !this.is_busy_update) {
                ...
                let is_chat_waiting = this.conversation.details.is_chat_waiting;
                let is_update = false;
                SBF.ajax({
                    ...
                    if (this.conversation != false) {
                        if (Array.isArray(response) && count > 0 && (!last_message || last_message.id != response[count - 1]['id'] || last_message.get('message') != response[count - 1]['message'])) {
                            ...
                            if (!admin && this.chat_open && SBF.isAgent(user_type) && user_type != 'bot' && last_message.get('conversation_status_code') != 4) {
                ...
                if(is_chat_waiting=='N'){
                    SBF.ajax({
                        function: 'is-conversation-assigned',
                        conversation_id: this.conversation.id,
                    }, (response) => {
                        if(response.is_chat_waiting == 'A'){
                            this.conversation.details.is_chat_waiting = response.is_chat_waiting;
                            $("#conversationWaiting").hide();
                        }else{
                            $("#conversationWaiting").show();
                        }
                    });
                }
        ...
        headerAgent: function () {
            if (!admin && !tickets && !this.dashboard && this.conversation != false && (this.agent_id == -1 || (this.conversation.getLastMessage() != false && SBF.isAgent(this.conversation.getLastMessage().get('user_type')) && this.conversation.getLastMessage().get('user_id') != this.agent_id))) {
                ...
                if (agent != false) {
                    ...
                    this.headerReset();
                    $(chat_header).addClass('sb-header-agent').attr('data-agent-id', this.agent_id).html(`<div class="sb-profile" ><div class="sb-profile-thumb"><img src="${DEFAULT_CHAT_HEADER_IMAGE}" /></div><div class="w-100"><span class="sb-name">Chatting with ${agent['first_name']}</span><span class="sb-status"></span></div><div class="sb-remove"><a href="javascript:void(0);" class="btn_close_conversation" data-user-id="`+this.conversation.details.user_id+`" data-conversation-id="`+this.conversation.details.id+`" title="Close Conversation"><img  alt="" src="${SB_URL}/media/close.svg" /></a></div></div>`);
            ...
            if (this.conversation != false) {
                for (var i = this.conversation.messages.length - 1; i > -1; i--) {
                    ...
                    if (SBF.isAgent(user_type) && (bot || (!bot && user_type != 'bot'))) {
                        agent = { user_id: this.conversation.messages[i].get('user_id'),first_name: this.conversation.messages[i].get('first_name'), full_name: this.conversation.messages[i].get('full_name'), profile_image: this.conversation.messages[i].get('profile_image') };
                        break;
                    }
                }
            }
        ...
        scrollBottom: function (top = true) {
            top = false;
            setTimeout(() => {
                ...
                if (action == 'check' && user_id != -1 && user_id != bot_id && (this.agent_online || (admin && this.user_online))) {
                    ...
                    if (!admin) {
                        if (start) {
                            ...
                            $(chat_status).removeClass('sb-status-typing').html('');
                        }
                    }
    ...
    var SBRichMessages = {
        ...
        submit: function (area, type, element) {
            if (!admin && !$(element).sbLoading() && !this.is_busy) {
                ...
                let user_settings = activeUser() == false ? { 'profile_image': ['', ''], 'first_name': ['', ''], 'last_name': ['', ''], 'email': ['', ''], 'password': ['', ''], 'user_type': ['', ''] } : { 'profile_image': [activeUser().image, ''], 'first_name': [activeUser().get('first_name'), ''], 'last_name': [activeUser().get('last_name'), ''], 'email': [activeUser().get('email'), ''], 'password': ['', ''], 'user_type': ['', ''], 'app_user_type':[activeUser().get('app_user_type'),''] };
                let settings = {};
                let input = element;
    ...
    */
    ...
    $(document).ready(function () {
        ...
        $(main).on('click', '.sb-chat-btn,.sb-responsive-close-btn', function () {
            let tmp_chat_open = $(main).sbActive();
            tmp_chat_open = tmp_chat_open == true?false:true;
            localStorage.setItem("chatOpen",tmp_chat_open);
            SBChat.open();
        });

        // Show the dashboard
        $(main).on('click', '.sb-dashboard-btn', function () {
            if($(this).hasClass('close_conversation_reps')){
                var conversationID = $('.sb-user-conversations li').attr('data-conversation-id');
                var userID = $('.sb-user-conversations li .sb-conversation-item').attr('data-user-id');
