...
if($type=='Agent' && $customer_id!=''){
	$res_name = $pdo->selectOne("SELECT concat(fname,' ',lname) as name,DATE_FORMAT(current_time(), '%a., %b. %d, %Y @  %r') as date, rep_id as display_id,now() as cdate from customer where md5(id) = :id",array("id"=>$customer_id));
}
