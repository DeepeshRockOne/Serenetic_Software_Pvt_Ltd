...
$member_id = $_GET['member_id'];
    ...
    $agentSql = "SELECT c.id as agentId,c.rep_id as agentDispId,c.email as agentEmail,IF(c.type='Group',c.business_name,CONCAT(c.fname,' ',c.lname)) as agentName ,agent_coded_id
                ...
                WHERE c.type='".$resMember['sponsor_type']."' AND c.status='Active' AND c.is_deleted='N' AND c.id!=:sponsorId AND ((c.upline_sponsors LIKE('%,".$agent_id.",%')) OR c.id='".$agent_id."')";
      ...
      $resAgent = $pdo->select($selAgent);
        if(!empty($resAgent)){
          ...
          foreach($resAgent as $key => $agent){
            ...
            $link_url = $agent['type'] == 'Customer' ? 'members_details.php?id='.md5($agent['id']) : 'agent_detail_v1.php?id='.md5($agent['id']);
            ...
            if($agent['id'] != 1){
              $sponsorArr['child_node'.$agent['id']] = array(
                ...
                "innerHTML" => "
                    <a href='".$link_url."'>
                      <h4>" . stripslashes($business_name) . "&nbsp;&nbsp;&nbsp;&nbsp;".$pr_type."</h4>
                    </a>
                    <p> ".$agent['rep_id']." </p>
              ...
              $sponsorparent = array(
                  ...
                  <a href='javascript:void(0);'>
                    <h4>" . stripslashes($business_name) . "</h4>
                  </a>
                  <p> ".$agent['rep_id']." </p>
                  <h5 " . (strtolower($agent['status']) != "active" ? "class='text-inactive'" : "") . ">" . strtoupper($coded_level) . "</h5>
                  <p>".checkIsset($icons)."</p>",
