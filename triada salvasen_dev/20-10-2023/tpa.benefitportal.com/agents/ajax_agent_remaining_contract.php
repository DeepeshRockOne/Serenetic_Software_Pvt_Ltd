...
$is_force = !empty($_POST["is_force"]) ? $_POST["is_force"] : 0;
$REAL_IP_ADDRESS = get_real_ipaddress();
if ($is_force) {
	$is_draft = 1;
}

//when user have not click on draft, and temporary approve then first step is compulsary
if ($is_force && $_POST["is_draft"] != 1 && $step == 1) {
	$is_draft = 0;
}
$checkLink_query = "SELECT c.id as id ,email,cell_phone,rep_id,sponsor_id,public_name,public_email,public_phone,user_name,display_in_member,is_branding,brand_icon,status,account_type,company_name,company_address,company_address_2,company_city,company_state,company_zip,w9_pdf,address,address_2,fname,tax_id,lname,city,state,zip,birth_date,type,npn,is_contract_approved,TIMESTAMPDIFF(HOUR,invite_at,now()) as difference,AES_DECRYPT(ssn,'" . $CREDIT_CARD_ENC_KEY . "') as dssn FROM customer c LEFT JOIN customer_settings cs on(cs.customer_id=c.id) WHERE rep_id=:invite_key AND (status in ('Pending Approval','Pending Contract','Pending Documentation'))";
...
// GENRAL VARIABLE
	$step_to_validate = 0;
	$cust_id = $hdn_cust_id = $agent_res["id"];
	if ($step == 1) {
		$admin_name = $_POST["admin_name"];
		$admin_phone = phoneReplace($_POST["admin_phone"]);
		...
		if (!$is_draft || $username!='') {
			...
			if (!$validate->getError('username')) {
				if (!isValidUserName($username, $cust_id)) {
	...
	if ($step == 2) {
		if ($agent_res["user_name"] == "") {
			$validate->setError('first_step', 'Please complete first step to continue..');
		} else {
			$fname = $_POST['fname'];
			$lname = $_POST['lname'];
			...
			if(!empty($license_expiry) && !$is_draft){
			// $alreadyExistsStateArray = array();
			...
			if (!in_array($_SESSION['agents']['agent_coded_level'], array("LOA"))) {
				...
				if (!$is_draft && empty($entered_account_number)) {
					...
					$validate->digit(array('required' => true, 'field' => 'bank_account_number', 'value' => $bank_account_number,'min'=>5,'max'=>17), array('required' => 'Account number is required', 'invalid' => "Enter valid Account number"));
				}
	...
	if ($step == 3) {
		if ($agent_res["user_name"] == "") {
			...
			// if ($agent_res["status"] == "Pending Approval") {
			// 	$validate->setError('final_contract_error', 'Your contract still not approved! Please try again later.');
			// } else {
				//Agreement Tab
	...
	if ($validate->isValid()) {
		...
		$s_where_cust_id = array(':id' => $hdn_cust_id);
		$s_res_enroll = $res_enroll = $pdo->selectOne($s_enroll_sql, $s_where_cust_id);

		if ($s_res_enroll['sponsor_id'] > 0) {
			$sponsor_id = $s_res_enroll['sponsor_id'];
		...
		$_SESSION['agent_tmp_member']['id'] = $res_enroll['id'];
		if ($step == 1) {
		...
		if ($step == 2) {
			...
			if (!$is_draft) {
				...
				// $description = array('account_type' => $account_type, 'license_array' => $license_arr, 'e_o_amount' => $e_o_amount, 'e_o_expiration' => date('Y-m-d', strtotime($e_o_expiration)), 'parent_agent_trigger' => $trigger_id2, 'admin_trigger' => $trigger_id);
				// $description = json_encode($description);
				// activity_feed(3, $_SESSION['agents']['id'], 'Agent', $_SESSION['agents']['id'], 'customer', 'Agent Submitted Documentation', $_SESSION['agents']['fname'], $_SESSION['agents']['lname'], $description);
				//trigger sent to parent agent when document is submit code end

				//ticket generate
				// $ticketDesc = "<p>A new agent, " . $_SESSION["agents"]["fname"] . " " . $_SESSION["agents"]["lname"] . ", has completed their agent enrollment and submitted it for review.  They are now ready to be verified by admin.</p>
				// <p>Please login to your admin account to manage this agent or for quick access you can click here:</p>
				// <a href='" . $rep_params['link'] . "'>" . $rep_params['link'] . "</a>
				// ";
				// createNewTicket("New Agent Enrollment Review Request", $ticketDesc, $_SESSION['agents']['id'], $_SESSION['agents']['type'], $_SESSION['agents']['email'], 2);
...
function agent_profile_activity($contract_activity,$is_draft){
	...
	// activity_feed(3, $_SESSION['agents']['id'], 'Agent', $_SESSION['agents']['id'], 'customer', 'Agent Submitted Documentation', $_SESSION['agents']['fname'], $_SESSION['agents']['lname'], $desc);
}
...
$response["is_draft"] = !empty($_POST["is_draft"]) ? $_POST["is_draft"] : 0;
$response["is_force"] = !empty($_POST["is_force"]) ? $_POST["is_force"] : 0;
