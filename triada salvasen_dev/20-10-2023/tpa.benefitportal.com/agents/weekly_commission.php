...
$options = array(
	...
	'url' => 'weekly_commission.php?' . $query_string,
	'db_handle' => $pdo->dbh,
...
if ($is_ajaxed) {
	try {
		$sel_sql = "SELECT 
             ...
             WHERE cs.commission_duration='weekly' 
             AND cs.customer_id = :agentId 
             AND cs.status IN ('Approved','Pending') AND cs.is_deleted = 'N' " . $incr . "
             GROUP BY t.id,cs.order_id,cs.order_detail_id
	...
	include_once 'tmpl/weekly_commission.inc.php';
	exit;
}
$header_commission_res = get_pay_period_commission_totals($_SESSION['agents']['id']);
$template = 'weekly_commission.inc.php';
