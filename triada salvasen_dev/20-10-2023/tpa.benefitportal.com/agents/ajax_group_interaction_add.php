...
include_once 'layout/start.inc.php';
    ...
    if(isset($_POST['type']) == 'delete' && !empty($_POST['interaction_detail_id'])){
        ...
        $group_name=$pdo->selectOne("SELECT CONCAT(fname,' ',lname) as name,rep_id,id from customer where is_deleted='N' and md5(id)=:id",array(":id"=>$group_id));
        $description =array();
        $description['ac_message'] = array(
            ...
            'ac_message_1' =>'  Deleted Interaction on '.$group_name['name'].' (',
            'ac_red_2'=>array(
        ...
        activity_feed(3,$_SESSION['agents']['id'],'Agent',$group_name['id'],'Group','Interaction Deleted.',"","",$desc);
    ...
    if(!empty($create_etickets)){
        ...
        if(empty($category_id)){
            $validate->setError("category_id","Please select any Category.");
        }
        if(!empty($category_id) && empty($assigne_admins)){
    ...
    if($validate->isValid()){
            ...
            if(!empty($interaction_detail) && $interaction_detail['id'] > 0){
                ...
                $update_activity['ac_message'] = array(
                    ...
                    'ac_message_1' =>'  Updated Interaction on '.$group_name.' (',
                    'ac_red_2'=>array(
                ...
                activity_feed(3,$_SESSION['agents']['id'],'Agent',$group_id, 'Group', 'Interaction Updated.',"","",$desc);
                ...
                $description['ac_message'] = array(
                    ...
                    'ac_message_1' =>'  created Interaction on '.$group_name.' (',
                    'ac_red_2'=>array(
                      'href'=> $ADMIN_HOST.'/groups_details.php?id='.md5($group_id),
                      'title'=> $group_repid,
                    ),
                ...
                $desc=json_encode($description);
                activity_feed(3,$_SESSION['agents']['id'], 'Agent' , $group_id, 'Group', 'Interaction Created.',"","",$desc);

                if(!empty($create_etickets)){
                    include_once __DIR__ . '/../includes/function.class.php';
                    $functionList = new functionsList();
                    $sessionArr['agents'] = $_SESSION['agents'];
                    $eticket_description .= '<p>'.$_SESSION['agents']['rep_id'].' - '.$_SESSION['agents']['fname'].' '.$_SESSION['agents']['lname'].'</p>';
                    $real_ip_address = !empty($REAL_IP_ADDRESS['original_ip_address']) ? $REAL_IP_ADDRESS['original_ip_address']:$REAL_IP_ADDRESS['ip_address'];
                   $ticketCreate=$functionList->createNewTicket($sessionArr,$category_id,$subject,$assigne_admins,$eticket_description,$group_id,'Group',$real_ip_address,array(),'notes');
                   if(!empty($ticketCreate['ticket_id'])){
                    $tid_where = array(
                            "clause" => "id=:id",
                            "params" => array(":id"=>$inserted_id)
                        );
                        $pdo->update("interaction_detail",array("e_ticket_id"=>$ticketCreate['ticket_id']),$tid_where);
                    }
                    $response['status'] = 'success';
                }
            }
