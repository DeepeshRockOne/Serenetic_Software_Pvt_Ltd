...
include_once dirname(__FILE__) . '/layout/start.inc.php';
    ...
    if(!empty($_GET['edit']) && $_GET['edit'] == md5('edit') && !empty($_GET['sa_id'])){
        $sub_agent = $pdo->selectOne("SELECT acl.id as id,acl.level AS name,sa.id as sid,sa.fname,sa.lname,sa.email,sa.access_type,sa.feature_access,sa.status,sa.passcode,sa.is_2fa,sa.send_otp_via,sa.via_email,sa.via_sms,sa.is_ip_restriction,sa.allowed_ip
        from sub_agent sa
    ...
    function activity_feed_account_manager($agent_id,$msg='',$extra = array()){
        ...
        activity_feed(3,$agent_name['id'], 'Agent',$agent_name['id'], 'Agent Account Manager','Agent Account Manager',"","",$desc);
    }
