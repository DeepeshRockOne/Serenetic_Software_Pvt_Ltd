...
if($is_ajaxed_customers){
	$sel_sql = "SELECT c.rep_id,
                  ...
                  WHERE c.type='Customer'  AND c.is_deleted = 'N' AND c.status NOT IN('Customer Abandon','Pending Quote','Pending Quotes','Pending Validation','Post Payment','Pending') AND 
                  c.sponsor_id IN(".implode(',',$sponsor_agents).")
                  " . $incr . " 
                  GROUP BY c.id 
