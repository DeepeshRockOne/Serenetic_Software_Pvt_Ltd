    ...
    <div class="table-responsive">
        <table class="<?= $table_class ?>">
            ...
            <tr class="data-head">
                ...
                <th>
                    <a href="javascript:void(0);" data-column="s.fname"
                       data-direction="<?php echo $SortBy == 's.fname' ? ($currSortDirection == 'ASC' ? 'DESC' : 'ASC') : 'ASC'; ?>">Enrolling Agent/ID</a>
                </th>
                <th width="15%">
                    <a href="javascript:void(0);" data-column="l.lead_type" data-direction="<?php echo $SortBy == 'l.lead_type' ? ($currSortDirection == 'ASC' ? 'DESC' : 'ASC') : 'ASC'; ?>">Lead Type</a></th>
                <th width="20%">
                    <a href="javascript:void(0);" data-column="l.opt_in_type" data-direction="<?php echo $SortBy == 'l.opt_in_type' ? ($currSortDirection == 'ASC' ? 'DESC' : 'ASC') : 'ASC'; ?>">Lead Tag</a>
                </th>
                <th>
                ...
                <?php foreach ($fetch_rows as $key => $rows) { ?>
                    <tr>
                        ...
                        <td class="icons text-left">
                            <a href="lead_details.php?id=<?= $rows['id'] ?>" target="_blank" data-toggle="tooltip" data-trigger="hover" title="Details">
                                <i class="fa fa-eye"></i>
                            </a>
                            <?php 
                                $allow_edit = true; 
                                if($rows['sponsor_id'] != $agent_id){
                                    if(!empty($resSubAgent["id"]) && checkIsset($rows["agent_coded_level"]) == "LOA"){
                                        $allow_edit = true;    
                                    }else{
                                        $allow_edit = false;
                                    }
                                }
                                if($rows['lead_type'] == "Member" && $rows['customer_status'] != "Post Payment" && in_array($rows['status'],array("New","Working","Open","Abandoned")) && $allow_edit)  {
                                    ?>
                                    <a href="member_enrollment.php?lead_id=<?=$rows['id'];?>" data-toggle="tooltip" data-trigger="hover" title="Member Enrollment"><img src="<?=$HOST?>/images/icons/lead_aae.svg" height="20px"></a>
                                    <?php
                                }
                            ?>
                            <?php 
                                if($rows['lead_type'] == "Agent/Group" && in_array($rows['status'],array("New","Working","Open")) && empty($rows['customer_id']))  {
                                    ?>
                                    <a href="javascript:void(0)" data-id="<?=$rows['id'];?>" data-toggle="tooltip" data-trigger="hover" title="Invite" class="btn_invite_agent_group"><i class="fa fa-user-plus"></i></a>
                                    <?php
                                }
...
<?php } else { ?>
    <div class="container m-t-30">
        <div class="panel panel-default panel-block panel-title-block">
            <form id="frm_search" action="lead_listing.php" method="GET" class="theme-form">
                    ...
                    <div class="panel-wrapper collapse in">
                        <div class="panel-body theme-form">
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group ">
                                        <select class="se_multiple_select listing_search" name="lead_type" id="lead_type" multiple="multiple">
                            ...
                            <div class="panel-footer clearfix">
                                ...
                                <button type="button" class="btn btn-info btn-outline" onClick="window.location = 'lead_listing.php'">
