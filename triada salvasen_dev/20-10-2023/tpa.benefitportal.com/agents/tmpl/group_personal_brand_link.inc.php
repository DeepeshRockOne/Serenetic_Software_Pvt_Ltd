<form action="" name="personal_brand_frm" id="personal_brand_frm" method="POST">
     ...
     <div class="row">
       <div class="col-sm-12">
         <div class="form-group height_auto">
           <label class="mn label-input"><input type="checkbox" id="display_in_member" name="display_in_member" value="Y" <?= $display_in_member == 'Y' ? 'checked' : '' ?>> Check this box if you do not wish to display your name, email, and phone as a point of contact inside the member portal.</label>
         </div>
       </div>
       <div class="col-sm-6">
       ...
       <div class="col-sm-6">
         <div class="form-group">
           <div class="input-group">
            ...
            <input  type="text" class="form-control <?= !empty($username) ? 'has-value' : '' ?>"  placeholder="<?= $DEFAULT_SITE_URL ?>/company001" name="username" id="username" class="form-control" value="<?= $username ?>">
    ...
    <p class="agp_md_title">Branding Customization</p>
      <div class="form-group height_auto">
        <label class="mn label-input"><input type="checkbox" id="is_branding" name="is_branding" <?=$is_branding == 'Y' ? 'checked' : ''?> value="Y">
...
<script type="text/javascript">
             ...
             var remaingContractDropzone = new Dropzone("#enrollment_profile .profile-dropzone", {
                 ...
                 init: function () {
                     ...
                     if (this.options.customErrorHandlingCode == 200) {
                         ...
                         $.ajax({
                             url: '<?=$AGENT_HOST?>/ajax_update_group_business_picture.php?id=' + $group_id + '',
    ...
    $(document).on('focusin click keyup', '#username', function() {
        ...
        if (user_name.match(pattern) && user_name.length > 3 && user_name.length < 21) {
            $.ajax({
                url: "check_already_username.php",
                data: {
                    username: user_name,
                    email: user_email,
                    agent_id: group_id
