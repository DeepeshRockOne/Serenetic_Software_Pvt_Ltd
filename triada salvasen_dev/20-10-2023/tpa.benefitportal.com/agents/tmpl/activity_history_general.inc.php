...
<div id="activity_inline_colorbox_div" style="display: none;">
  ...
  <ul class="timeline left-timeline lead_activity steamline">
      ...
      foreach ($fetch_rows as $row) {?>
        <li class="timeline-inverted" data-activity_feed_id="<?=$row['id']?>"  data-user_type="<?=$row['user_type']?>" data-action="<?=$row['entity_action']?>">
            ...
            <div class="timeline-body">
                ...
                <?php
                    if(!empty($content))
                    {
                        ...
                        $content = str_replace_deep(array($ADMIN_HOST,'agent_detail_v1.php'),array($AGENT_HOST,'javascript:void(0);'),$content);
                                    ...
                                    <?php foreach($value as $ekey => $evalue){ ?>
                                        <?php if(preg_match('/ac_red_\d/',$ekey)){?>
                                            <?php if(!empty($evalue['href'])) { ?>
                                                <a href="<?= $evalue['href'] ?>" class="text-red"><?= $evalue['title'] ?></a>
                                            <?php }else{ ?>
                            ...
                            <?php }else if(preg_match('/ac_description_link_\d/',$key) || $key == 'ac_description_link'){ ?>
                                <p>
                                    <?php foreach ($value as $desHeading => $descValue) { ?>
                                        <?= $desHeading ?> <a href="#javascript:void(0)" data-desc="<?= $descValue['data-desc'] ?>" class="<?= $descValue['class'] ?>" data-encode="<?= checkIsset($descValue['data-encode']) ?>"><?= $descValue['title'] ?></a>
                                    <?php } ?>
                                </p>
