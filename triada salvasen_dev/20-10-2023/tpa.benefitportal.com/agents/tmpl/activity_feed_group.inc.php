...
<p class="agp_md_title">Activity History</p>
  <form action="ajax_activity_feed_group.php?id=<?php echo $_REQUEST['id']; ?>&for=Group" method="POST" id="acc_feed_search_form" class="theme-form">
  <div class="m-b-10" >
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $(".lead_activity li.timeline-inverted").filter(function() {
          ...
          var keyword_search = $("#acc_feed_search_form #keyword_search").val();
                ...
                $('#ajax_loader').show();
                    ...
                    if (flag == 0 && from_limit != 'undefined') {
                        ...
                        $('#ajax_loader').show();
                        $.ajax({
                            url: 'ajax_activity_feed_group.php',
                            type: 'POST',
                            data: {
  ...
  function ajax_load_activity_feed() {
    ...
    $.ajax({
      url: "ajax_activity_feed_group.php",
      data: {id: '<?= $_REQUEST['id'] ?>',user_type:'Group',activity_type:activity_type,keyword_search:keyword_search,acc_his_custom_date:acc_his_custom_date,acc_his_fromdate:acc_his_fromdate,acc_his_todate:acc_his_todate,for:'Group'},
      type: 'POST',
      success: function(res) {
        $('#ajax_loader').hide();
  ...
  function ajax_reload_activity_feed() {
    ...
    $.ajax({
      url: "ajax_activity_feed_group.php",
      data: {id: '<?= $_REQUEST['id'] ?>',user_type:'Group',for:'Group'},
      type: 'POST',
      success: function(res) {
        $('#ajax_loader').hide();
        $("#sec_acct_his").html(res);
