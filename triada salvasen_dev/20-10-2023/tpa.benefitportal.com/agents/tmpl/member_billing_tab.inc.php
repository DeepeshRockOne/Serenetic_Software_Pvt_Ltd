            ...
            <tr>
               ...
               <td class="text-center">
                  <?php if($has_full_access == true) { ?>
                     <a href="javasacript:void(0);"><input type="radio" data-bill_id="<?=$info['id']?>" class="change_default" name="is_default" <?=$info['is_default'] == 'Y' ? 'checked="checked"' : ''?> value="Y" id="is_default_<?=$info['id']?>"></a>
                  <?php } else { ?>
                     <?=$info['is_default'] == 'Y' ? 'Yes' : 'No'?>
                  <?php } ?>
               </td>
               <td class="icons">
                  <?php if($has_full_access == true) { ?>
                  <a href="<?=$HOST?>/add_billing_profile.php?location=agent&id=<?=$customer_id?>&action=Edit&bill_id=<?=$info['id']?>" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="Edit" class="edit_billing_profile"><i class="fa fa-edit" aria-hidden="true"></i></a>
                  <?php if($info['is_default'] !='Y' && count($billing_info) > 1) { 
                     //OP29-738 Remove ability for Agent to do delete billing method if no other exists
                  ?>
...
<div class="text-right">
   <?php if($has_full_access == true) { ?>
   <a href="<?=$HOST?>/add_billing_profile.php?location=agent&id=<?=$customer_id?>&action=Add" class="add_billing_profile btn btn-action">+ Billing Method</a>
   <?php } ?>
</div>
<hr>
   ...
   $(document).on('click','.delete_billing_profile',function(e){
      var $bill_id = $(this).attr('data-bill_id');
         swal({
            ...
            $.ajax({
               url:'<?=$HOST?>/add_billing_profile.php?location=agent',
               method : 'POST',
               data : {
                  id:'<?=$customer_id?>',
                  bill_id:$bill_id,
