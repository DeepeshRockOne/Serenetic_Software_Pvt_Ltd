<div id="open_account_search">
    <form action="ajax_activity_feed_lead.php?id=<?php echo $_REQUEST['id']; ?>&for=Lead" method="POST"
          id="acc_feed_search_form" class="theme-form">
        <div class="m-b-10" >
        ...
        <div id="search_activty" class="flex_row activity_filter">
            <div class="form-group">
                ...
                <input type="hidden" id="user_type" name="user_type" value="<?=$_GET['user_type']?>">
                <input type="hidden" id="customer_id" name="customer_id" value="<?=$_GET['customer_id']?>">
                <label>Keyword Search</label>
            </div>
            <div class="form-group min-w175">
                <select class="form-control " name="acc_his_custom_date"
                        id="acc_his_custom_date">
...
<script type="text/javascript">
                ...
                if (is_all_activity_popup == 'Y') {
                    ...
                    if (from_limit != 0 && total_rows > 0) {
                        ...
                        if (flag == 0 && from_limit != 'undefined') {
                            ...
                            $('#ajax_loader').show();
                            $.ajax({
                                url: 'ajax_activity_feed_lead.php',
                                type: 'POST',
                                data: {
    ...
    function ajax_reload_activity_feed() {
        ...
        $.ajax({
            url: "ajax_activity_feed_lead.php",
            data: {id: '<?= $_REQUEST['id'] ?>', user_type: user_type, customer_id: customer_id, for: 'Lead'},
            type: 'POST',
            success: function (res) {
                $('#ajax_loader').hide();
                $("#sec_acct_his").html(res);
