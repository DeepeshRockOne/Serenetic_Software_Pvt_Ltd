               ...
               <tr>
                 ...
                 <td class="icons">
                    <?php if(check_is_card_exist($row['p_id'])){ ?>
                      <a href="<?=$HOST?>/id_card_popup.php?ws_id=<?=md5($row['ws_id'])?>&user_type=Agent&user_id=<?=$_SESSION['agents']['id']?>" class="id_card_popup" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="ID Card"><i class="fa fa-address-card-o" aria-hidden="true"></i></a>
                    <?php } ?>
                    <a href="member_product_detail.php?customer_id=<?=$row['customer_id']?>&product_id=<?=$row['p_id']?>&plan_id=<?=$row['matrix_id']?>" class="member_product_detail" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="Product Details"><i class="fa fa-info-circle" aria-hidden="true"></i></i></a>
                    ...
                    <a href="../policy_document.php?userType=Agent&customer_id=<?=md5($row['customer_id'])?>&ws_id=<?=md5($row['ws_id'])?>" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="Plan Doc"><i class="fa fa-download" aria-hidden="true"></i></a>
                  ...
                  <div class="row table-footer-row">
                     <div class="col-sm-12">
                        ...
                        <div class="pull-right">
                          ...
                          <?php } ?> 
                           <!-- <?php if(agent_has_member_access($member_id) == true) { ?>
                            <a href="<?=$HOST?>/term_multiple_products.php?location=agent&member_id=<?=$member_id?>" class="btn btn-info term_multiple_products">Multi-Term</a>
                            <?php } ?> -->
                        </div>
                     </div>
                  </div>
...
$(document).ready(function() {
    ...
    $(document).on('change','.product_change_action', function(e) {
        ...
        if(new_prd_id != old_prd_id){
            $.colorbox({
                href: '<?=$HOST?>/benefit_tier_change.php?location=agent&action=policy_change&new_prd_id=' + new_prd_id + '&ws_id=' + ws_id,
                iframe: true,
                height: '500px',
                width: '650px',
                open: true,
    ...
    $(document).on('click','.btn_product_change', function(e) {
         var ws_id = $(this).data('ws_id');
        $.colorbox({
            href: '<?=$HOST?>/benefit_tier_change.php?location=agent&action=policy_change&show_life_event=Y&ws_id=' + ws_id,
            iframe: true,
            height: '500px',
            width: '650px',
            open: true,
    ...
    $.ajax({
      ...
      success: function(res) {
         ...
         $(document).on('click', '.btn_edit_tier_change_date', function (e) {
            ...
            var ws_id = $(this).attr('data-ws_id');
            $.colorbox({
                "href": '<?=$HOST?>/change_benefit_tier_popup.php?location=agent&customer_id=' + customer_id + '&ws_id=' + ws_id,
                height: '400px',
                width: '500px',
                iframe: true,
                onClosed: function () {
...
$(document).on('click','.remove_proof_of_coverage',function(){
  ...
  parent.swal({
        ...
        $('#ajax_loader').show();
        $.ajax({
          url: '<?=$HOST?>/ajax_delete_coverage_doc.php?location=agent',
          type: 'POST',
          data: {ce_id:id},
          dataType: 'JSON',
          success: function(res) {
...
$(document).on('click','.dependent_delete',function(){
  ...
  parent.swal({
        ...
        $('#ajax_loader').show();
        $.ajax({
          url: '<?=$HOST?>/ajax_delete_dependent.php?location=agent',
          type: 'POST',
          data: {id:id},
          dataType: 'JSON',
          success: function(res) {
...
$(document).on('click','#btn_cancel_termination',function(){
    ...
    parent.swal({
        ...
        $('#ajax_loader').show();
        $.ajax({
          url: '<?=$HOST?>/ajax_cancel_termination_date.php?location=agent',
          type: 'POST',
          data: {ws_id:ws_id},
          dataType: 'JSON',
          success: function(res) {
...
$(document).on('change','.select_term_reason', function(e) {
  ...
  parent.swal({
        ...
        $('#ajax_loader').show();
        $.ajax({
          url: '<?=$HOST?>/ajax_change_term_reason.php?location=agent',
          type: 'POST',
...
existing_dependant_popup = function (customer_id,product_id,web_id,plan_id) {
    window.parent.$.colorbox({
        href: '<?=$HOST?>/benefit_tier_change.php?location=agent&new_plan_type=' + plan_id + '&ws_id=' + web_id,
        iframe: true,
        height: '500px',
        width: '650px',
        open: true,
