<div id="smarteapp_vue" class="panel panel-default ">
	<div class="panel-heading">
		<div class="panel-title">
			<h4 class="mn">Personal Branding - <span class="fw300"><?=!empty($_SESSION['agents']['public_name']) ? $_SESSION['agents']['public_name'] : $_SESSION['agents']['fname'].' '.$_SESSION['agents']['lname'];?></span></h4>
		</div>
	</div>
	<form action="personal_branding.php" role="form" method="post" class="theme-form " name="form_personal_branding" id="form_personal_branding" enctype="multipart/form-data">
        ...
        <div class="panel-body">
            ...
            <div class="row color_selection_label">
                <div class="col-xs-4">
                    <label for="default_color" class="<?=$tmp_theme_color == "skin-default" ? "active" : ""?>" >
                        ...
                        <div class="text-center">
                            <img src="<?=$AGENT_HOST?>/images/theme/default-color.png<?=$cache?>">
                        </div>
                        <span>Default</span>
                    </label>
                </div>
                <div class="col-xs-4">
                    <label for="dark_color" class="<?=$tmp_theme_color == "skin-dark" ? "active" : ""?>">
                        ...
                        <div class="text-center">
                            <img src="<?=$AGENT_HOST?>/images/theme/dark-color.png<?=$cache?>">
                        </div>
                        <span>Dark</span>
                    </label>
                </div>
                <div class="col-xs-4">
                    <label for="light_color" class="<?=$tmp_theme_color == "skin-light" ? "active" : ""?>">
                        ...
                        <div class="text-center">
                            <img src="<?=$AGENT_HOST?>/images/theme/light-color.png<?=$cache?>">
                        </div>
                        <span>Light</span>
                    </label>
                </div>
            ...
            <div class="theme-screenshot-wrap">
                <?php 
                    if($tmp_theme_color == "skin-dark"){
                        $theme_img = $AGENT_HOST . "/images/theme/dark-theme.png" . $cache;
                    }else if($tmp_theme_color == "skin-light"){
                        $theme_img = $AGENT_HOST . "/images/theme/light-theme.png" . $cache;
                    }else{
                        $theme_img = $AGENT_HOST . "/images/theme/default-theme.png" . $cache;
                    } 
                ?>
            ...
            <div class="text-right pro_link_div m-t-15">
                ...
                <?php if ($contract_business_image != "" && file_exists($AGENTS_BRAND_ICON . $contract_business_image)) { ?>
...
<script type="text/javascript">
	$(document).ready(function() {
        $('#btn_apply').on('click',function(e){
            ...
            $.ajax({
                url: '<?=$AGENT_HOST?>/ajax_change_theme_color.php',
                data: {theme:theme},
                type: 'POST',
                dataType: 'json',
                beforeSend: function () {
        ...
        $('input:radio[name=theme_color]').click(function(){
            ...
            if(value == 'skin-default'){
                image_name = "<?=$AGENT_HOST?>/images/theme/default-theme.png<?=$cache?>";
            } 
            if(value == 'skin-dark'){
                image_name = "<?=$AGENT_HOST?>/images/theme/dark-theme.png<?=$cache?>";
            }
            if(value == 'skin-light'){
                image_name = "<?=$AGENT_HOST?>/images/theme/light-theme.png<?=$cache?>";
            }  
		...
		<?php
        $currentImage = '';
        if ($contract_business_image != "" && file_exists($AGENTS_BRAND_ICON . $contract_business_image)) {
        ...
        var remaingContractDropzone = new Dropzone("#enrollment_profile .profile-dropzone", {
            ...
            init: function () {
                ...
                if (this.options.customErrorHandlingCode == 200) {
                    ...
                    $.ajax({
                        url: '<?=$AGENT_HOST?>/ajax_update_rep_business_picture.php?id=' + $agent_id + '&is_my_profile=Y&personal_branding=Y',
    ...
    function delete_business_image() {
        if (confirm('Are you sure you want to delete business logo?')) {
            $.ajax({
                url: '<?=$AGENT_HOST?>/delete_business_image.php?id=<?=$agent_id?>&personal_branding=Y',
                type: 'POST',
                dataType: 'json',
                beforeSend: function () {
                    $('#ajax_loader').show();
