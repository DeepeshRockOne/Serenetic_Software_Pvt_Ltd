...
try {
    ...
    $userId = checkIsset($_REQUEST['customer_id']);
    $userIncr = "";
    if(!empty($userId)){
        $userIncr .= " OR (
            ((af.user_id=:userId AND af.user_type='Customer') OR (af.entity_id=:userId AND af.entity_type='Customer')) AND af.entity_action IN('Agent Invited','Agent Invite Accepted','Agent Submitted Documentation','Review Documentation','Agent Documentation Rejected','Joined','Welcome email delivered','Invited Group','Group Signed Contract','Group Invite Accepted')
                        )";
        $sch_params[':userId'] = $userId;
    }

    $sel_sql = "SELECT af.* FROM activity_feed af
                WHERE 
                af.entity_action!='New Order' AND 
                ((user_id=:id AND af.user_type='Lead') OR (entity_id=:id AND af.entity_type='Lead') $userIncr) AND
