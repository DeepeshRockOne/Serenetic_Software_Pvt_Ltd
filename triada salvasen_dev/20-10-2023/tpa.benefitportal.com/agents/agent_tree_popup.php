...
$agentId =$_GET['agent_id'];
  ...
  if(!empty($resAgent)){
    ...
    $agentSql = "SELECT c.id as agentId,c.rep_id as agentDispId,c.email as agentEmail,concat(c.fname,' ', c.lname) as agentName 
                FROM customer c
                JOIN customer_settings cs on (cs.customer_id=c.id)
                WHERE c.type='Agent' AND c.status='Active' AND c.is_deleted='N' AND cs.agent_coded_level!='LOA' AND cs.agent_coded_id >= :codedId AND c.id!=:sponsorId AND c.id!=:agentId";
