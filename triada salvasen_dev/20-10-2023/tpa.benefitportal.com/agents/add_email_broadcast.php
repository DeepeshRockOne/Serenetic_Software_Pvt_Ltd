...
$breadcrumbes[2]['title'] = "Email Broadcaster";
$breadcrumbes[2]['link'] = 'add_email_broadcast.php';

$agent_id = $_SESSION["agents"]["id"];
$summernote = "Y";
$company_id = 3;
$is_clone = isset($_GET['is_clone']) ? $_GET['is_clone'] : "N";
$templateSql = "SELECT id,title FROM trigger_template WHERE is_deleted='N' AND is_default='Y'";
$templatedata = $pdo->selectOne($templateSql);
$email_attachment_id = 0;
$attachmentRow = array();
$broadcast_from_address = $emailer_settings[$company_id]['tg_from_mailid'];

// product code start
  $company_arr = get_active_global_products_for_filter($agent_id);
// product code ends

// Agent, Member value code start
  $agent_res = array();
  $agent_count = 0;

  $selAgent = "SELECT id,rep_id,CONCAT(fname,' ', lname) as name,type FROM customer WHERE is_deleted='N' AND status NOT IN ('Customer Abandon','Pending Quote','Invited') AND type IN ('Agent') AND upline_sponsors LIKE '%,$agent_id,%'";
  $resAgent = $pdo->select($selAgent);
  if(!empty($resAgent)){
    foreach ($resAgent as $key => $row) {
        $agent_res[$agent_count] = $row;
        $agent_count++;
    }
  }
  ...
  $member_res = array();
  $member_count = 0;
  $selCustomer = "SELECT id,rep_id,CONCAT(fname,' ', lname) as name,type FROM customer WHERE is_deleted='N' AND status NOT IN ('Customer Abandon','Pending Quote','Invited') AND type IN ('Customer') AND sponsor_id IN ($agent_id)";
...
// Leads Value Start
  ...
  $lead_res = $pdo->select($lead_tag_sql,array(":sponsor_id" => $agent_id));
...
if(isset($_GET['broadcaster_id']) && !empty($_GET['broadcaster_id'])){
  ...
  $broadcaster_res = $pdo->selectOne("SELECT id, brodcast_name, display_id, from_address, subject, user_type, email_template_id, mail_content, is_for_specific, admin_level, specific_user_ids, product_ids, product_status,agent_status,lead_status, lead_tags, is_schedule_in_future, status FROM broadcaster WHERE type='email' AND md5(id) = :broadcaster_id", array(":broadcaster_id" => $broadcaster_id));
  ...
  if(!empty($broadcaster_res)){
    ...
    $specific_user_ids_array = !empty($broadcaster_res['specific_user_ids']) ? $broadcaster_res['specific_user_ids'] : "";
    ...
    $lead_status = !empty($broadcaster_res['lead_status']) ? $broadcaster_res['lead_status'] : "";
    $lead_tags_array = !empty($broadcaster_res['lead_tags']) ? explode(",", $broadcaster_res['lead_tags']) : array();
    $agent_status = !empty($broadcaster_res['agent_status']) ? explode(",", $broadcaster_res['agent_status']) : array();
    ...
    $description['ac_message'] = array(
      ...
      'ac_red_2'=>array(
          'href'=>$AGENT_HOST.'/add_email_broadcast.php?broadcaster_id='.md5($broadcaster_res['id']),
          'title'=>$broadcaster_res['display_id'],
      ),
    ); 

    activity_feed(3, $_SESSION['agents']['id'], 'Agent', $broadcaster_res['id'], 'email_broadcaster','Read Email Broadcaster', $_SESSION['agents']['fname'],$_SESSION['agents']['lname'],json_encode($description));
