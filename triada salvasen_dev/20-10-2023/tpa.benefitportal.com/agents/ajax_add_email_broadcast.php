<?php include_once dirname(__FILE__) . '/layout/start.inc.php';
  ...
  $specific_agents = isset($_POST['specific_agents']) ? implode(',',$_POST['specific_agents']) : "";
  $specific_group = isset($_POST['specific_group']) ? implode(',',$_POST['specific_group']) : "";
  $specific_member = isset($_POST['specific_member']) ? implode(',',$_POST['specific_member']) : "";
  $specific_leads = isset($_POST['specific_leads']) ? implode(',',$_POST['specific_leads']) : "";
    ...
    if ($validate->isValid()) {
      if(empty($action_type) || (!empty($action_type)) && $action_type != 'send_email'){
        ...
        $insert_param = array(
          ...
          'from_name' => !empty($_SESSION['agents']['public_name']) ? $_SESSION['agents']['public_name'] : ($_SESSION['agents']['fname'] . " " . $_SESSION['agents']['lname']),
        ...
        if(!empty($update_broadcaster_id) && $is_clone == 'N'){
          ...
          $broadcaster_res = $pdo->selectOne("SELECT id,display_id,brodcast_name,from_address,from_name,subject,mail_content,user_type,is_for_specific,email_template_id,is_schedule_in_future,admin_level,specific_user_ids,lead_tags,product_ids,product_status,status FROM broadcaster WHERE id = :broadcaster_id", array(":broadcaster_id" => $update_broadcaster_id));
        ...
        if(isset($update_broadcaster_id) && !empty($update_broadcaster_id) && $is_clone == 'N'){
          ...
          if(!empty($checkDiff)){
            ...
            $activityFeedDesc['ac_message']['ac_red_2']=array(
              'href'=>$AGENT_HOST.'/add_email_broadcast.php?broadcaster_id='.md5($update_broadcaster_id),
              'title'=>$broadcaster_res['display_id']
            ); 
            
            activity_feed(3, $_SESSION['agents']['id'], 'Agent', $update_broadcaster_id, 'email_broadcaster','Agent Updated Email Broadcaster', $_SESSION['agents']['fname'],$_SESSION['agents']['lname'],json_encode($activityFeedDesc),'',json_encode($extraJson));
          }
          ...
          $description['ac_message'] =array(
            ...
            'ac_red_2'=>array(
                'href'=>$AGENT_HOST.'/add_email_broadcast.php?broadcaster_id='.md5($broadcaster_id),
                'title'=>$display_id,
            ),
          );

          activity_feed(3, $_SESSION['agents']['id'], 'Agent', $broadcaster_id, 'email_broadcaster','Agent Created Email Broadcaster', $_SESSION['agents']['fname'],$_SESSION['agents']['lname'],json_encode($description));
        }
      }
      ...
      if(!empty($action_type) && $action_type == 'send_email'){
        ...
        $params['EMAILER_SETTING']['from_mail_name'] = !empty($_SESSION['agents']['public_name']) ? $_SESSION['agents']['public_name'] : ($_SESSION['agents']['fname'] . " " . $_SESSION['agents']['lname']);
        $emailSubject = !empty($broadcast_subject) ? $broadcast_subject : 'Test Send Mail';
        $attachments = array();
        
        if(!empty($email_attachment_id) && !empty($broadcaster_id)){
