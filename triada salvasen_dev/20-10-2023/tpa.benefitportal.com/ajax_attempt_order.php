...
$post_date = checkIsset($_POST['post_date']);
...
$validate->string(array('required' => true, 'field' => 'post_date', 'value' => $post_date), array('required' => 'Select Payment Date'));


$order_row = $pdo->selectOne("SELECT id,display_id,customer_id,is_renewal,post_date,status,future_payment FROM orders WHERE id=:order_id AND status IN ('Post Payment','Payment Declined')", array(":order_id" => $order_id));
if(empty($order_row)) {
    setNotifyError("Order Not Found");
    $response['order_not_found'] = "Order Not Found";
...
if($validate->isValid()){
    ...
    if(!empty($customer_id) && !empty($lead_id) && !empty($order_id)) {
        ...
        if(!empty($already_submitted)) {
            $lead_track = array(
                'status' => 'Enrollment',
                'description' => 'Enrollment Application Already Submitted - ajax_attempt_order',
            );
    ...
    $cust_row = $pdo->selectOne("SELECT c.id,c.sponsor_id,c.email,c.fname,c.lname,c.address,c.city,c.state,c.zip,c.status FROM customer c WHERE c.id = :id", array(":id" => $customer_id));
    ...
    $function_list->setDefaultBillingProfile($customer_id,$cb_profile_id);
    ...
    if(strtotime(date('Y-m-d',strtotime($post_date))) <= strtotime($today_date)) {
    ...
    if($order_row['status'] == 'Payment Declined'){
        $other_params=array();
        $transId = $function_list->transaction_insert($order_id,'Credit','Post Payment','Post Transaction',0,$other_params);
    }

    if($order_row['future_payment'] == "Y") {
        if(strtotime($post_date) != strtotime($order_row['post_date'])) {
            $activity_description['key_value']['desc_arr']['Post Date'] = ' updated from '.displayDate($order_row['post_date']).' to '.displayDate($post_date);
            $activity_update = true;
        }
    }

    /*--- update customer status ---*/
    if(in_array($cust_row['status'],array("Pending Validation","Pending Quote"))) {
        $member_setting = $memberSetting->get_status_by_payment(true,"",true,$cust_row['status']);
    ...
    $response["msg"] = "Billing Information Saved Successfully";
    ...
    if($activity_update){
        if($location == "admin") {
            ...
            $af_data['ac_message'] =array(
                ...
                'ac_message_1' =>' updated Billing Information on Order '.$order_row['display_id'],
            );
        ...
        if(!empty($activity_description)) {
            ...
            activity_feed(3,$cust_row['sponsor_id'],'Agent',$lead_id, 'Lead', 'Billing Information Updates', $cust_row['fname'], $cust_row['lname'], json_encode($af_data), $REQ_URL);
        }
    }
    ...
    if(strtotime(date('Y-m-d',strtotime($post_date))) <= strtotime($today_date)) {
        ...
        if($location == "admin") {
            ...
            $af_data['ac_message'] =array(
                ...
                'ac_message_1' =>' attempted Post Date Order '.$order_row['display_id'].' <br/>',
                'ac_message_2' =>' Billed to '.$billing_detail
            );
            activity_feed(3,$_SESSION['admin']['id'],'Admin',$lead_id, 'Lead', 'Attempted Order', $cust_row['fname'], $cust_row['lname'], json_encode($af_data), $REQ_URL);
            ...
            activity_feed(3,$_SESSION['agents']['id'],'Agent',$lead_id, 'Lead', 'Attempted Order', $cust_row['fname'], $cust_row['lname'], json_encode($af_data), $REQ_URL);
            ...
            activity_feed(3,$_SESSION['agents']['id'],'Group',$lead_id, 'Lead', 'Attempted Order', $cust_row['fname'], $cust_row['lname'], json_encode($af_data), $REQ_URL);
        }      

        $function_list->generatePostOrder($order_id);
        $order_row = $pdo->selectOne("SELECT id,status FROM orders WHERE id=:order_id", array(":order_id" => $order_id));
        $response["status"] = "success";
        if($order_row['status'] == "Payment Approved") {
            setNotifySuccess("Order Attempted Successfully");
            $response["msg"] = "Order Attempted Successfully";
        }            
        if($order_row['status'] == "Payment Declined") {
            setNotifyError("Payment Failed");
        ...
        if(!empty($lead_tracking_id)) {
            ...
            $tracking_data = array(
                ...
                'order_status' => (isset($order_row['status'])?$order_row['status']:''),
            );
