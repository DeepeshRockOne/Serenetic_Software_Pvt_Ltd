...
$orderRow = $pdo->selectOne("SELECT o.id,o.display_id,o.is_renewal,o.customer_id,c.fname,c.lname,c.rep_id FROM orders as o JOIN customer as c ON (c.id = o.customer_id) WHERE md5(o.id) = :id AND md5(c.id)=:cid AND o.status NOT IN('Pending Settlement','Payment Approved')", array(":id" => $orderId,":cid"=>$customer_id));

if(empty($orderRow['id'])){
    echo "<script>
    parent.setNotifyError('Order status outdated');
