...
$test_date = !empty($_GET['test_date']) ? date('Y-m-d',strtotime($_GET['test_date'])) : '';
$function_list = new functionsList();
$currentDate = date('Y-m-d');
if(!empty($test_date)){
    $currentDate = $test_date;
}
// $currentDate = date('2023-05-17');
...
if ((date('l', strtotime($currentDate)) == 'Saturday' || date('l', strtotime($currentDate)) == 'Sunday' || in_array($currentDate, $publicHolidaysArr))) {
    ...
    $payDateArr = $function_list->getPayDateForHrmPayment($currentDate);
    if(empty($payDateArr)){
        echo "exit";
    }
}
...
$payDateSql = "SELECT c.id AS customerId,gc.pay_period,cs.class_id,gcp.paydate AS class_paydate,cgs.billing_type,s.id AS sponsorId,lb.id AS listBillId
                ...
                WHERE c.is_deleted='N' AND c.type='Customer' AND gcp.paydate IN("."'".implode("','", $payDateArr)."'".") GROUP BY lb.id,IF(cgs.billing_type='individual',c.id,s.id)";
