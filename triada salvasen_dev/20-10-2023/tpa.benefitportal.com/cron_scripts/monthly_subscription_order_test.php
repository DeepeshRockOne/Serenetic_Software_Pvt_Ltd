...
$today = date('Y-m-d');
$today = date('Y-m-d',strtotime($_GET['dateCheck']));
...
if (count($AutoRows) > 0) {
    ...
    foreach ($AutoRows as $autorow) {
        ...
        if ($allow_process) {
            ...
            if ($payment_approved) {
                ...
                $txn_id = $payment_res['transaction_id'];
                $other_params=array("transaction_id"=>$txn_id,"req_url" => "cron_scripts/monthly_subscription_order.php",'transaction_response'=>$payment_res,"transaction_date"=>$today);
        ...
        if (!$allow_process && $decline_type != "") {
                ...
                foreach ($ProfileRows as $row) {
                    ...
                    if ($attemptRow) {
                        ...
                        $ac_descriptions_ti['ac_message'] =array(
                            ...
                            'ac_message_1' =>' E-Ticket Opened For Plan '.$row['website_id']
                        );
                    ...
                    if ($fail_trigger_id > 0) {
                        ...
                        $ac_descriptions_tri['ac_message'] =array(
                            ...
                            'ac_message_1' =>' Failed Renewal For Plan '.$row['website_id']
                        );
            ...
            //************************ insert transaction code start ***********************
            $txn_id = $payment_res['transaction_id'];
            $other_params=array("transaction_id"=>$txn_id,"req_url" => "cron_scripts/monthly_subscription_order.php","reason"=>checkIsset($payment_error),'cc_decline_log_id'=>checkIsset($decline_log_id));
    ...
    if (count($sendEmailSummary)) {
        ...
        trigger_mail_to_email($sendEmailSummary, $DEFAULT_ORDER_EMAIL, "Op29 : Renewals Order");
    }
}
