...
// Header Added to both files Main File and Eligibility File 
  ...
  $selSql = "SELECT c.rep_id,c.fname,c.lname,c.address,c.address_2,c.city,c.zip,c.state,c.cell_phone,c.email,c.gender,c.birth_date,w.eligibility_date,w.termination_date,p.name,p.product_code as product_id,p.name as product_name,w.active_date,s.rep_id as enrolling_agent,w.website_id,ppt.title as benefit_tier,ce.id as cust_enrollment_id,w.created_at as added_date,p.id as p_id,ppc.plan_code_value as group_code,ce.first_eligibility_date,ce.most_recent_date 
    ...
    WHERE c.is_deleted='N' AND c.status in ('Active','Inactive') AND c.type='Customer' AND w.status not in('Pending','Post Payment') AND (p.id IN ($prd_ids) OR p.parent_product_id IN($prd_ids)) $incr GROUP BY w.id";
