...
include_once dirname(__DIR__) . '/includes/function.class.php';
  ...
  /*---------- System script status code start -----------*/
    ...
    $cronWhere = array(":script_code" => "regenerate_commission");
    $cronRow = $pdo->selectOne($cronSql,$cronWhere);
      if(!empty($cronRow['is_running']) && $cronRow['is_running'] == "Y") {
        ...
        $cronWhere = array(
                          ...
                          "params" => array(
                              ":script_code" => 'regenerate_commission'
                          )
                      );
  ...
  $selRegenComm = "SELECT * FROM regenerated_commission WHERE status='Pending' AND is_cancelled='N' 
  AND is_process_active='N' LIMIT 0,1";
  $resRegenComm = $pdo->selectOne($selRegenComm);

  $orderIds = checkIsset($resRegenComm["order_ids"]);
  $regenerateCommId = checkIsset($resRegenComm["id"]);
  $commPeriod = checkIsset($resRegenComm["comm_period"]);
  
  if(!empty($orderIds)){
  /*---------- Fetch Regenerate Commission Orders Code START -----------*/
    $odrSql = "SELECT 
      o.id,ws.customer_id,IF(o.is_renewal='L',od.is_renewal,o.is_renewal) as is_renewal,o.created_at as odrDate,
      ...
      WHERE c.type IN ('Customer','Group')
      AND s.id > 1 AND s.type in('Agent','Group') AND od.is_refund='N' 
      AND o.status IN('Payment Approved','Completed','Pending Settlement') 
      AND p.product_type!='ServiceFee' AND o.id IN(".$orderIds.") 
      GROUP BY od.id
      ORDER BY o.id ASC";
    $odrRes = $pdo->select($odrSql);
    echo "Orders Found:" . count($odrRes) . "<br>";
  /*---------- Fetch Regenerate Commission Orders Code Ends ------------*/
  }
  // pre_print($odrRes);
  if(count($odrRes) > 0){

    $updParams = array("is_process_active"=>"Y","status"=>"Processing");
    $updWhere = array(
                "clause" => "id=:id", 
                "params" => array(
                    ":id" => $regenerateCommId
    ...
    $pdo->update('regenerated_commission',$updParams,$updWhere);

    foreach ($odrRes as $order) {

      if($commPeriod == 'earned_period'){
        $payDate = date('Y-m-d',strtotime($order["odrDate"]));
      }else{
        $payDate = date('Y-m-d');
      }
      
      $weeklyPayPeriod = $commObj->getWeeklyPayPeriod($payDate);
      $monthlyPayPeriod = $commObj->getMonthlyPayPeriod($payDate);
      ...
      $selTransactions = "SELECT id FROM transactions WHERE order_id=:odrID ORDER BY id DESC";
        ...
        $pmpmCommissions = $commObj->getPmpmCommissionRules($order['product_id'],$order['customer_id'],$order['id'],$order['is_renewal'],$renewCount,$pmpmExtra);
          ...
          if(!empty($pmpmCommissions)){
            foreach ($pmpmCommissions as $key => $pmpmComm){
              ...
              $regeneratePMPM = false;
              $regenerate = $commObj->regenerateReverseCommission($pmpmComm['amount'],$order['id'],$order['order_detail_id'],$order['customer_id'],$pmpmComm['agentId'],$order['plan_id'],"PMPM",$regenerateCommId,$commRevCount,$order["transaction_tbl_id"]);
              if($regenerate == "Y"){
                $regeneratePMPM = true;
              }
              if($regeneratePMPM){
                $pmpmSql = array(
                    ...
                    "regenerate_comm_id" => $regenerateCommId,
                ...
                if($amount > 0) {
                    ...
                    // Graded commission code start
                      ...
                      if(checkIsset($gradedOn) == "Y"){
                        ...
                        if(!empty($gradedRuleRow) && $gradedRuleRow["charged_to"] == "Agents"){
                          ...
                          $commObj->generateGradedFeeCommission($insCommSql, $commRules, $gradedRuleRow,$stepFeePrdIds,$order["is_renewal"],$renewCount,$gradedArr);
                        }
                      }
                    ...
                    /*---------- generating renewal order commission ---------*/
                      if ($order['is_renewal'] == "Y") {
                        ...
                        $regenerateEarnComm = false;
                        $regenerate = $commObj->regenerateReverseCommission($amount,$order['id'],$order['order_detail_id'],$order['customer_id'],$agentId,$order['plan_id'],"Renewals",$regenerateCommId,$commRevCount,$order["transaction_tbl_id"]);
                        if($regenerate == "Y"){
                          $regenerateEarnComm = true;
                        }

                        if($regenerateEarnComm){
                    ...
                    /*---------- generating New order commission ---------*/
                        ...
                        $regenerateEarnComm = false;
                        $regenerate = $commObj->regenerateReverseCommission($amount,$order['id'],$order['order_detail_id'],$order['customer_id'],$agentId,$order['plan_id'],"New",$regenerateCommId,$commRevCount,$order["transaction_tbl_id"]);
                        ...
                        /*--------------- Generate Advance commission Code Start ------------------*/
                          ...
                          if(checkIsset($advanceOn) == "Y"){
                            ...
                            if(!empty($advanceRuleRow)){
                              ...
                              $commObj->generateAdvanceCommission($insCommSql, $commRules, $advanceRuleRow,$stepFeePrdIds,$advArr);
  ...
  /*----------------------------------------------------------*/
    ...
    $commTotalRes = $pdo->selectOne("SELECT sum(amount) as totalAmt FROM commission WHERE regenerate_comm_id=:regenerate_comm_id",array(":regenerate_comm_id"=>$regenerateCommId));
    $commissionTotal = !empty($commTotalRes["totalAmt"]) ? $commTotalRes["totalAmt"] : "";

    /*--------- UPDATE REGENERATE STATUS ----------*/
    $updParams = array("status" => "Completed",
                      "regeneration_date" => "msqlfunc_NOW()",
  ...
  /*-------------------------------------------------*/
   ...
   /*--------- System script status code start ----------*/
    ...
    $cronUpdParams = array("is_running" => "N",
                          "next_processed"=>date("Y-m-d H:i:s",strtotime("+60 minutes", strtotime($cronRow['last_processed'])))
                          );
    $cronWhere = array(
                "clause" => "script_code=:script_code", 
                "params" => array(
