...
if($file_type == "full_file"){
  ...
  // $incr .= " AND (DATE(c.created_at) <= :to_date OR w.term_date_set <= :to_date OR w.eligibility_date <= :to_date)";
  $file = (__DIR__) . "/" .$req_row['file_name'] . "_Main_File.csv";

  if(!file_exists($file)){
    $file = $req_row['file_name'] . "_Main_File.csv";
    fopen($file,"w");  
  }
...
$header_row = "PLAN_NUMBER" . $csv_seprator .
        ...
        "ENROLLING_AGENT" . $csv_seprator .
        "GROUP_CODE" . $csv_seprator .
        "PLAN_CODE1" . $csv_seprator .
        "PLAN_CODE2" . $csv_seprator .
        "PLAN_CODE3" . $csv_seprator .
        "PLAN_CODE4" . $csv_seprator .
        "PLAN_CODE5" . $csv_seprator .
        "PLAN_CODE6" . $csv_seprator .
        "PLAN_CODE7" . $csv_seprator .
        "PLAN_CODE8" . $csv_seprator .
        "PLAN_CODE9" . $csv_seprator .
        "PLAN_CODE10" . $csv_line;
// Header Added to both files Main File and Eligibility File 
  ...
  $selSql = "SELECT c.rep_id,c.fname,c.lname,c.address,c.address_2,c.city,c.zip,c.state,c.cell_phone,c.email,c.gender,c.birth_date,w.eligibility_date,w.termination_date,p.name,p.product_code as product_id,p.name as product_name,w.active_date,s.rep_id as enrolling_agent,w.website_id,ppt.title as benefit_tier,ce.website_id as cust_enrollment_id,w.created_at as added_date,p.id as p_id,ppc.plan_code_value as group_code,ce.id as ce_id 
    FROM customer c 
    ...
    JOIN prd_plan_type ppt ON (pm.plan_type=ppt.id)
    LEFT JOIN prd_plan_code ppc ON(ppc.product_id = p.id AND ppc.is_deleted = 'N' AND code_no = 'GC')
    WHERE c.is_deleted='N' AND ce.is_fulfillment = 'N' AND c.status in ('Active','Inactive') AND c.type='Customer' AND w.status not in('Pending Declined','Pending Payment') AND (p.id IN ($prd_ids) OR p.parent_product_id IN($prd_ids)) $incr GROUP BY w.id";
  ...
  foreach ($mecRows as $key => $row) {
      ...
      if($row['ce_id']){
        ...
        $upd_data = array(
          ...
          'fullfillment_csv_file_name' => $csv_filename,
          'fulfillment_admin_id' => $req_row['user_id'],
        );
        $pdo->update("customer_enrollment",$upd_data,$upd_where);
      }
