      ...
      /*---------- Coverage Date Validation Start ---------*/ 
        foreach ($selected_product as $product_id) {
          ...
          if($product_type[$product_id] == 'Fees'){ 
            $start_coverage_date[$product_id] = isset($start_coverage_date[$fee_applied_for_product[$product_id]])?$start_coverage_date[$fee_applied_for_product[$product_id]]:'';
            $ord_regenerate_product[$product_id] = isset($ord_regenerate_product[$fee_applied_for_product[$product_id]])?$ord_regenerate_product[$fee_applied_for_product[$product_id]]:'';
          }
          ...
          if(empty($ord_regenerate_product[$product_id]) || strtotime($ord_regenerate_product[$product_id]) < 0) {
            $validate->setError('ord_regenerate_product_'.$product_id,'Please select coverage date');
          } else {
            if(strtotime($ord_regenerate_product[$product_id]) < strtotime($dateObj->default_effective_from)) {
              $validate->setError('ord_regenerate_product_'.$product_id, 'Coverage date must be greater than or equal to ' . date('m/d/Y', strtotime($dateObj->default_effective_from)));
            } else {
    ...
    if ($validate->isValid()) {
      ...
      $order_row = $pdo->selectOne($order_sql, $order_where);
        ...
        if (count($order_row) > 0 && !empty($coverage_dates)) {
          ...
          $functionLst->create_reorder($order_row['id'], $order_row['customer_id'], $payment_method, $post_payment_date, $order_row['is_renewal'], $coverage_dates,$order_display_id,$other_params);
        }
