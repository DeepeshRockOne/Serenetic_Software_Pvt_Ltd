...
if($location == 'admin') {
    ...
    $tz = new UserTimeZone('M d, Y h:i:s A T', $_SESSION['agents']['timezone']);
...
if($resOrder){
	...
	$active_products = $pdo->select("SELECT IF(p.name = '' AND p.product_type = 'ServiceFee','Service Fee',p.name) AS name,p.id as p_id,w.id as w_id,w.website_id,w.plan_id,w.price 
		FROM orders o
		...
		WHERE o.id = :order_id and w.termination_date IS NULL AND od.is_refund = 'N' order by p.type DESC,od.product_name ASC",array(':order_id' => $orderId));
	              ...
	              <ul class="list-unstyled mn">
	                <li>
	                  <div class="phone-control-wrap">
	                    ...
	                    <div class="phone-addon text-left">Reverse commissions for the voided product</div>
	                  </div>
	                </li>
	              </ul>
	            </div>
	              ...
	              <ul class="list-unstyled mn">
	                <li>
	                  <div class="phone-control-wrap m-b-25">
	                    <div class="phone-addon w-25"><i class="text-success fs18 fa fa-check-circle"></i></div>
	                    <div class="phone-addon text-left">Reverse commissions for the refunded product</div>
	                  </div>
	                </li>
	                <li>
	                  <div class="phone-control-wrap m-b-25">
	                    <div class="phone-addon w-25"><i class="text-success fs18 fa fa-check-circle"></i></div>
	                    <div class="phone-addon text-left">Set a termination date for the refunded products</div>
	                  </div>
	                </li>
	              </ul>
	            </div>
